...
if [ -z "$pid" ]; then
    ...
    echo "copy maven repository..."
    tar -xf /repository.tar -C $home/../assembly
    rm /repository.tar
    # 执行
    java -server ${MEM_OPTS} ${JAVA_OPTS} -Duser.timezone=${suposTimeZone} -Dorchid.env=dev -Dintegration.supos.enabled=true -jar bootstrap.jar
else
