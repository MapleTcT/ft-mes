...
import com.supcon.supfusion.framework.scaffold.auditlog.constant.FilterField;
...
public class ClassExUtil extends ObjectUtils {
     ...
     * 业务主键对应字段
     */
    public static final String AUDIT_BUSINESS_KEY = "AUDIT_BUSINESS_KEY";
    public static final String HAS_AUDIT_BUSINESS_KEY = "HAS_AUDIT_BUSINESS_KEY";
    ...
    public static Map<String, Object> modelToMap(@NonNull Object model) throws IllegalAccessException {
        ...
        for (Field field : allFields) {
            ...
            // 是否是需要忽略的字段
            if (isIgnoreFiled(field, value) || FilterField.filterFieldSet.contains(fieldName)) {
                continue;
            }
            // 如果是基本类、包装类或字符串
            if (ClassUtils.isPrimitiveOrWrapper(fieldClazz) || String.class == fieldClazz || ClassUtils.isPrimitiveArray(fieldClazz)) {
                ...
                // 判断字段是否是业务主键
                if (field.isAnnotationPresent(AuditBusinessKey.class) && value != null) {
                    modelMap.put(AUDIT_BUSINESS_KEY, value.toString());
                    modelMap.put(HAS_AUDIT_BUSINESS_KEY, true);
                }
                continue;
            }
            // 如果是Date，存储时间戳
            if (Date.class == fieldClazz) {
                ...
                modelMap.put(fieldName, dtf.format(date));
                continue;
            }
            // 如果是BigDecimal或枚举类型
     ...
     * @param field 字段
     * @param value 字段值
     * @see AuditJoinPK 通过该注解申明连接对象的主键字段名称，如果没有使用AuditJoinPK注解，会尝试以对象的id作为主键字段
     * 优先级： AuditJoinPK  > id
     */
     ...
     * @return 模型信息
     */
    public static ModelObjectInfo convertModelInfo(AuditDataLogBO auditLog) {
        ...
        try {
            ...
            Object id = modelMap.get(FIELD_ID);
            if (id != null) {
                modelObjectInfo.setModelObjPk(id.toString());
            }
            String code = (String)modelMap.get(FIELD_CODE);
            if (code != null) {
                modelObjectInfo.setModelObjCode(code);
            }
            String name = (String)modelMap.get(FIELD_NAME);
            if (name != null) {
                modelObjectInfo.setModelObjName(name);
            }
            String businessKey = (String) modelMap.get(AUDIT_BUSINESS_KEY);
            if (businessKey != null) {
                modelObjectInfo.setBusinessKey(businessKey);
            }
            Boolean hasBusinessKey = (Boolean) modelMap.get(HAS_AUDIT_BUSINESS_KEY);
            if (hasBusinessKey != null) {
                modelObjectInfo.setHasBusinessKey(hasBusinessKey);
            }
        } catch (IllegalAccessException e) {
