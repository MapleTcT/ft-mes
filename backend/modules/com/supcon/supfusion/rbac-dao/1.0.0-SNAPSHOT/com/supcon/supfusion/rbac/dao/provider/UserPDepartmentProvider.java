...
public class UserPDepartmentProvider {

    public String deleteUserPDepartment(@Param("cid") Long cid, @Param("opIds") List<Long> opIds, @Param("roleId") Long roleId, @Param("userId") Long userId){
        StringBuilder sql = new StringBuilder();
        sql.append("<script>");
        sql.append("DELETE FROM rbac_userpdepartment WHERE USERPERMISSION_ID in (SELECT up.ID FROM rbac_userpermission up WHERE up.CID=#{cid}");
