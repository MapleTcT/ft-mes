package com.supcon.supfusion.systemconfig.service.utils;

import com.google.common.collect.Lists;
import com.supcon.supfusion.systemconfig.api.dto.CatalogDTO;
import com.supcon.supfusion.systemconfig.api.dto.ConfigDTO;
...
public class XMLParseService {
    public static Logger LOG = LoggerFactory.getLogger(XMLParseService.class);
...
//        InputStream xmlInputStream = getXmlInputStream("D:\\a.xml.txt");
    ...
    public static CatalogDTO getConfigVOByXml(String xmlString) throws Exception {
        InputStream stringStream = getStringStream(xmlString);
        //2-解析XML-IO流 ，获取Document 对象，以及Document对象 的根节点
        Element rootElement = getRootElementFromIs(stringStream);
        //3~5-从根元素解析得到元素
        List<CatalogDTO> catalogDTOList = parseElementFromRoot(rootElement);
        if (ObjectUtils.isEmpty(catalogDTOList)) {
            return null;
        }
    ...
    //3-从根元素解析得到元素
    private static List<CatalogDTO> parseElementFromRoot(Element root) {
        List<CatalogDTO> catalogVOS = new ArrayList<>();
...
//                getDataFromElement(ele);
                ...
                CatalogDTO catalogVO = getCertainElementFromParentElement(ele);
                if (!ObjectUtils.isEmpty(catalogVO)) {
                    catalogVOS.add(catalogVO);
                }

                //国际化
                try {
                    if ("zh_CN".equals(ele.getAttribute("locale"))) {
                        for (CatalogDTO catalogDTO : catalogVOS) {
                            if (ObjectUtils.isEmpty(catalogDTO)) {
                                continue;
                            }
                            String finalValue = getFinalName(catalogDTO.getName(), ele);
                            catalogDTO.setName(finalValue);
                            for (ConfigDTO configDTO : catalogDTO.getConfig()) {
                                String configName = getFinalName(configDTO.getName(), ele);
                                configDTO.setName(configName);

                                ConfigDTO.TypeConfig typeConfig = configDTO.getTypeConfig();
                                if (!ObjectUtils.isEmpty(typeConfig)) {
                                    String configDes = getFinalName(configDTO.getTypeConfig().getRemind(), ele);
                                    configDTO.getTypeConfig().setRemind(configDes);
                                    for (ConfigDTO.TypeConfig.OptionalValue optionalValue : typeConfig.getOptionalValue()) {
                                        optionalValue.setLabel(getFinalName(optionalValue.getLabel(), ele));
                                    }
                                }
                            }
                        }
                    ...
                    LOG.error("保存ocd文件时，解析xml国际化相关报错，catalogVOS：{}", catalogVOS);
                }
    ...
    //根据原始值获取中文
    private static String getFinalName(String startValue, Element ele) {
        if (!ObjectUtils.isEmpty(startValue) && startValue.startsWith("%")) {
            String nodeValue = ele.getFirstChild().getNodeValue();
            String trim = nodeValue.replace("\t", "~").replace("\n", "~").trim();
            ...
            while (m.find()) {
                int i = 1;
                String group = m.group(i).replace("~", "").trim();
                keyList.add(group);
                i++;
            }
            ...
            Pattern pattern1 = Pattern.compile("=(.*?)~");// 匹配的模式
            Matcher m1 = pattern1.matcher(trim);
            while (m1.find()) {
                int i = 1;
                String group = m1.group(i).replace("~", "").trim();
            ...
            for (int i = 0; i < keyList.size(); i++) {
                for (int j = 0; j < valueList.size(); j++) {
                    if (i == j) {
                        map.put(keyList.get(i), valueList.get(j));
                    }
                }
            }
            return map.get(startValue.split("%")[1]);
        }
        return startValue;
    }
    ...
    //5-从元素解析特定子元素并解析(以property为例)
    private static CatalogDTO getCertainElementFromParentElement(Element ele) {
        String moduleCode = ele.getAttribute("moduleCode");
        ...
        List<ConfigDTO> configVOList = new ArrayList<>();
        ...
        for (int i = 0; i < propertyEleList.getLength(); i++) {
            ...
            if (node instanceof Element) {
                ...
                String isCheckBox = propertyEle.getAttribute("isCheckBox");
                ...
                List<ConfigDTO.TypeConfig.OptionalValue> optionList = Lists.newArrayList();
                ...
                if (!ObjectUtils.isEmpty(optionEleList) && optionEleList.getLength() > 0) {
                    ...
                    if (!ObjectUtils.isEmpty(cardinality) && cardinality.equals("n")) {
                        configVO.setType(4);
                        if (!ObjectUtils.isEmpty(isCheckBox) && isCheckBox.equalsIgnoreCase("true")) {
                            configVO.setType(1);
                        } else if (!ObjectUtils.isEmpty(isCheckBox) && isCheckBox.equalsIgnoreCase("false")) {
                            configVO.setType(4);
                        }
                    }
                } else {
                ...
                for (int i1 = 0; i1 < optionEleList.getLength(); i1++) {
                    ...
                    if (item instanceof Element) {
                        ...
                        ConfigDTO.TypeConfig.OptionalValue optionalValue = new ConfigDTO.TypeConfig.OptionalValue();
                ...
                //typeConfig
                ConfigDTO.TypeConfig typeConfig = new ConfigDTO.TypeConfig();
                typeConfig.setOptionalValue(optionList);
                typeConfig.setRemind(description);
                configVO.setTypeConfig(typeConfig);

                //verify
                List<ConfigDTO.Verify> verifyList = new ArrayList<>();
                ConfigDTO.Verify verify = new ConfigDTO.Verify();
