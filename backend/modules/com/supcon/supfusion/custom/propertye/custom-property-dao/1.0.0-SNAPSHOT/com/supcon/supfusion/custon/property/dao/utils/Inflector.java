package com.supcon.supfusion.custon.property.dao.utils;
...
public class Inflector {
	...
	public static String underscore(String camelCasedWord) {
		StringBuffer buf = new StringBuffer(camelCasedWord.replace('.', '_'));
	...
	public String minus(String camelCasedWord) {
		String underscoredWord = UNDERSCORE_PATTERN_1.matcher(camelCasedWord).replaceAll("$1-$2");
		underscoredWord = UNDERSCORE_PATTERN_2.matcher(underscoredWord).replaceAll("$1-$2");
		underscoredWord = underscoredWord.toLowerCase();
...
//		for (String tmp : BAPNamingStrategy.entities) {
...
//		}
	...
	public String tableize(String prefix, String entityName) {
		return ((null != prefix ? prefix + "_" : "") + tableize(entityName)).toUpperCase();
	}
	
	public String columnize(String propertyName) {
		return underscore(propertyName).toUpperCase();
	}
	...
	public String columnToField(String columnName) {
		...
		String[] charArray = columnName.split("_");
		String fieldStr = charArray[0];
		for(int i = 1; i < charArray.length; i++) {
			char fl = ((String) charArray[i]).charAt(0);
			fieldStr += Character.toLowerCase(fl) + ((String) charArray[i]).substring(1);
		}
		return fieldStr;
	}
	...
	public static void main(String... args) {
		System.out.println(Inflector.instance.underscore("tableInfoId"));
	}
}
