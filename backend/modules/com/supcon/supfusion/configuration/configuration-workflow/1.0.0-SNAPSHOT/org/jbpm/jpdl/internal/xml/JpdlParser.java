...
//    setSchemaResources(SCHEMA_RESOURCES);
  ...
  public void parseTransitions(Element element, ActivityImpl activity, Parse parse) {
    ...
    for (Element transitionElement : transitionElements) {
      ...
      String transitionDescription = XmlUtil.attribute(transitionElement, "internationalKey");
      if(transitionDescription==null||transitionDescription.length()==0){
    	  transitionDescription = XmlUtil.attribute(transitionElement, "desc");
      }
      Integer select= XmlUtil.attributeInteger(transitionElement, "selectStaff", parse);
      Integer sequence= XmlUtil.attributeInteger(transitionElement, "sequence", parse);
      Integer requiredStaff= XmlUtil.attributeInteger(transitionElement, "requiredStaff", parse);
      Integer defaultSelectStaff= XmlUtil.attributeInteger(transitionElement, "defaultSelectStaff", parse);
      Integer notificationType= XmlUtil.attributeInteger(transitionElement, "notificationType", parse);
      Integer cancel= XmlUtil.attributeInteger(transitionElement, "cancel", parse);
      Integer reject= XmlUtil.attributeInteger(transitionElement, "reject", parse);
      Integer userRangeFalg= XmlUtil.attributeInteger(transitionElement, "userRangeFalg", parse);
...
//      }
      ...
      transition.setDescription(transitionDescription);
      transition.setSelectPeople(null == select ? 0 : select);
      transition.setRouterSequence(null == sequence ? 0 : sequence);
      transition.setNotificationType(null == notificationType ? 0 : notificationType);
      transition.setCancel(null == cancel ? 0 : cancel);
      transition.setRequiredStaff(null == requiredStaff ? 0 : requiredStaff);
      transition.setDefaultSelectStaff(null == defaultSelectStaff ? 0 : defaultSelectStaff);
      
      transition.setReject(null == reject ? 0 : reject);
      transition.setUserRangeFalg(null == userRangeFalg ? 0: userRangeFalg);
