...
import com.supcon.supfusion.configuration.workflow.entities.RolePPosition;
import com.supcon.supfusion.configuration.workflow.entities.RolePStaff;
import com.supcon.supfusion.configuration.workflow.entities.RolePermission;
import com.supcon.supfusion.configuration.workflow.entities.UserPPosition;
import com.supcon.supfusion.configuration.workflow.entities.UserPStaff;
...
@Service
@Transactional
public class ProcessServiceFlowImpl extends BaseServiceImpl implements ProcessServiceFlow {
...
//        Assert.notNull(deployments);
    ...
    public void deleteFlow(Deployment deployment, boolean flag, boolean flowFlag) {
        // 删除流程，包括deployment、menuinfo、menuoperate、pending
        Object[] params = new Object[2];
        params[0] = deployment.getProcessKey();
        params[1] = deployment.getProcessVersion();
        List<Deployment> deList = Collections.EMPTY_LIST;
        if(flowFlag) {
            deList = deploymentDao.findByCriteria(Restrictions.eq("processKey", deployment.getProcessKey()),
                    Restrictions.eq("processVersion", deployment.getProcessVersion()),Restrictions.eq("valid", true));
        }

        if(!flowFlag || (flowFlag && deList.isEmpty())) {
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_flow_permission_staff where FLOWPERMISSION_ID in (select id from rbac_flow_permission where ENTITY_CODE=?)",
                            deployment.getEntityCode()).addSynchronizedEntityClass(DataPermissionStaff.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_flow_permission_position where FLOWPERMISSION_ID in (select id from rbac_flow_permission where ENTITY_CODE=?)",
                            deployment.getEntityCode()).addSynchronizedEntityClass(DataPmsPosition.class).executeUpdate();
            pendingDao.createNativeQuery("delete from rbac_flow_permission where ENTITY_CODE=?", deployment.getEntityCode()).addSynchronizedEntityClass(DataPermission.class)
                    .executeUpdate();

            pendingDao
                    .createNativeQuery(
                            "delete from rbac_rolepstaff where ROLEPERMISSION_ID in (select dp.id from rbac_rolepermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.ENTITY_CODE=?))",
                            deployment.getEntityCode()).addSynchronizedEntityClass(RolePStaff.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_rolepposition where ROLEPERMISSION_ID in (select dp.id from rbac_rolepermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.ENTITY_CODE=?))",
                            deployment.getEntityCode()).addSynchronizedEntityClass(RolePPosition.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_rolepermission where MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.ENTITY_CODE=?)",
                            deployment.getEntityCode()).addSynchronizedEntityClass(RolePermission.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_userpstaff where USERPERMISSION_ID in (select dp.id from rbac_userpermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.ENTITY_CODE=?))",
                            deployment.getEntityCode()).addSynchronizedEntityClass(UserPStaff.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_userpposition where USERPERMISSION_ID in (select dp.id from rbac_userpermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.ENTITY_CODE=?))",
                            deployment.getEntityCode()).addSynchronizedEntityClass(UserPPosition.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_userpermission where MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.ENTITY_CODE=?)",
                            deployment.getEntityCode()).addSynchronizedEntityClass(UserPermission.class).executeUpdate();
        }

        if(!flowFlag || (flowFlag && !deList.isEmpty())) {
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_rolepstaff where ROLEPERMISSION_ID in (select dp.id from rbac_userpermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.FLOW_KEY=? and FLOW_VERSION=?))",
                            params).addSynchronizedEntityClass(RolePStaff.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_rolepposition where ROLEPERMISSION_ID in (select dp.id from rbac_rolepermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.FLOW_KEY=? and FLOW_VERSION=?))",
                            params).addSynchronizedEntityClass(RolePPosition.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_userpstaff where USERPERMISSION_ID in (select dp.id from rbac_userpermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.FLOW_KEY=? and FLOW_VERSION=?))",
                            params).addSynchronizedEntityClass(UserPStaff.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_userpposition where USERPERMISSION_ID in (select dp.id from rbac_userpermission dp where dp.MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.FLOW_KEY=? and FLOW_VERSION=?))",
                            params).addSynchronizedEntityClass(UserPPosition.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_rolepermission where MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where mp.FLOW_KEY=? and FLOW_VERSION=?)",
                            params).addSynchronizedEntityClass(RolePermission.class).executeUpdate();
            pendingDao
                    .createNativeQuery(
                            "delete from rbac_userpermission where MENUOPERATE_ID in (select mp.id from rbac_menuoperate mp where  mp.FLOW_KEY=? and FLOW_VERSION=?)",
                            params).addSynchronizedEntityClass(UserPermission.class).executeUpdate();
        }


        if(flag) {
            menuInfoService.deleteMenuOperateByFlowPhysical(deployment.getProcessKey(), String.valueOf(deployment.getProcessVersion()));
        } else {
            menuInfoService.deleteMenuOperateByFlow(deployment.getProcessKey(), String.valueOf(deployment.getProcessVersion()));
        }

        String queryPendingSql = "select INSTANCE_ID from wfm_task_pending where PROCESS_KEY=?1 and PROCESS_VERSION=?2";

        List<String> pendingList = pendingDao.createNativeQuery(queryPendingSql, params).list();

        if (null != pendingList && pendingList.size() > 0) {
            String instanceId = pendingList.get(0);
            String queryTableSql = "select j.TABLE_NAME_ TABLENAME, j.ENTITY_CODE_ ENTITYCODE from jbpm4_execution j where j.ID_=?";
            List<Map<String, Object>> list = pendingDao.createNativeQuery(queryTableSql, instanceId).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
            if (null != list && list.size() > 0) {
                ...
                String updateStatusSql = "update "
                        + tableName
                        + " set " + statusColName + " = 0 where " + statusColName + " <> 99 and " + tableInfoIdColName + " in (select p.TABLE_INFO_ID FROM wf_pending P WHERE P.PROCESS_KEY=? and p.PROCESS_VERSION=?)";
                pendingDao.createNativeQuery(updateStatusSql, params).addSynchronizedQuerySpace("").executeUpdate();
            }
            String delPendingSql = "delete wfm_task_pending where PROCESS_KEY=?1 and PROCESS_VERSION=?2";
            pendingDao.createNativeQuery(delPendingSql, params).executeUpdate();
        }

        String delTaskSql = "delete from wf_task where DEPLOYMENT_ID = ?";
        String delTransitionSql = "delete from wf_transition where DEPLOYMENT_ID = ?";
        String delDeploymentSql = "delete from wf_deployment where ID = ?";
        Object[] depIdParams = new Object[1];
        depIdParams[0] = deployment.getId();
        deploymentDao.createNativeQuery(delTaskSql, depIdParams).addSynchronizedEntityClass(Task.class).executeUpdate();
        deploymentDao.createNativeQuery(delTransitionSql, depIdParams).addSynchronizedEntityClass(Transition.class).executeUpdate();
        instanceService.deleteProcessInstance(deployment.getProcessKey(), deployment.getProcessVersion());

        deploymentDao.createNativeQuery(delDeploymentSql, depIdParams).addSynchronizedEntityClass(Deployment.class).executeUpdate();
        pendingDao.flush();
        pendingDao.clear();
        deploymentDao.flush();
        deploymentDao.clear();
    ...
    public void deleteFlow(String deploymentIds) {
        String[] ids = deploymentIds.split(",");
        for (String id : ids) {
            Deployment d = getDeployment(Long.valueOf(id));

            if(null == d) {
                throw new StaleObjectStateException(Deployment.class.getName(), id);
            }
            deleteFlow(d, true, true);
            deleteTaskInfo(id);
        }
     ...
     * @param deploymentId
     */
    public void deleteTaskInfo(String deploymentId){
        pendingDao.createNativeQuery("update wf_task set valid=0 where deployment_id=? ",deploymentId).addSynchronizedEntityClass(Task.class).executeUpdate();
        pendingDao.createNativeQuery("update wf_transition set valid=0 where deployment_id=? ",deploymentId).addSynchronizedEntityClass(Transition.class).executeUpdate();
    }
    ...
    public List<Staff> getSupervises(Long deploymentId) {
        ...
        List<Supervise> list=superviseDao.findByHql(hql, params);
        if(list==null){
            return null;
        }
                ...
                if(menuInfos.size() == 1){
                    ...
                    if(null != company){
                        ...
                        if(null != menuInfoMap.get(company.getId())){ //如果存在cid为当前公司的菜单则取当前公司
                            ...
                            if(!company.getType().equals(CompanyType.ORGANIZATION)){ //如果不存在且当前不为组织则取集团菜单
...
//            throw new BAPException(BAPException.Code.DOCUMENT_ERROR);
    ...
    public void saveSupervise(Long deploymentId,String superviseNamesMultiIDs){
        ...
        for(String idStr:ids){
            ...
            Long maxID = IDGenerator.newInstance().generate().longValue();
    ...
    @Override
    @Transactional
    public void currentVersion(String key, int version) {
        deploymentDao.bulkExecute("update Deployment set isCurrentVersion = false where processKey = ?0", key);
        deploymentDao.bulkExecute("update Deployment set isCurrentVersion = true where processKey = ?0 and processVersion = ?1", key, version);
    }

    @Override
    @Transactional
    public Page<Map<String, Object>> findRecordPageByParams(Page<Map<String, Object>> page, String queryResultSQL, String pageSQL,Map<String, Object> paramMap) {
        SQLQuery query = pendingDao.createNativeQuery(queryResultSQL);
        SQLQuery countQuery = pendingDao.createNativeQuery(pageSQL);
        ...
        List<Map<String, Object>> list = getResult(page,query,Transformers.ALIAS_TO_ENTITY_MAP);
        page.setResult(list);
        List totalList = countQuery.list();
        long totalCount = null == totalList ? 0 : Integer.valueOf(totalList.get(0).toString());
    ...
    @Override
    public List<Map<String, Object>> getModuleAndEntitys() {
        ...
        String preModuleCode = "";
        String currModuleCode = "";
        String preEntityCode = "";
        String currEntityCode = "";
        String hql = "select e.module.code,e.module.name,e.code,e.name "
                + " from Entity e where e.valid=true and e.module.valid=true order by e.module.code,e.code ";
        List<Object[]> results = pendingDao.createQuery(hql).list();
        for (Object[] objs : results) {
            if (objs[0] != null) {
                currModuleCode = (String) objs[0];
            }
            if (!currModuleCode.equals(preModuleCode)) {
                moduleMap = new HashMap<String, Object>();
                moduleMap.put("id", currModuleCode);
                moduleMap.put("code", currModuleCode);
                if (objs[1] != null) {
                    moduleMap.put("name", InternationalResource.get((String) objs[1]));
                }
                entityList = new ArrayList<Map<String, Object>>();
                moduleMap.put("children", entityList);
                moduleList.add(moduleMap);
                preModuleCode = currModuleCode;
            }

            if (objs[2] != null) {
                currEntityCode = (String) objs[2];
            }
            if (!currEntityCode.equals(preEntityCode)) {
                entityMap = new HashMap<String, Object>();
                entityMap.put("id", currEntityCode);
                entityMap.put("code", currEntityCode);
                if (objs[1] != null) {
                    entityMap.put("name", InternationalResource.get((String) objs[3]));
                }
                entityList.add(entityMap);
                preEntityCode = currEntityCode;
            }
