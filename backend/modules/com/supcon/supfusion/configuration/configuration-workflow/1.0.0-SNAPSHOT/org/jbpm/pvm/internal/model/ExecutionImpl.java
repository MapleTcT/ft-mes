...
import org.jbpm.pvm.internal.history.events.*;
...
///////////////////////////////我们自定义////////////////////////////////////
	protected Long processInitiator;//流程启动者,对应user id.
	protected Long tableInfoId;//TABLEINFO
	protected Long deploymentId;
//	protected Long entityId;
	protected String entityCode;
	protected Long ownerId;
	protected Long ownerPositionId;
	protected Long initiatorPositionId;
//	protected String initiatorGroupIds;//采用逗号隔开
	protected Long modelId;//模型对应实例的ID,不是Model的ID
	protected String tableNo;//单据编号
	protected String tableName;//业务表名
	protected Boolean groupEnabled;
	protected VariablesProvider variablesProvider;//不持久化
	protected WorkFlowVar workFlowVar;
	protected Boolean crossCompanyFlag;
	protected Set<Long> assignUsers;
	protected String scriptExcuteBeanName;//该实例调用的是哪个模块的servicename
	...
	public Set<Long> getAssignUsers() {
		return assignUsers;
	}
	public void setAssignUsers(Set<Long> assignUsers) {
...
//protected String[] additionalUsers;
...
public enum Propagation {
  ...
  protected void save() {
    this.dbid = DbidGenerator.getNextId("JBPM4_EXECUTION",1, null);
  ...
  public void end(String state) {
   ...
   // making a copy of the executions to prevent ConcurrentMoidificationException
    ...
    DbSessionImpl dbSession = EnvironmentImpl.getFromCurrent(DbSessionImpl.class, false);
...
//        }
    	  if(!"notification".equals(state))
    		  dbSession.getSession().createSQLQuery("DELETE FROM " + Pending.TABLE_NAME + " WHERE EXECUTION_ID = ?").addSynchronizedEntityClass(Pending.class).setString(0, id).executeUpdate();
...
//      HistoryEvent.fire(new ProcessInstanceEnd(), this);//不记录
  ...
  public void signal(WorkFlowVar workFlowVar){
	  if(null == workFlowVar)throw new IllegalArgumentException("the workFlowVar can not be null.");
	  this.variablesProvider = workFlowVar.getVariablesProvider();
	  fullVariablesProvider(this.variablesProvider);
	  this.workFlowVar = workFlowVar;
	  signal(workFlowVar.getOutcome(), workFlowVar.getVariables());
  }
  private void fullVariablesProvider(VariablesProvider variablesProvider){
	  ExecutionImpl e = this;
	  while(null != e.getParent()){
		  e = e.getParent();
		  e.setVariablesProvider(variablesProvider);
	  }
  }
  ...
  public void mergeExcutionImpl(){
	  DbSession dbSession = EnvironmentImpl.getFromCurrent(DbSession.class, false);

	    if (dbSession != null) {
	      dbSession.merge(this);
	    }
  }
  ...
  public SwimlaneImpl createSwimlane(String swimlaneName) {
    ...
    long dbid = DbidGenerator.getNextId("JBPM4_EXECUTION",1, null);
  ...
  protected Set<String> addActiveActivityNames(Set<String> activityNames) {
    if (
//    		(state.equals(STATE_ACTIVE_ROOT) || state.equals(STATE_ACTIVE_CONCURRENT))
//      && 
      activityName != null) {
  ...
  public void historyAutomatic(String transitionName) {
	    HistoryEvent.fire(new AutomaticEnd(), this);
	  }
  public void historyDecision(String transitionName) {
    HistoryEvent.fire(new DecisionEnd(transitionName), this);
  }
  public void historyActivityStart(String transitionName) {
	    HistoryEvent.fire(new ActivityStart(transitionName), this);
	  }
  ...
  public void deleteHistory(){
	  HistoryEvent.fire(new ActivityDelete(), this);
  }
