...
<body class="ec-config-page" style="background-color:#f2f2f2">
							...
							<#else>
								...
								<div class="operateDiv">
									...
									<div style="padding-left:20px;display:inline-block">
										...
										<a class="cui-btn mr10 cui-btn-columnattr" href="#" onclick="listec.setColumnPropertys()">${getHtmlText('ec.view.colattribute')}</a>
									    ...
									    <a class="cui-btn mr10 cui-btn-edit" href="#" onclick="listec.setwidth()">${getHtmlText('ec.view.setwidth')}</a>
									</div>
								</div>
										...
										<td>
											...
											<#if (ev.configMap)??  && (ev.configMap.layout)??>
												...
												<#else>
													...
													defaultTextareaRows="<#if listProperty.defaultTextareaRows?? >${(listProperty.defaultTextareaRows!)?string}<#else>0</#if>"
                                                    defaultRows="<#if listProperty.defaultRows?? >${(listProperty.defaultRows!)?string}<#else>0</#if>"
                                                    maxTextareaRows="<#if listProperty.maxTextareaRows?? >${(listProperty.maxTextareaRows!)?string}<#else>0</#if>"
                                                    maxTableHeight="${(listProperty.maxTableHeight!)?string}"
                                                    widthType="${(listProperty.widthType!)?string}"
														...
														<#list section.cells as column>
															...
															<#if !(column.none)??>
																...
																<td class="list_design_ul_li"
																	...
																	<#if (prop.type)!?? && '${(prop.type)!}' == 'LAYER'>layerType='${(column.layerType)!}'</#if>
																	...
																	referenceview='${(column.referenceview)!}'
																 	...
																 	isrefselect='${(column.isrefselect!false)?string('true','false')}'
	...
	<div id="design-button" style="height:30px;width:100%;float:right;text-align:right;margin-top:20px">
		<#if !isReadOnlyMode>
		<button class="Dialog_button btn_pointer btn-primary" onclick="listec.property_save()" type="button" style="margin-right:20px">${getHtmlText('common.button.save')}</button>
		</#if>
		...
		<table id="tab-common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			...
			<tr>
				...
				<td class="co" width="30%" id="decimalTextTd"><input id="property_decimal" type="text" class="cui-edit-field" /></td>
				 -->
				<td class="la" width="20%" align="right"  padding-right="10px">${getHtmlText('ec.view.width')}</td>
			...
			<tr id="form_tr_isrefselect">
			    <td id="selectcomp_checkbox_label" class="la" width="20%" align="right"  padding-right="10px">${getHtmlText('foundation.view.common.select')}</td>
            	<td id="selectcomp_checkbox_select" class="co" width="30%"><input type="checkbox" value="true"></td>
			</tr>
	...
	<div id="listPropertyDlg" style="display:none;">
		...
		<table id="tab_common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="80%" align="left" style="margin-top: 10px">
			<tr>
				...
				<td class="la" width="10%" style="padding-right:10px;"><input type="checkbox" id="exportExcel" /></td>
				<td class="co" width="40%">${getHtmlText('ec.action.datagrid.properties.excel.export')}</td>
			</tr>
			<tr>
			    <td class="la" width="10%" style="padding-right:10px"><input type="checkbox" id="isEasyTable" onchange="isEasyTableChange(this)"/></td>
                <td class="co" width="40%" >${getHtmlText('自动扩展单元格')}</td>
				<td class="la" width="10%" style="padding-right:10px;">${getHtmlText('ec.action.datagrid.properties.height')}</td>
				<td class="co" width="40%">
				...
				<td class="la" width="10%" style="padding-right:10px;display:none">${getHtmlText('ec.view.defaultRows')}</td>
                <td class="co" width="40%"style="display:none">
                    <input type="text" id="defaultRows" class="cui-edit-field" style="width:50px" onchange="defaultRowsChange(this)">${getHtmlText('ec.action.datagrid.properties.height.unit')}
                </td>
				<td class="la" width="10%" style="padding-right:10px;"><input type="checkbox" id="cannotAddNewRow" /></td>
				<td class="co" width="40%">${getHtmlText('ec.view.noaddRow')}</td>
				<td class="la" width="10%" style="padding-right:10px;display:none"><input type="checkbox" id="isSuperTable" /></td>
				<td class="co" width="40%" style="display:none">${getHtmlText('ec.view.superTable')}</td>
			</tr>
			<tr id="isEasyTableTr" style="display:none">
                <td class="la" width="10%" style="padding-right:10px;">${getHtmlText('ec.view.defaultTextareaRows')}</td>
                <td class="co" width="40%">
                    <input type="text" id="defaultTextareaRows" class="cui-edit-field" style="width:50px" >${getHtmlText('ec.action.datagrid.properties.height.unit')}
                </td>
                <td class="la" width="10%" style="padding-right:10px;">${getHtmlText('ec.view.maxTextareaRows')}</td>
                <td class="co" width="40%">
                    <input type="text" id="maxTextareaRows" class="cui-edit-field" style="width:50px">${getHtmlText('ec.action.datagrid.properties.height.unit')}
                </td>
            </tr>
			<#if model.dataType == 2>
...
<p class="baphelp-hint">注意：该功能需要参照列表的表格属性配置自定义条件，具体见其帮助说明。</p>
	...
	<div id="button_property_dlg" style="display:none">
		...
		<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			...
			<tr style="">
				...
				<td class="co" width="30%">
					<select id="operatetype"  class="cui-edit-field" style="width:100%">
						...
						<#if dataGrid.targetModel.entity.module.type?? && dataGrid.targetModel.entity.module.type=="Mis">
						<option value="MOVEUP">${getText('ec.view.isMoveUp')}</option>
						<option value="MOVEDOWN">${getText('ec.view.isMoveDown')}</option>
						</#if>
			...
			<tr>
				...
				<td class="co" width="30%">
					<select id="buttonstyle"  class="cui-edit-field" style="width:100%">
						...
						<option value="own-sy">${getText('ec.view.moveupstyle')}</option>
						<option value="own-xy">${getText('ec.view.movedownstyle')}</option>
						<option value="eighteen-dt-op-reference">${getText('ec.view.isrefrowstyle')}</option>
						<option value="upgrade">${getText('ec.view.upgradestyle')}</option>
						<option value="degrade">${getText('ec.view.degradestyle')}</option>
					</select>
			...
			<tr id="eventTr">
				<td class="la supplant-required-field" width="30%" align="center" padding-right="10px">${getHtmlText('ec.view.customevent')}
...
<p class="baphelp-hint">注意：event为事件对象</p>
					...
					<table style="border: 1px solid #DDD;padding: 0 10px;margin: 10px 0 0 10px;width: 100%;">
						...
						<tr class="customExampleRow">
							...
							<td class="co" colspan="3">
								<div class="fix-input">
									<#-- international_errorMsg    international_errorMsg_showName -->
									<@international name="errorMsgCustom" isNew=true isOldEdit=true moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-noborder-input" />
								</div>
							</td>
						</tr>
					</table>
        ...
        <div id="set_with"  style="display:none;">
            <@errorbar id="Set_withErrorBar"></@errorbar>
            <table id="easytable_type_tb" style="width: 256px;margin-top: 10px;">
            <tr>
            <td class="la" width="100px" align="right">${getHtmlText('ec.view.easytable.widthtype')}</td>
            <td class="co" >
                <select id="widthType" class="cui-edit-field" style="width:150px">
                <option value="1">绝对值模式</option>
                <option value="0">百分比模式</option>
                </select>
            </td>
            </tr>
			</table>
			<div style="height:225px;overflow:auto;width: 100%;">
                <table id="table_set_with" style="width: 256px;">
                </table>
                </div>
                <table id="table_set_totalwith" style="width: 256px;">
                </table>
            </div>
	</div>
	<script type="text/javascript">
		...
		function getRefView(){
			...
			if(modelCode!=""){
				CUI.ajax({
					url: "/msService/ec/view/referenceViews"+(window._proj_config_flag?'?isProj=true':''),
					type: 'post',
					async: false,
...
<script type="text/javascript">
	...
	$(function(){
		...
		<#if dataGrid.view?? && dataGrid.view.type=="EDIT">
		var buttons="del,add";
    ...
    //修改初始行数同步displayRowsCount
    function defaultRowsChange(obj){
        $("#displayRowsCount").val($(obj).val());
    }
