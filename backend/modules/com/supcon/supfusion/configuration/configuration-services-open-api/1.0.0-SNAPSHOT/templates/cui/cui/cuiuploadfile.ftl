<#assign ns=parameters.namespace?default('upload.defalut')> 
<#if parameters.integrateFlag?default(false)>
	...
	${ns}.resumeLabel = "${getText('foundation.upload.resume')}";
	...
	YAHOO.util.Event.onDOMReady(function() {
		...
		testnode.key = 'name';
		testnode.label = "${getText('foundation.fileupload.filename')}";
		testnode.width = '150';
		testnode.edit = false;
		...
		testnode.label = "${getText('foundation.upload.fileSize')}";
		testnode.width = '50';
		testnode.edit = false;
		...
		testnode.label = "${getText('foundation.upload.time')}";
		testnode.width = '80';
		testnode.edit = false;
		noNestColumnDefs[noNestColumnDefs.length] = testnode;
		...
		testnode.label = "${getText('foundation.upload.staff')}";
		testnode.width = '60';
		testnode.edit = false;
		noNestColumnDefs[noNestColumnDefs.length] = testnode;
		...
		testnode.key = 'memo';
		testnode.title = 'memo';
		testnode.label = "${getText('upload.memo')}";
		testnode.width = '120';
		testnode.edit = false;
		noNestColumnDefs[noNestColumnDefs.length] = testnode;
		...
		testnode.label = "${getText('foundation.common.delete')}";
		testnode.colclick=function(obj){${ns}.deleteSingle(null,obj);};
		testnode.width = '25';
		testnode.edit = true;
		noNestColumnDefs[noNestColumnDefs.length] = testnode;
		try{
		   var url = encodeURI('/foundation/workbench/upload-list?linkId=${parameters.linkId?default('')}&type=${parameters.type}');
		   if(YAHOO.env.ua.ie){
		        ${ns}.DT_ExistsFiles = new CUI.DataTable("${ns}.DT_ExistsFiles",noNestColumnDefs,null,{
		          	  ShowSetPath : '/foundation/workbench/upload-list?linkId=${parameters.linkId?default('')}&type=${parameters.type}',
		              paginator : false,
	...
	<input type="hidden" name="linkId" value="${parameters.linkId?default('')}"/>
	<#if staff?exists>
		<input type="hidden" name="files.staffId" value="${staff.id?c}" />
	</#if>
	<input type="hidden" name="files.type" value="${parameters.type}"/>
		...
		<div id="${ns}.selectedTable" style="width:60%;height:1px;overflow:hidden;">
			<table id="${ns}.selectedFiles" cellpadding="0" cellspacing="0" border="0" height="100%" style="margin-left: 12px;">
				<tr style="display: none;" height="26px">
					...
					<td width="64%" nowrap>${getText('upload.memo')}<input size="30" maxlength="300" type="text" name="files.memos" style="width:93%;" class="cui-edit-field"></input></td>
				</tr>
			</table>
		</div>
...
<#else>
	...
	${ns}._showUploadDialog = function(){
		try{
			try{
				var _callbefore_ = '${parameters.onClickBefore?default('')}';
				if(_callbefore_!=null&&_callbefore_!='') {
					var retValue = eval('${parameters.onClickBefore?default('')}()');
					if(retValue==false) {
						return;
					}
				}
			...
			var params = {};
			params.acceptType = "${parameters.acceptType?default('all')?lower_case}";
			if(params.acceptType=='all') {
				params.acceptType = '';
			}
			// params.onClickBefore = "${parameters.onClickBefore?default('all')}";
			var url = "/foundation/workbench/upload-init?linkId=${parameters.linkId?default('')}&type=${parameters.type}&callback=${parameters.callback}&multi=${parameters.multi?string('true','false')}&maxSize=${parameters.maxSize?default('10485760')}";
			uploadDialog = new CUI.Dialog({
				title: "${getText('foundation.cuiuploadfile.description.FILE_UPLOAD')}",
				url:url,
				argument:params,
				<#if parameters.acceptType?default('all')!='all'>
				description:"${getText('foundation.staff.infoset.allowUpload')}：" + params.acceptType,
				</#if>
				modal:true,
				height:'${parameters.height?default('500')}',
				width: '${parameters.width?default('650')}',
				dragable:true,
				buttons:[
					<#if parameters.multi?string('true','false')=='true'>
						{name:"${action.getText('upload.download')}",handler:function(){${ns}.downloadFiles();}},{name:"${action.getText('common.button.save')}",handler:function(){submitUploadForm();}},{name:"${getText('js.cui.js.close')}",id:"close",handler:function(){this.close();}}
					<#else>
						{name:"${action.getText('common.button.upload')}",handler:function(){submitUploadForm();}},{name:"${action.getText('common.button.cancel')}",id:"close",handler:function(){this.close();}}
					</#if>
					]
			});
			uploadDialog.show();
	...
	<#if parameters.look?default('button') == 'button'>
		<input name="_uploadButton" id="_uploadButton" type="button" value="${parameters.text}" onclick="javascript:${ns}._showUploadDialog();" />
	<#else>
		<span name="_uploadLabel" id="_uploadLabel" style="cursor:pointer;border:none;" onclick="javascript:${ns}._showUploadDialog();">${parameters.text}</span>
	</#if>
</#if>