	...
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
		...
		<style type="text/css">
			...
			/* #main_form_tab .etv-content{min-height:${(viewHeight!500)-30}px;} */
	...
	<body class="ec-config-page" style="background-color:#f2f2f2" layoutCode="${(ev.configMap.layout.layoutCode)!ecCodeInit('layout')}" page_beforesave="${((ev.configMap.layout.pageConfig.beforeSave)!)?html}" page_onload="${((ev.configMap.layout.pageConfig.onload)!)?html}" page_onsave="${((ev.configMap.layout.pageConfig.onsave)!)?html}" page_aftersave="${((ev.configMap.layout.pageConfig.afterSave)!)?html}" page_beforesubmit="${((ev.configMap.layout.pageConfig.beforeSubmit)!)?html}" page_aftersubmit="${((ev.configMap.layout.pageConfig.afterSubmit)!)?html}" page_helpInfo="${((ev.configMap.layout.pageConfig.helpInfo)!)?html}">
		<@errorbar id="workbenchErrorBar" offsetY=83 />
			<div class="layout grid-s4m0"> 	
			<div class="col-top">
				<div><h3 style="color:#3A70AA;padding-top:7px;padding-left:15px;float:left">
			...
			<div class="col-sub">
				...
				<div id="form_select_elements">
					...
					<#list fieldTypes?keys as key>
						<#if key == 'TEXTFIELD' || key == 'TEXTAREA' || key == 'DATE' || key == 'DATETIME' || key == 'LABEL' || key == 'DATAGRID' || key =='EASYTABLE' || key == 'MULTSELECT' || key == 'RADIO' || key == 'CHECKBOX' || key == 'SELECT' || key == 'MULTFILES' || key == 'RICHTEXT' || key == 'OUTDATA'>
						...
						<#else>
						<#if key == 'DATAGRID' || key == 'MULTSELECT'|| key =='EASYTABLE'>
						<li source="el" fieldType="${key}">${fieldTypes[key]!}</li>
						</#if>
						</#if>
						-->
					...
					<#if (view.entity.workflowEnabled)?? && view.entity.workflowEnabled>
								...
								<#-- <div class="main-wrap" id="main_design_container"> 
									...
									<div id="form_area">
										<div id="main_form_tab" class="etv-navset" style="overflow: hidden;">
											...
											<div id="buttonOperateDiv" style="margin:10px 0 15px;padding-bottom:5px;background:#F3F3F3!important;border:1px solid #0099CC;">
												<div class="operateDiv">
													...
													<a class="cui-btn mr10 cui-btn-leftmove" href="#" onclick="ec.leftmove('buttonOperateDiv')" title="${getText('ec.view.toleft')}">&nbsp;</a>
													<a class="cui-btn mr10 cui-btn-rightmove" href="#" onclick="ec.rightmove('buttonOperateDiv')" title="${getText('ec.view.toright')}">&nbsp;</a>
													</div>
												</div>
												...
												<#else>
													...
													<#list (ev.configMap.layout)['tabs'] as tab1>
														<#if !tab1.namekey?? && (tab1.name)?? && commoninfo?? && commoninfo == tab1.name>
															<li class="selected" namekey="ec.view.commoninfo" <#if tab1.ptRealTimeLoad??>ptRealTimeLoad="${tab1.ptRealTimeLoad}"</#if> ${getHtmlText('ec.view.commoninfo')}</li>
														<#else>
															<li namekey="${(tab1.namekey!)}" <#if tab1.ptRealTimeLoad??>ptRealTimeLoad="${tab1.ptRealTimeLoad}"</#if>>${getHtmlText('${(tab1.namekey)!((tab1.name)!)}')}</li>
														</#if>
													</#list>
													</#if>
												</#if>
													...
													<div class="div-selected">
														<#list tab['sections'] as secatt>
															...
															<ul class="form_design_ul <#if ((secatt.customSection)!false)>custom_section</#if>" sectionCode="${sectionCode!}" onclick="ec.selectsection(this)" colwidth="${(secatt.pageConfig.colwidth)!}" colNum="${sectionColNum}" name="${(secatt.name)!}" isborder="${(secatt.isborder)!1}" cssstyle="${(secatt.cssstyle)!''}">
																...
																<#if (element.element.showType!'') == 'DATAGRID' || (element.element.showType!'') == 'EASYTABLE'>targetPropertyCode="${(element.element.targetPropertyCode)!}" DataGridCode="${(element.element.DataGridCode)!}" hideKey="${(element.element.hideKey)!}"<#assign targetPropertyCode = element.element.targetPropertyCode></#if>
																...
																acceptRevisions="${((element.element.acceptRevisions)!false)?string('true','false')}" officeNotNull="${((element.element.officeNotNull)!true)?string('true','false')}" officePrint="${((element.element.officePrint)!false)?string('true','false')}" officeOpenType="${(element.element.officeOpenType)!}" officeSaveType="${(element.element.officeSaveType)!}"
																		...
																		<dd>
																			...
																			<#elseif (element.element.showType) == 'DATE' || (element.element.showType) == 'DATETIME'>
																			...
																			<#elseif (element.element.showType) == 'EASYTABLE'>
																				<table cellspacing="0" cellpadding="0" style="margin-left:10px;width:98%"><thead><tr><td class="datagird_th_td0">${getHtmlText('ec.view.num')}</td><td class="datagird_th_td1"><#if targetPropertyCode??>
																					<#if assos?has_content>
																						<#list assos as asso>
																							<#if asso.originalProperty.code == targetPropertyCode>
																							EASYTABLE:${getText('${asso.originalProperty.model.name}')}[${getText('${asso.originalProperty.displayName}')}]
																							</#if>
																						</#list>
																					</#if>
																				<#else>${getHtmlText('ec.view.modelfield')}</#if></td></tr></thead><tbody><tr><td class="datagird_tb_td0">1</td><td class="datagird_tb_td1"></td></tr></tbody></table>
																			...
																			<#elseif (element.element.showType) == 'SELECTTAGNUMBER'>
																				<input type="text" style="color:gray;width:100%;height: 20px;line-height: 20px;margin-right: -5px;" value="${ckname!}"/><input type='button' class='cui-search-click' />
																			</#if>
																			</#if>
																		</dd>
		...
		<div id="form_set_item_property_dlg" style="display:none">
			...
			<table id="tab-common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px;">
				...
				<tr id="tr_picSize" style="display:none;">
...
<p class="baphelp-hint">注意：只有表单才有operateType属性</p>
				...
				<!--<tr>
					...
					<td class="co" colspan="3"><textarea id="helpInfo" class="cui-edit-textarea" style="width:100%;height:100px;margin-top:5px;"></textarea></td>
				</tr>-->
			</table>
			</div>
...
<p class="baphelp-hint">注意：直接写css样式。</p>
	 	...
	 	<div id="tab_setting_dlg" style="display:none;">
	 		...
	 		<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
				...
				<tr class="tr-ptRealTimeLoad">
					<td class="la" width="15%" align="right" padding-right="10px"><label for="panel_property_is_ptRealTimeLoad" >${getHtmlText('ec.view.ptRealTimeLoad')}</label></td>
					<td class="co"><input id="panel_property_is_ptRealTimeLoad" type="checkbox" /></td>
				</tr>
			</table>
	 	</div>
	 	...
	 	<table id="table_validate_example_row" style="display:none;">
			...
			<div id="button_property_dlg" style="display:none">
				...
				<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
					...
					<tr id="eventTr"></#--xkl-->
						<td class="la" width="30%" align="left" style="color: rgb(179, 3, 3);" padding-right="10px">${getHtmlText('ec.edit.button.onclick')}
						<span id="functionHelpinfo" class="baphelp-icon"></span>
<div id="functionHelpinforef" style="display:none">
	<p class="baphelp-info">自定义按钮的单击事件</p>
	<p class="baphelp-example">范例</p>
<div class="baphelp-code">
...
<p class="baphelp-hint">注意：event为事件对象，可不传</p>
	...
	<script type="text/javascript">	
		...
		function _init_size(){
			...
			<#else>
			$('#form_area .etv-navset').height($(window).height()-100);
			$('#form_area .etv-navset').width($(window).width()-251);
			$('#form_area .etv-content').height($(window).height()-255);
	...
	ec_view_select_property.showOverLayer = function(obj,url){
		...
		try {
			ec_view_select_property.showOverLayerDiv && ec_view_select_property.showOverLayerDiv.close()
		} catch (e) {}
