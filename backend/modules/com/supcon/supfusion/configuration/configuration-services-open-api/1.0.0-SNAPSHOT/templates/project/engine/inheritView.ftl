...
CUI(function(){
	...
	function ajaxFileUpload(sUrl,formId) {
		ajaxFileUploadCommon({
				...
				success : function(msg) {
					var formId = 'ec_view_inherit_form';
					...
					if(msg && msg.success == false){
						...
						CUI.each(msg.items,function(index,item){
							if(index.indexOf('.id') != -1 && index.indexOf('.id')+3 == index.length) {
								$("#ec_view_inherit_form *[name^='"+(index.substring(0,index.length -3))+"'][type!='hidden']").each(function(){
									if($(this).parents('td[nullable=false]').length > 0) {
										showErrorField($(this));
									}
								});
							} else if($("#ec_view_inherit_form input[name='"+index+"AddIds'][type='hidden']").length > 0) {
								// 多选控件验证
								showErrorField($("#ec_view_inherit_form *[name='"+index+"AddIds']").parent('div'));
							} else {
								var field = CUI("#ec_view_inherit_form *[name='"+index+"']");
							...
							CUI("#ec_view_inherit_form *[name='"+index+"']").first().focus();
						...
						oErrorWidget = ec_view_inherit_formDialogErrorBarWidget;
						oErrorWidget.showMessage(errorMsgs);
						// oErrorWidget.show(errorMsgs);
						if(CUI.Dialog){
					    	 CUI.Dialog.toggleAllButton('ec_view_inherit_form', true);
					     }
					}else{
						window.onbeforeunload = null;
						...
						ec.view.inheritcallBack(msg,CUI('#'+formId).serialize());
					}
				}
			}
		);
	...
	function submitBapForm(){//电子签名成功之后出现进度条并提交表单
		...
		if(ecFormFlag && ( $('#ec_view_inherit_form').parents('.ewc-dialog-blove').length > 0 || ( window != top && !window.frameElement.getAttribute( 'v3_tab_iframe' ) ) ) ) {
			// dialog不出进度条
			ecFormFlag = false;
		}
		...
		//前台验证通过之后出进度条
		CUI.Dialog.toggleAllButton('ec_view_inherit_form',true,ecFormFlag, true);
	...
	setTimeout(function(){
			...
			CUI.ajax({
				...
				success : function(msg){
					...
					ec.view.inheritcallBack(msg,postData);
				}
			});
		}
	}, 600);
	...
	CUI('#ec_view_inherit_form').unbind('submit.bapForm').bind('submit.bapForm',function(){
		//每次提交时先隐藏报错信息
		try{
		}catch(e){
		...
		//if(CUI.Dialog) CUI.Dialog.toggleAllButton('ec_view_inherit_form',true,ecFormFlag);
		//});
			//if(!validateForm_ec_view_inherit_form())	return false;
		$('#ec_view_inherit_form').trigger('beforeSubmit');
			var successFlag = ec.view.inheritonsubmit();
			if(successFlag!=null && !successFlag) {/*if(CUI.Dialog) CUI.Dialog.toggleAllButton('ec_view_inherit_form', true);*/return false;}
		if($('#ec_view_inherit_form input[name="operateType"]').val() == "submit"){
			var deploymentId=$('#ec_view_inherit_form input[name="deploymentId"]');
			var buttonCode=$('#ec_view_inherit_form input[name="buttonCode"]');
			var namespace=$('#ec_view_inherit_form input[name="namespace"]');
			if( deploymentId.length > 0 && deploymentId.val() != undefined && deploymentId.val() != ''){
				...
				if(signatureInfo[0] != '') {
					...
					if(cancelItem.val() != "cancel") {
						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],'ec.flow.submit','','ec_view_inherit_form');
						$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','ec_view_inherit_form','ec.flow.submit',false)});
					}
					else {
						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],'ec.edit.remove','','ec_view_inherit_form');
						$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','ec_view_inherit_form','ec.edit.remove',false)});
					}
				...
				if(signatureInfo[0] != '') {
					if(namespace.length > 0 && namespace.val() != undefined && namespace.val() != '') {
						parent.signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],buttonCode.val(),namespace.val(),"ec_view_inherit_form",false,'');
						if(signatureInfo[0] == 'singleSign') {
							parent.$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk(namespace.val(),'ec_view_inherit_form',buttonCode.val(),false)});
						}
						else {
							setTimeout(function(){parent.$('#secondSignerOK').unbind('click').bind('click',function(){doubleSignOK(this,namespace.val(),'ec_view_inherit_form',buttonCode.val(),false)});},2000);
						}
					}
					else {
						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],buttonCode.val(),'',"ec_view_inherit_form",false,'');
						if(signatureInfo[0] == 'singleSign') {
							$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','ec_view_inherit_form',buttonCode.val(),false)});
						}
						else {
							setTimeout(function(){$('#secondSignerOK').unbind('click').bind('click',function(){doubleSignOK(this,'','ec_view_inherit_form',buttonCode.val(),false)});},2000);
						}
					}
				}
				else {
...
<form id="ec_view_inherit_form" action="/msService/ec/engine/inheritSave" namespace="/msService/ec/engine" validate="true" onsubmitMethod="ec.view.inheritonsubmit()" callback="ec.view.inheritcallBack">
	<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%">
		...
		<tr>
			<td class="la">${getHtmlText('ec.engine.chooseview')}</td>
			<td class="co">
				<select id="view_code" name="view.code"  style="width:100%" onchange="ec.view.changeInheritCode(this)">
				<#list viewPage.result as v>
					<option value="${v.code}" editViewType="${v.editViewType}" type="${v.type}" showtype="${v.showType}">${getText(v.displayName)}(${v.name})</option>
				</#list>
				</select>
			</td>
			<td class="la"></td>
		...
		<tr>
			<td class="la" style="width:25%">${getHtmlText('ec.engine.inherittype')}</td>
			<td class="co" style="width:65%">
				<input type="radio" id='inheritType1' name="inheritType" value="1"/><label id="labinheritType" for="inheritType1">${getHtmlText('ec.engine.halfinherit')}</label>
				<input type="radio" id='inheritType2' name="inheritType" value="2" checked="true"/><label for="inheritType2">${getHtmlText('ec.engine.totalinherit')}</label>
			</td>
			<td class="la" style="width:10%"></td>
		</tr>
	</table>
...
(function(){  //onload事件，自动调用
	var form = CUI('#ec_view_inherit_form');
})();

    function validateForm_ec_view_inherit_form() {
        form = document.getElementById("ec_view_inherit_form");
        // 内容前后去空格
    	$('select,input[type="text"],textarea', '#ec_view_inherit_form').each(function(){
...
<script type="text/javascript">
ec.view.changeInheritCode=function(obj){
	$("#inheritType1").show();
	var type=$("#view_code").find("option:selected").attr('type');
	var editViewType=$("#view_code").find("option:selected").attr('editViewType');
	var showtype=$("#view_code").find("option:selected").attr('showtype');
	if(showtype=="SINGLE"||showtype=="PART"){
		if(editViewType==1||(type!='EDIT'&&type!='VIEW'&&type!='LIST'&&type!='REFERENCE')){
			$("#inheritType1").hide();
			$("#labinheritType").hide();
			$("#inheritType1").attr("checked",false);
			$("#inheritType2").attr("checked",true);
		}else{
			$("#inheritType1").show();
			$("#labinheritType").show();
		}
...
ec.view.changeInheritCode();
ec.view.inheritonsubmit=function(){
	if($("#view_code").val()==""||$("#view_code").val()==null){
