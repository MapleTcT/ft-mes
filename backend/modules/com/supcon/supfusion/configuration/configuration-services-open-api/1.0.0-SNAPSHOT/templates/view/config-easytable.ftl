			...
			<div class="main-wrap" id="main_design_container">
				...
				<div id="form_area" style="padding-left:3px;">
					...
					<div id="main_form_tab" class="etv-navset">
						<ul class="etv-nav">
							<li class="selected">Easytable</li>
						</ul>
									...
									<tr>
										...
										<td>
											...
											<#if (ev.configMap)??  && (ev.configMap.layout)??>
												...
												<#if (configMap.sections)??>
												<#list (configMap.sections) as section>
												<#if section?? && (section.regionType=='DATAGRID'||section.regionType=='EASYTABLE')>
												...
												<#else>
													...
													defaultTextareaRows="<#if listProperty.defaultTextareaRows?? >${(listProperty.defaultTextareaRows!)?string}<#else>3</#if>"
													defaultRows="<#if listProperty.defaultRows?? >${(listProperty.defaultRows!)?string}<#else>5</#if>"
													maxTextareaRows="${(listProperty.maxTextareaRows!)?string}"
			...
			<tr id="gridDefaultTR" style="display:none">
					...
					<td class="co" width="80%" colspan="3">
						...
						<div id="grid_ck_boolean_div" style="display:none;width:100%">
							<input type="checkbox" id="grid_ck_boolean" value="true" onChange="_changeDefaultValue(this)"/>${getHtmlText('ec.property.default.yes')}
						</div>
		...
		<table id="tab_common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="80%" align="left" style="margin-top: 10px">
			<tr>
				...
				<td class="co" width="40%">
					<@international name="dataGrid.dataGridName" isNew=true isOldEdit=true moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="${dataGrid.dataGridName!''}" style="width:100%;margin-right: -5px;" cssClass="cui-edit-field" />
				</td>
			...
			<tr >
				...
				<td class="la" width="10%" style="padding-right:10px;">${getHtmlText('ec.view.defaultRows')}</td>
				<td class="co" width="40%">
					<input type="text" id="defaultRows" class="cui-edit-field" style="width:50px" >${getHtmlText('ec.action.datagrid.properties.height.unit')}
				</td>
			</tr>
			<tr >
				...
				<td class="la" width="10%" style="padding-right:10px;">${getHtmlText('ec.view.maxTableHeight')}</td>
				<td class="co" width="40%">
					<input type="text" id="maxTableHeight" class="cui-edit-field" style="width:50px">${getHtmlText('ec.action.datagrid.properties.height.px')}
				</td>
			</tr>
			<#if model.dataType == 2>
			...
			<tr <#if dataGrid.view?? && dataGrid.view.type=="VIEW">style="display:none;"</#if>>
...
<pre><code><span>if(test==1){</span>
	...
	return '(P.ID = \$\{deptId,String} OR P.LAY_REC LIKE \\'\$\{layrec,String}\\')';
		...
		<table id="tab_event" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="80%" align="left" style="margin-top: 10px;display:none;">
			...
			<tr>
				<td class="la" style="width:20%" align="right" padding-right="10px">renderOver:
					...
					<div id="renderOverHelpinforef" style="display:none">
						<p class="baphelp-info">Easytable数据请求执行接口</p>
						<p class="baphelp-example">范例</p>
					<div class="baphelp-code">
					<pre><code>alert ( " 示例 " );
					</code></pre>
			...
			<tr>
				<td class="la" style="width:20%" align="right" padding-right="10px">ptInit（Easytable${getText('ec.view.init')}）:
				<span id="ptInitHelpinfo" class="baphelp-icon"></span>
				<div id="ptInitHelpinforef" style="display:none">
					<p class="baphelp-info">Easytable的初始化函数</p>
					<p class="baphelp-example">范例</p>
				<div class="baphelp-code">
				<pre><code>alert ( " 示例 " );
				</code></pre>
