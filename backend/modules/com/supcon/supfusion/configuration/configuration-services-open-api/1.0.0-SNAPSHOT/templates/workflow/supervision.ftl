<div id="portalSupervision" class="rowItemDiv">
        ...
        <#if tableinfos?exists>
        	<#if tableinfos["on"]?exists>
        	 <ul id="remindList" class="port-list">
			    ...
			    <li title="${info['CREATE_TIME']?string("yyyy-MM-dd")} ${info['NAME']?default('')}(${info['SUMMARY']?default(info['TABLE_NO']?default(''))}) ${info['STATUS']?default('')}" onclick="foundation.supervision.opentableinfoURL('${info['ID']}','${info['TARGET_ENTITY_CODE']}','${info['TARGET_TABLE_NAME']}')">
			  ...
			  <a href="#" style="float:right;color:#0066CC;margin-right:15px;padding-top:3px;clear:both;" url="/msService/ec/myWorkflow/mySupervision.action" code="MySupervision" parentCode="myFlowManageWorkBench" onclick="foundation.workbench.myPending(this)">${getHtmlText('foundation.more')}</a>
			 <#else>
...
//注册命名空间
CUI.ns("foundation.supervision");
foundation.supervision.opentableinfoURL = function(ID,entityCode,targetTablename){
