<script type="text/javascript">
	$(function(){
		$('#adv_select_elements').accordion();
		$('#adv_select_elements .accordion_pane').height(419);
	});
</script>
<@errorbar id="ec_advQuery_edit_formDialogErrorBar" />
<div id="adv_query_content_config" style="margin-left:2px;margin-top:8px;width: 160px;height: 473px;float:left;background-color:white;border:1px solid #3C7FB1">
	<script type="text/javascript">
    ...
    CUI(function(){
    	...
    	function ajaxFileUpload(sUrl,formId) {
    		ajaxFileUploadCommon({
    				...
    				success : function(msg) {
    					var formId = 'ec_advQuery_edit_form';
    					...
    					if(msg && msg.success == false){
    						...
    						CUI.each(msg.items,function(index,item){
    							if(index.indexOf('.id') != -1 && index.indexOf('.id')+3 == index.length) {
    								$("#ec_advQuery_edit_form *[name^='"+(index.substring(0,index.length -3))+"'][type!='hidden']").each(function(){
    									if($(this).parents('td[nullable=false]').length > 0) {
    										showErrorField($(this));
    									}
    								});
    							} else if($("#ec_advQuery_edit_form input[name='"+index+"AddIds'][type='hidden']").length > 0) {
    								// 多选控件验证
    								showErrorField($("#ec_advQuery_edit_form *[name='"+index+"AddIds']").parent('div'));
    							} else {
    								var field = CUI("#ec_advQuery_edit_form *[name='"+index+"']");
    							...
    							CUI("#ec_advQuery_edit_form *[name='"+index+"']").first().focus();
    						...
    						oErrorWidget = ec_advQuery_edit_formDialogErrorBarWidget;
    						oErrorWidget.showMessage(errorMsgs);
    						// oErrorWidget.show(errorMsgs);
    						if(CUI.Dialog){
    					    	 CUI.Dialog.toggleAllButton('ec_advQuery_edit_form', true);
    					     }
    					}else{
    						window.onbeforeunload = null;
    						...
    						ec.advQuery.editCallback(msg,CUI('#'+formId).serialize());
    					}
    				}
    			}
    		);
    	...
    	function submitBapForm(){//电子签名成功之后出现进度条并提交表单
    		...
    		if(ecFormFlag && ( $('#ec_advQuery_edit_form').parents('.ewc-dialog-blove').length > 0 || ( window != top && !window.frameElement.getAttribute( 'v3_tab_iframe' ) ) ) ) {
    			// dialog不出进度条
    			ecFormFlag = false;
    		}
    		...
    		//前台验证通过之后出进度条
    		CUI.Dialog.toggleAllButton('ec_advQuery_edit_form',true,ecFormFlag, true);
    	...
    	setTimeout(function(){
    			...
    			CUI.ajax({
    				...
    				success : function(msg){
    					...
    					ec.advQuery.editCallback(msg,postData);
    				}
    			});
    		}
    	}, 600);
    	...
    	CUI('#ec_advQuery_edit_form').unbind('submit.bapForm').bind('submit.bapForm',function(){
    		//每次提交时先隐藏报错信息
    		try{
    		}catch(e){
    		...
    		//if(CUI.Dialog) CUI.Dialog.toggleAllButton('ec_advQuery_edit_form',true,ecFormFlag);
    		//});
    			//if(!validateForm_ec_advQuery_edit_form())	return false;
    		$('#ec_advQuery_edit_form').trigger('beforeSubmit');
    		if($('#ec_advQuery_edit_form input[name="operateType"]').val() == "submit"){
    			var deploymentId=$('#ec_advQuery_edit_form input[name="deploymentId"]');
    			var buttonCode=$('#ec_advQuery_edit_form input[name="buttonCode"]');
    			var namespace=$('#ec_advQuery_edit_form input[name="namespace"]');
    			if( deploymentId.length > 0 && deploymentId.val() != undefined && deploymentId.val() != ''){
    				...
    				if(signatureInfo[0] != '') {
    					...
    					if(cancelItem.val() != "cancel") {
    						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],'ec.flow.submit','','ec_advQuery_edit_form');
    						$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','ec_advQuery_edit_form','ec.flow.submit',false)});
    					}
    					else {
    						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],'ec.edit.remove','','ec_advQuery_edit_form');
    						$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','ec_advQuery_edit_form','ec.edit.remove',false)});
    					}
    				...
    				if(signatureInfo[0] != '') {
    					if(namespace.length > 0 && namespace.val() != undefined && namespace.val() != '') {
    						parent.signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],buttonCode.val(),namespace.val(),"ec_advQuery_edit_form",false,'');
    						if(signatureInfo[0] == 'singleSign') {
    							parent.$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk(namespace.val(),'ec_advQuery_edit_form',buttonCode.val(),false)});
    						}
    						else {
    							setTimeout(function(){parent.$('#secondSignerOK').unbind('click').bind('click',function(){doubleSignOK(this,namespace.val(),'ec_advQuery_edit_form',buttonCode.val(),false)});},2000);
    						}
    					}
    					else {
    						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],buttonCode.val(),'',"ec_advQuery_edit_form",false,'');
    						if(signatureInfo[0] == 'singleSign') {
    							$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','ec_advQuery_edit_form',buttonCode.val(),false)});
    						}
    						else {
    							setTimeout(function(){$('#secondSignerOK').unbind('click').bind('click',function(){doubleSignOK(this,'','ec_advQuery_edit_form',buttonCode.val(),false)});},2000);
    						}
    					}
    				}
    				else {
	...
	<form id="ec_advQuery_edit_form" name="ec_advQuery_edit_form" action="/msService/ec/view/save-advqueryconfig" method="post" callback="ec.advQuery.editCallback"  onreset="clearErrorMessages(this);clearErrorLabels(this);>
	<input type="hidden" name="fastDelCells" name="fastDelCells" value="${fastDelCells!}"/>
        <#if aqj??>
        <input type="hidden" name="aqj.code" value="${aqj.code!}" />
        <input type="hidden" name="aqj.version" value="${aqj.version!}" />
        <#else>
        <input type="hidden" name="aqj.code" value="${aqj.code!}" />
        <input type="hidden" name="aqj.version" value="0" />
        </#if>
        <#if aqj.targetModel??>
        <input type="hidden" name="aqj.targetModel.code" value="${targetModel.code!}" />
        </#if>
        <input type="hidden" name="aqj.layoutName" value="${layoutName!}" />
        <input type="hidden" name="aqj.view.code" value="${aqj.view.code!}"/>
        <input type="hidden" name="aqj.queryConfig" value='${aqj.queryConfigEscapeHtml!}' />
	...
	<div id="adv_select_elements">
		...
		<div class="accordion_pane" style="display:block;overflow:auto;height:419px;">
				...
				<#if subs?? && (subs.associatedInfos)??>
					...
					<#list associatedInfos as ass>
						...
						<li source='test' parentModelDataType='<#if (ass.targetProperty.model.dataType)?? && (ass.targetProperty.model.dataType) == 2>tree<#else>simple</#if>' partDepend='common' assTar='${ass.targetProperty.code}' assPropertyName='${ass.targetProperty.name}' propertyCode="${ass.originalProperty.code!}" assOrg="${ass.originalProperty.code}" dbname='${(ass.targetProperty.model.modelName)!}' name='${(ass.originalProperty.name)!}' entityCode='${(ass.targetProperty.model.code)!}' relativeName='${(ass.targetProperty.model.tableName)!},${(ass.targetProperty.columnName)},${(ass.originalProperty.model.tableName)!},${(ass.originalProperty.columnName)!}'>
							<img sourceType='adv' align='absmiddle' src='/bap/static/treeview/assets/ectree_colse.gif' flag='true' hasclick='false' onclick='ec.showQueryAssProperty(this,"${(ass.targetProperty.model.code)!}","adv")'></img>
		...
		<div class="accordion_pane" style="overflow:auto;height:410px;"><ul class="onetomany_properties_container">
				...
				<#assign j = 1>
					<#list oneToManyAssociatedInfos as onetoManyAss>
						...
						<li source='test' parentModelDataType='<#if (onetoManyAss.targetProperty.model.dataType)?? && (onetoManyAss.targetProperty.model.dataType) == 2>tree<#else>simple</#if>' partDepend='one2many' assTar='${onetoManyAss.targetProperty.code}' propertyCode="${onetoManyAss.targetProperty.code!}" assOrg="${onetoManyAss.originalProperty.code}" dbname='${(onetoManyAss.targetProperty.model.modelName)!}' name='${(one2ManyTargetPropertyName)!}' entityCode='${(onetoManyAss.targetProperty.model.code)!}' relativeName='${(one2ManyOriginalTableName)!},${(onetoManyAss.targetProperty.columnName)!},${(onetoManyAss.originalProperty.model.tableName)!},${(onetoManyAss.originalProperty.columnName)}'>
							<img sourceType='adv' align='absmiddle' src='/bap/static/treeview/assets/ectree_colse.gif' flag='true' hasclick='false' onclick='ec.showQueryAssProperty(this,"${(onetoManyAss.targetProperty.model.code)!}","adv")'></img>
...
<div name="advQueryContentAndConfig" style="width:520px;height: 473px;background-color:white;border:2px solid #D4D4D4;float:left;margin: 8px 5px 0 7px;"> 	
	<div style="position:absolute;height:89%;width:400px;">
		<div style="float:right;padding-top:10px;">
			<a style="border:1px solid #fff;padding-right:15px;" class="cui-btn mr10 cui-btn-setting" href="#" onclick="ec.advFieldProperty()">${getHtmlText('ec.view.fastFieldProperty')}</a>
		</div>
		<div style="margin-left:10px;margin-top:40px;overflow-y:auto;clear:both;height:82%">
		<table cellpadding="0" id="advColTable" style="font-size:12px;width:96%;" cellspacing="0" align="center" class="infoTable">
			<tbody id="advColOrder">
			<#if aqj?? && (aqj.queryConfigMap)?? && (aqj.queryConfigMap.advQueryJson)?? >
			<#assign configMap = (aqj.queryConfigMap.advQueryJson)>
			...
			<#list (configMap.sections) as section>
				...
				<#if (cell.element.propertyCode)?? &&propertyMap?? && propertyMap['${propCode}']?? >
					...
					assPropertyName="${cell.assPropertyName!}"
	...
	<div id="advContent" style="background-color:#f8f6f7;position:absolute;right:30px;width:105px;height:89%;">
		<div style="padding-top:80px!important;" class="ec-list-btndiv"><div class="ec-list-topbtn" onclick="ec.firstRow('adv')" id="advFirstMove" ></div></div>
		<div class="ec-list-btndiv"><div class="ec-list-prevbtn" onclick="ec.upRow('adv')" id="advUpMove" ></div></div>
		<div class="ec-list-btndiv"><div class="ec-list-nextbtn" onclick="ec.downRow('adv')" id="advDownMove"></div></div>
		<div class="ec-list-btndiv"><div class="ec-list-lastbtn" onclick="ec.lastRow('adv')" id="advLastMove"></div></div>
	</div>
</div>
<script type="text/javascript">
(function(){
	...
	/**
	 * 快速查询提交
	 * @method ec.advQuery.Callback
	 */
	ec.advQuery.editCallback = function(msg){
		if(msg && msg.code) {
			ec_advQuery_edit_formDialogErrorBarWidget.show("${getHtmlText('ec.dataclassific.submitsuccessful')}","s");			
			$("#ec_advQuery_edit_form input[name='aqj.version']").val(msg.version);
			setTimeout(function(){
				advQuerySetting_dialog && advQuerySetting_dialog.close();
			},1500);
		}else{
			ec_advQuery_edit_formDialogErrorBarWidget.show("${getHtmlText('ec.dataclassific.submitfailure')}");
        }
