...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
	<title>${getText(view.assModel.entity.name)}-${getText(view.displayName)}-${getText('ec.project.view_setting')}-<#if view.type == 'EXTRA' || view.type == 'EDIT'>${getText('ec.view.edit')}<#else>${getText('ec.view.view')}</#if></title>
	<@ec_extraTop/>
	<@loadpanel />
	<style>
	...
	<style type="text/css">
		...
		#form_select_elements{height:70%}
			...
			//	$('#form_select_elements').tabs("#form_select_elements div.accordion_pane", {tabs:'h2',effect:'slide',initialIndex:null});
				...
				ec = new CUI.EntityConfExtra(
					...
					<#if (ev.config)?? && ev.config != ''>
						<#--<@s.property value="ev.config" escape=false/>-->{pageConfig:{colNum:${(ev.configMap.layout.pageConfig.colNum)!defaultColNum},layoutType:"${(ev.configMap.layout.pageConfig.layoutType)!}",selfAdaption:${(ev.configMap.layout.pageConfig.selfAdaption)?default(false)?c}}}
					<#else>
...
<body class="ec-config-page" style="background-color:#f2f2f2" layoutCode="${(ev.configMap.layout.layoutCode)!ecCodeInit('layout')}" page_beforesave="${((ev.configMap.layout.pageConfig.beforeSave)!)?html}" page_onload="${((ev.configMap.layout.pageConfig.onload)!)?html}" page_onsave="${((ev.configMap.layout.pageConfig.onsave)!)?html}" page_aftersave="${((ev.configMap.layout.pageConfig.afterSave)!)?html}" page_beforesubmit="${((ev.configMap.layout.pageConfig.beforeSubmit)!)?html}" page_aftersubmit="${((ev.configMap.layout.pageConfig.afterSubmit)!)?html}">
		...
		<div class="col-sub">
			...
			<div id="form_select_elements">
				...
				<div class="accordion_pane" style="display:block;overflow:auto;">
					<ul class="main_properties_container">
						<#list view.assModel.properties as p>
							...
							<#else>
								<#if (p.name) != "extraCol" && (p.name) != "tableInfoId" && (p.name) != "version" && (!((p.isCustom)!false)||(p.projCustomInUse)!false)>
									<li source='<#if p.type == "OFFICE">office<#else>main</#if>' name='${view.assModel.modelName?uncap_first}.${p.name}' propDefaultValue='${(p.defaultValue)!}'  propertyCode='${(p.code)!}' namekey="${(p.displayName)!}" columnType='${(p.type)!}' propShowFormat='${(p.format)!}' propShowType='${(p.fieldType)!}'  propnullable='${(p.nullable!false)?string('true','false')}' multable='${(p.multable!false)?string('true','false')}' fillContent='${(p.fillcontent)!}' propPrecision='${(p.decimalNum)!}'  isCustom='${((p.isCustom)!false)?string}'
									<#if subs?? && (subs.associatedInfos)??>
										<#assign assFlag = false>
										<#assign associatedInfos = (subs.associatedInfos)>
				...
				<#list fieldTypes?keys as key>
					<#if key == 'TEXTFIELD' || key == 'TEXTAREA' || key == 'DATE' || key == 'DATETIME' || key == 'LABEL' || key == 'DATAGRID' || key == 'MULTSELECT' || key == 'RADIO' || key == 'CHECKBOX' || key == 'SELECT' || key == 'MULTFILES' || key == 'RICHTEXT' || key == 'OUTDATA' || key == 'LAYER' >
				...
				<#if (view.entity.workflowEnabled)?? && view.entity.workflowEnabled && view.type!="EXTRA">
				<h2>${getHtmlText('ec.view.workflow')}</h2>
				<div class="accordion_pane"><ul class="main_properties_container">
				<#if workFlowElements??>
				<#list workFlowElements?keys as key>
					<li source="wf" showType="${key}" namekey="${workFlowElements[key]!}" name="${view.assModel.modelName?uncap_first}.supervision">${getHtmlText('${workFlowElements[key]!}')}</li>
				</#list>
				</#if>
				</ul></div>
				</#if>
				<h2>${getHtmlText('ec.view.layout')}</h2>
				<div class="accordion_pane">
					<ul>
						<li source="layout">${getHtmlText('ec.view.layout')}</li>
						<li source="tabs">${getHtmlText('ec.view.tab')}</li>
					</ul>
				</div>
				<h2>${getHtmlText('ec.view.echarts')}</h2>
				<div class="accordion_pane">
					<ul>
						<li source="echarts" showtype="line-bar" viewCode=${view.code}>${getHtmlText('ec.view.echarts.line')}/${getHtmlText('ec.view.echarts.bar')}</li>
						<#--<li source="echarts" showtype="pie" viewCode=${view.code}>${getHtmlText('ec.view.echarts.pie')}</li>
						<li source="echarts" showtype="gauge" viewCode=${view.code}>${getHtmlText('ec.view.echarts.gauge')}</li>-->
					</ul>
				</div>
			</div>
			<div class="treeBox">
				<h3>${getHtmlText('ec.view.layoutStructure')}</h3>
				<ul id="layoutTree" class="ztree">
				</ul>
			</div>
		</div>
		<div class="col-main">
			...
			<#if view.openType == 'dialog'>
				<#if view.dialogType?? && view.dialogType != 'DIALOG_OTHER'>
					<#if view.dialogType?index_of('_1') gt 0>
						<#assign viewWidth = 375>
						<#assign viewHeight = 164 + 150 >
					<#elseif view.dialogType?index_of('_2') gt 0>
						<#assign viewWidth = 280 + 150 >
						<#assign viewHeight = 202 + 150 >
					<#elseif view.dialogType?index_of('_3') gt 0>
						<#assign viewWidth = 460 + 150 >
						<#assign viewHeight = 330 + 150 >
					<#elseif view.dialogType?index_of('_4') gt 0>
						<#assign viewWidth = 700>
						<#assign viewHeight = 500>
					<#elseif view.dialogType?index_of('_5') gt 0>
						<#assign viewWidth = 840>
						<#assign viewHeight = 600>
				...
				<#else>
					<#assign viewWidth = view.width!460>
					<#assign viewHeight = view.height!330>
				</#if>
			</#if>
			-->
			<#if view.openType == 'dialog'>	
										...
										<div style="background-color:#e3f1f9;overflow-y: auto;" class="etv-content" >
											...
											<#else>
												<#if (ev.configMap)?? && (ev.configMap.layout)??>
													...
													<script type="text/javascript">
													$(function(){
														$('.col-main').width($(window).width()-346);
														<#if view.openType == 'dialog'>	
														...
														<#else>
														$('#form_area .etv-content').height($(window).height()-249);
		...
		<div class="col-operate">
			<div>
				<ul id="main_form_tab_operate" >
					<li/>
					<li title="${getText('ec.view.pageProperty')}"><button class="r_ymsx" onclick="ec.cellPageProperty()" type="button">${getHtmlText('ec.view.pageProperty')}</button></li>
					<li><hr class="line-icon"></hr></li>
					<li title="${getText('ec.view.addSection')}"><button class="r_zjj" onclick="ec.addSection()" type="button">${getHtmlText('ec.view.addSection')}</button></li>
					<li title="${getText('ec.view.delSection')}" ><button class="r_scj" onclick="ec.delSection()" type="button">${getHtmlText('ec.view.delSection')}</button></li>
					<li title="${getText('ec.view.sectionattr')}"><button class="r_jsx" onclick="ec.sectionProperty()" type="button">${getHtmlText('ec.view.sectionattr')}</button></li>
					<li><hr class="line-icon"></hr></li>
					<li title="${getText('ec.view.addline')}"><button class="r_zh" onclick="ec.addCell(1)" type="button">${getHtmlText('ec.view.addline')}</button></li>
					<li  title="${getText('ec.view.deleteline')}"><button class="r_sh" onclick="ec.delRow()" type="button">${getHtmlText('ec.view.deleteline')}</button></li>
					<li><hr class="line-icon"></hr></li>
					<li title="${getText('ec.view.splitcell')}"><button class="r_cfdyg" onclick="ec.splitCell2()" type="button">${getHtmlText('ec.view.splitcellEdit')}</button></li>
					<li title="${getText('ec.view.delcell')}"><button class="r_scdyg" onclick="ec.delCell()" type="button">${getHtmlText('ec.view.delcell')}</button></li>
					<li title="${getText('ec.view.cellproperty')}"><button class="r_dygsx" onclick="ec.cellProperty()" type="button">${getHtmlText('ec.view.cellproperty')}</button></li>
				</ul>
			</div>
		</div>		
	</div>
	...
	<div id="design-button" style="height:30px;width:100%;overflow:hidden;clear:both;text-align:right;padding-top:10px">
		<button class="Dialog_button btn_pointer btn-primary" onclick="ec.publish()" type="button">${getHtmlText('ec.view.publishview')}</button>
	...
	<div id="form_set_item_property_dlg" style="display:none">
		...
		<div id="dlg-div" class="dlg-etv-navset">
			<div class="etv-scrollbar" style="margin:0 5px;">
				<ul class="etv-nav" style="display: block;">
					...
					<li name="proertyTabs" id="validate_defined">
						<span class="etv-nav-span">
		...
		<table id="tab-common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px;">
			...
			<tr id="tr_showFormat">
				...
				<td class="la complex-td" width="20%" align="right" padding-right="10px"><label id="iscomplexLable">${getHtmlText('ec.view.iscomplexlable')}</label></td>
				<td class="co complex-td" width="30%">
					<input type="checkbox" id="form_property_is_complex" disabled="true"/>
					<select id="selectcomp_type"  class="cui-edit-field" style="display:none;width:100%">
					</select>
					<select id="select_layerType"  class="cui-edit-field" style="display:none;width:100%">
					</select>
				</td>
			</tr>
			<tr id="defaultTR" style="display:none">
				...
				<td class="co" width="80%" colspan="3">
					...
					<div id="ck_boolean_div" style="display:none;width:100%">
						<select id="ck_boolean" class="cui-edit-field" onChange="_changeDefaultValue(this)"  style="width:40%" >
							<option value=""></option>
					...
					<script type="text/javascript">
						...
						function _showFormatChangeEvent(obj){
							...
							if($('#form_property_column_type').val()=='DATE' ){
								$('#defaultTR').show();
								$('#dateRadio_div').show();
								$('#dateTimeRadio_div').hide();
							}else if($('#form_property_column_type').val()=='DATETIME'){
								$('#defaultTR').show();
								$('#dateRadio_div').hide();
								$('#dateTimeRadio_div').show();
							}
			...
			<tr id="tr_common" class="not-query-set">
				...
				<td id="label_checkbox_isTitle" class="co" width="30%">
					${getHtmlText('foundation.view.common.label.title')}<input id="label_checkbox_isTitle_checkbox" type="checkbox" value="true">
				</td>
				<td id="selectcomp_checkbox_select" class="co" width="30%">
					${getHtmlText('foundation.view.common.select')}<input type="checkbox" value="true">
				</td>
				<td id="selectcomp_checkbox_mneenable_checkbox" class="co" width="60%">
					${getHtmlText('ec.view.mneenable')}<input type="checkbox" id="selectcomp_checkbox_mneenable" value="true">
				</td>
			</tr>
			<tr class="tr_outdata" style="display:none;">
				<td class="la" width="20%" align="right" padding-right="10px"><label>${getHtmlText('ec.view.outdata.database.url')}</label></td>
				<td class="co" colspan="3" width="80%">
					<input type="text" id="outdata_database_url" class="cui-edit-field" style="width:100%" >
				</td>
			</tr>
			<tr class="tr_outdata" style="display:none;">
				<td class="la" width="20%" align="right" padding-right="10px"><label>${getHtmlText('ec.view.outdata.database.username')}</label></td>
				<td class="co" width="30%">
					<input type="text" id="outdata_username" class="cui-edit-field" style="width:100%" >
				</td>
				<td class="la" width="20%" align="right" padding-right="10px"><label>${getHtmlText('ec.view.outdata.database.password')}</label></td>
				<td class="co" width="30%">
					<input type="password" id="outdata_password" class="cui-edit-field" style="width:100%" >
				</td>
			</tr>
			<tr class="tr_outdata" style="display:none;">
				<td class="la" width="20%" align="right" padding-right="10px" style="vertical-align: top;"><label>${getHtmlText('ec.view.outdata.database.sql')}</label></td>
				<td class="co" colspan="3" width="80%">
					<textarea id="outdata_sql" class="cui-edit-field" style="width:100%;height:120px;"></textarea>
				</td>
			</tr>
			<tr class="tr-office">
			...
			<tr class="tr-office">
				...
				<td class="la" width="20%" align="right" padding-right="10px"><label id="downloadDoc">${getHtmlText('ec.view.downloadDoc')}</label></td>
				<td class="co" width="30%"><input type="checkbox" id="form_property_is_downloadDoc" /></td>
			</tr>
			<tr class="tr-office">
			...
			<tr class="convert not-query-set">
			...
			<#-- enumerate  -->
			<tr id="baseproperty_enum_enumerate" class="not-query-set">
			...
			<tr id="tr_readonly" class="not-btn-set">			
				<td class="la" width="20%" align="right" padding-right="10px"><label for="form_property_is_readonly" id="isreadOnlyLable"><span i18n='ec.view.isreadonly'>只读</span></label></td>
				<td class="co" width="30%"><input type="checkbox" id="form_property_is_readonly" /></td>
				<td class="la nullable-td" width="20%" align="right" padding-right="10px"><label for="form_property_is_nullable" id="isnullableLable"><span i18n='ec.view.isnullable'>可空</span></label></td>
				<td class="co nullable-td" width="30%"><input type="checkbox" id="form_property_is_nullable" /></td>
				<td class="la operate-td" width="20%" align="right" padding-right="10px"><label id="operator" style="display:none;"><span i18n='ec.flowActive.doperate'>操作</span></label></td>
				<td class="co operate-td" width="30%">
				<select id="form_property_exp_type" class="cui-edit-field" style="display:none;width:96%">
				</select></td>
			</tr>
			<tr class="case-tr query-set">
				<td class="la" width="20%" align="right" padding-right="10px"><label><span i18n='ec.view.caseSensitive'>区分大小写</span></label></td>
				<td class="co" width="30%"><input type="checkbox" id="form_property_caseSensitive" /></td>
			</tr>
			<tr id="tr_picSize" class="not-query-set" style="display:none;">
			...
			<tr class="couple-tr not-query-set">
			...
			<tr id="form_tr_isgroup" class="not-query-set">
			...
			<tr class="iscrosscompany-tr query-set" style="display:none;">
				<td class="la" width="20%" align="right" padding-right="10px"><label><span i18n='ec.view.iscrosscompany'>跨公司</span></label></td>
				<td class="co" width="30%"><input type="checkbox" id="form_property_iscrosscompany" /></td>
			</tr>
			<tr id="form_tr_allow_view" class="not-query-set" style="display:none;">
			...
			<tr id="ref_condition" style="height:30px">
				<td class="la" width="20%" align="right" padding-right="7px"><label id="refConditionLabel">${getHtmlText('ec.view.refCondition')}</label></td>
				<td class="co" colspan="3">
					<textarea id="ref_condition_content" class="cui-edit-textarea" style="width:100%;height:50px;margin-top:5px"></textarea>
				</td>
			</tr>
			<tr class="isrefselect-tr not-btn-set query-set">
				<td class="la" width="20%" align="right" padding-right="10px"><label><span i18n='ec.view.isrefselect'>参照下拉</span></label></td>
				<td class="co" width="30%"><input type="checkbox" id="form_property_isrefselect" /></td>
			</tr>
			<tr class="containLower-tr not-btn-set query-set">
				<td class="la" width="20%" align="right" padding-right="10px"><label><span i18n='ec.view.containLower'>包含下级</span></label></td>
				<td class="co" width="30%"><input type="checkbox" id="containLower" /></td>
			</tr>
			
			<tr id="form_tr_precision" class="not-query-set">
			...
			<tr class="auto-resize-tr not-query-set">
				<td class="la" width="20%" align="right" padding-right="10px">
					<div id="form_textarea_auto_resize_div_label" style="width:100%">
						${getHtmlText('ec.property.auto.resize.td')}
					</div>
				</td>
				<td class="co" width="80%" colspan="3">
					<div id="form_textarea_auto_resize_div_checkbox" style="display:none;width:100%">
		...
		<table id="tab-style" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="display:none">
			...
			<tr class="form-property-style-com">
				<td class="la" style="width:20%" align="right" padding-right="10px">${getHtmlText('ec.view.comstyle')}
...
<p class="baphelp-hint">注意：直接写css样式</p>
				...
				<td class="co" style="width:80%"><textarea id="form_property_field_style_com" class="cui-edit-textarea" style="width:100%;height:50px;margin-top:5px"></textarea></td>
			</tr>
			<tr class="form-property-style-container">
				<td class="la" style="width:20%" align="right" padding-right="10px">${getHtmlText('ec.view.tdstyle')}
...
<p class="baphelp-hint">注意：直接写css样式</p>
				...
				<td class="co" style="width:80%"><textarea id="form_property_field_style_container" class="cui-edit-textarea" style="width:100%;height:50px;margin-top:5px"></textarea></td>
			</tr>
			<tr class="not-btn-set" style="display:none;">
		...
		<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			<tr>
				...
				<td class="la" width="20%" title="${getText("ec.view.config.layout.description")?html}" padding-right="10px">
					<select id="layout_type" class="cui-edit-field" style="width:100px" >
				...
				<td class="co" width="30%">
					${getHtmlText('ec.view.config.selfAdaption')}<input type="checkbox" id="self_adaption">
					<span id="selfAdaptionHelpinfo" class="baphelp-icon"></span>
					<div id="selfAdaptionHelpinfoRef" style="display:none">
						<p class="baphelp-info">标准模式：所有内容在窗口内显示，如果显示不全，在内部出滚动条，不出外部的滚动条。</p>
						<p class="baphelp-info">自适应：表示上面的布局显示内容未超过设定的高度时，布局高度自适应，空出的空间由下面的布局填充。如果超出了，则已设置的高度显示，出滚动条。自适应只适用标准模式。</p>
						<p class="baphelp-info">扩展模式：按照布局内容实际高度显示，布局内不出滚动条，页面显示一个滚动条。</p>
					<p class="baphelp-hint">注意：首页显示处理了，需要设置为扩展模式。</p>
					<script type="text/javascript">
						$('#selfAdaptionHelpinfo').helptip({refElm: "#selfAdaptionHelpinfoRef", html: true , isCustom :false, width: 480 , title :"说明"});
					</script>
				</td>
...
<div id="onSaveHelpinforef" style="display:none">
	...
	<p class="baphelp-example">范例</p>
<div class="baphelp-code">
<pre><code>var type = ReactAPI.getOperateType();
if(type =='submit'){//提交
	alert("这是示例");
...
<p class="baphelp-hint">注意：只有表单才有operateType属性</p>
	...
	<div id="layout-config" style="display:none;">
		...
		<div class="layout-configuration-title">
			...
			<label class="layout-config-border"><input type="checkbox" />${getText('ec.view.layoutNeedBorder')}</label>
		...
		<div class="searchBox">
			<div style="width:100%;height:100%;">
				<div class="search-config-wrapper">
					<button class="search-config-button" onclick="ec.dataClassific(this)"  title="${getText('ec.view.list.dcsetclick')}">${getHtmlText('ec.view.list.dcset')}</button>
				</div>
				<div class="search-config-wrapper">
					<button class="search-config-button" onclick="ec.fastQuerySetting(this,'${view.assModel.code}')" title="${getText('ec.view.list.fastsetclick')}">${getHtmlText('ec.view.list.fastset')}</button>
				</div>
				<div class="search-config-wrapper">
...
<p class="baphelp-hint">注意：直接写css样式。</p>
	...
	<div id="layer_setting_dlg" style="display:none;">
		<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			<tr>
				<td class="la" width="40%" align="right" padding-right="5px">${getHtmlText('ec.view.choiceLayerPro')}</td>
				<td class="co" width="60%">
					<select id="layerProperty_type" class="cui-edit-field" style="width:100%">
					<#if view.assModel.properties?has_content>
						<#list view.assModel.properties as p>
							<#if p.type == "LAYER">
								<option value="${(p.code)!}" id="${(p.code)!}" name="${view.assModel.modelName?uncap_first}.${(p.name)!}" namekey="${(p.displayName)!}">${getText('${p.name}')}[${getText('${p.displayName}')}]</option>
							</#if>
						</#list>
					</#if>
					</select>
			...
			<tr>
				<td class="la" width="40%" align="right" padding-right="5px">${getHtmlText('ec.view.layerType')}</td>
				<td class="co" width="60%">
					<select id="layer_type" class="cui-edit-field" style="width:100%">
					<#if layerTypes?has_content>
						<#list layerTypes as layerType>
							<option value="${layerType.id}">${getText('${layerType.value}')}</option>
						</#list>
					</#if>
					</select>
				</td>
				...
				<table style="border: 1px solid #DDD;padding: 0 10px;margin: 10px 0 0 10px;width: 100%;">
					...
					<tr class="regexExampleRow">
						...
						<td class="co" colspan="3">
							<div class="fix-input">
								...
								<@international name="errorMsgRegex" isNew=true isOldEdit=true moduleCode="${view.assModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-noborder-input" />
							</div>
						</td>
					</tr>
					...
					<tr class="customExampleRow">
						...
						<td class="co" colspan="3">
							<div class="fix-input">
								<#-- international_errorMsg    international_errorMsg_showName -->
								<@international name="errorMsgCustom" isNew=true isOldEdit=true moduleCode="${view.assModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-noborder-input" />
							</div>
						</td>
					</tr>
				</table>
	...
	<div id="button_property_dlg" style="display:none">
		...
		<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			<tr>
				<td class="la" width="20%" style="color: rgb(179, 3, 3);" align="right" padding-right="10px">${getHtmlText('ec.view.operatetype')}</td>
				<td class="co" width="30%">
					<select id="operatetype"  class="cui-edit-field" style="width:100%">
						<option value="CUSTOM">${getText('ec.view.customoperate')}</option>
						<#--<option value="CLOSE">${getText('foundation.common.closeWindow')}</option>-->
					</select>
				</td>
			...
			<tr>
				...
				<td class="la" width="20%" padding-right="10px"></td>
				<td class="co" width="30%">
				</td>
			</tr>
			<tr id="permissionTr">
			...
			<tr>
				...
				<td class="la" width="20%" align="right" padding-right="10px" id="td_align_label"style="<#if view.openType == 'dialog'><#else>display:none;</#if>">${getHtmlText('ec.edit.button.align')}</td>
				<td class="co" width="30%" id="td_align_select" style="<#if view.openType == 'dialog'><#else>display:none;</#if>">
			...
			<tr id="eventTr">
				<td class="la" width="30%" align="right" style="color: rgb(179, 3, 3);" padding-right="10px">${getHtmlText('ec.edit.button.onclick')}<span id="functionHelpinfo" class="baphelp-icon"></span>
...
<script type="text/javascript">	
	...
	//自动设置中间编辑区域的宽度
	function _init_size(){
		$('.col-main').width($(window).width()-348);
		$('.col-main').height($(window).height()-117);
		...
		$('#layoutTree').height($(".col-sub .treeBox").height()-41);
		$('.col-operate').height($(window).height()-117);
		<#if view.openType == 'dialog'>	
		...
		$('#form_area .etv-content > .tab-pane').height($('#form_area .etv-content').height()-15);
		<#else>
		$('#form_area .etv-navset').height($(window).height()-117);
		//$('#form_area .etv-navset').width($(window).width()-251);
		$('#form_area .etv-navset').width($('.col-main').width()-2);

		$('#form_area .etv-content').height($(window).height()-237);
		//$('#form_area .etv-content').width($(window).width()-255);
		$('#form_area .etv-content').width($('.col-main').width()-10);
		$('#form_area .etv-content > .tab-pane').height($('#form_area .etv-content').height()-4);
		</#if>
