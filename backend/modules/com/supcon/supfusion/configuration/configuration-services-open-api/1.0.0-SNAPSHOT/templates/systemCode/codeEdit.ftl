<@errorbar id="SubmitSystemEntityFormDialogErrorBar"></@errorbar>
...
CUI(function(){
	...
	function ajaxFileUpload(sUrl,formId) {
		ajaxFileUploadCommon({
				...
				success : function(msg) {
					var formId = 'SubmitSystemEntityForm';
					...
					if(msg && msg.success == false){
						...
						CUI.each(msg.items,function(index,item){
							if(index.indexOf('.id') != -1 && index.indexOf('.id')+3 == index.length) {
								$("#SubmitSystemEntityForm *[name^='"+(index.substring(0,index.length -3))+"'][type!='hidden']").each(function(){
									if($(this).parents('td[nullable=false]').length > 0) {
										showErrorField($(this));
									}
								});
							} else if($("#SubmitSystemEntityForm input[name='"+index+"AddIds'][type='hidden']").length > 0) {
								// 多选控件验证
								showErrorField($("#SubmitSystemEntityForm *[name='"+index+"AddIds']").parent('div'));
							} else {
								var field = CUI("#SubmitSystemEntityForm *[name='"+index+"']");
							...
							CUI("#SubmitSystemEntityForm *[name='"+index+"']").first().focus();
						...
						oErrorWidget = SubmitSystemEntityFormDialogErrorBarWidget;
						oErrorWidget.showMessage(errorMsgs);
						// oErrorWidget.show(errorMsgs);
						if(CUI.Dialog){
					    	 CUI.Dialog.toggleAllButton('SubmitSystemEntityForm', true);
					     }
					}else{
						window.onbeforeunload = null;
						...
						foundation.systemCode.systemEntityCallBackInfo(msg,CUI('#'+formId).serialize());
					}
				}
			}
		);
	...
	function submitBapForm(){//电子签名成功之后出现进度条并提交表单
		...
		if(ecFormFlag && ( $('#SubmitSystemEntityForm').parents('.ewc-dialog-blove').length > 0 || ( window != top && !window.frameElement.getAttribute( 'v3_tab_iframe' ) ) ) ) {
			// dialog不出进度条
			ecFormFlag = false;
		}
		...
		//前台验证通过之后出进度条
		CUI.Dialog.toggleAllButton('SubmitSystemEntityForm',true,ecFormFlag, true);
	...
	setTimeout(function(){
			...
			CUI.ajax({
				...
				success : function(msg){
					...
					foundation.systemCode.systemEntityCallBackInfo(msg,postData);
				}
			});
		}
	}, 600);
	...
	CUI('#SubmitSystemEntityForm').unbind('submit.bapForm').bind('submit.bapForm',function(){
		//每次提交时先隐藏报错信息
		try{
		}catch(e){
		...
		//if(CUI.Dialog) CUI.Dialog.toggleAllButton('SubmitSystemEntityForm',true,ecFormFlag);
		//});
		$('#SubmitSystemEntityForm').trigger('beforeSubmit');
		if($('#SubmitSystemEntityForm input[name="operateType"]').val() == "submit"){
			var deploymentId=$('#SubmitSystemEntityForm input[name="deploymentId"]');
			var buttonCode=$('#SubmitSystemEntityForm input[name="buttonCode"]');
			var namespace=$('#SubmitSystemEntityForm input[name="namespace"]');
			if( deploymentId.length > 0 && deploymentId.val() != undefined && deploymentId.val() != ''){
				...
				if(signatureInfo[0] != '') {
					...
					if(cancelItem.val() != "cancel") {
						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],'ec.flow.submit','','SubmitSystemEntityForm');
						$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','SubmitSystemEntityForm','ec.flow.submit',false)});
					}
					else {
						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],'ec.edit.remove','','SubmitSystemEntityForm');
						$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','SubmitSystemEntityForm','ec.edit.remove',false)});
					}
				...
				if(signatureInfo[0] != '') {
					if(namespace.length > 0 && namespace.val() != undefined && namespace.val() != '') {
						parent.signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],buttonCode.val(),namespace.val(),"SubmitSystemEntityForm",false,'');
						if(signatureInfo[0] == 'singleSign') {
							parent.$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk(namespace.val(),'SubmitSystemEntityForm',buttonCode.val(),false)});
						}
						else {
							setTimeout(function(){parent.$('#secondSignerOK').unbind('click').bind('click',function(){doubleSignOK(this,namespace.val(),'SubmitSystemEntityForm',buttonCode.val(),false)});},2000);
						}
					}
					else {
						signatureUtil.showSignatureDialog(signatureInfo[1],signatureInfo[0],buttonCode.val(),'',"SubmitSystemEntityForm",false,'');
						if(signatureInfo[0] == 'singleSign') {
							$('a[name="signature_signatureDialog_showDialog_0"]').unbind('click').bind('click',function(){singleSignOk('','SubmitSystemEntityForm',buttonCode.val(),false)});
						}
						else {
							setTimeout(function(){$('#secondSignerOK').unbind('click').bind('click',function(){doubleSignOK(this,'','SubmitSystemEntityForm',buttonCode.val(),false)});},2000);
						}
					}
				}
				else {
...
<form id="SubmitSystemEntityForm" name="SubmitSystemEntityForm" action="/msService${namespace}/systemEntitySave" method="post" callback="foundation.systemCode.systemEntityCallBackInfo">
	<input type="hidden" name="entityId" value="<#if systemEntity?? && systemEntity.id??>${systemEntity.id}</#if>" id="SubmitSystemEntityForm_systemEntity_id"/>
	<input type="hidden" name="systemEntity.id" value="<#if systemEntity?? && systemEntity.id??>${systemEntity.id}</#if>" />
    <input type="hidden" name="systemEntity.version" value="0" id="SubmitSystemEntityForm_systemEntity_version"/>
	<table cellpadding="0"  cellspacing="0" class="cui-fd-infotable">
		<tr>
			<td class="lab" style="width:8%;color:#B30303">
				${getHtmlText('foundation.systemCode.code')} 
			</td>
			<td style="width:16%" align="left">
				<#if systemEntity??&&systemEntity.code??>
					<div class="fix-input-readonly">
					<input type="text" name="systemEntity.code" value="${systemEntity.code}" id="SubmitSystemEntityForm_systemEntity_code" readOnly="true" class="cui-noborder-input"/>
					</div>
				<#else>
					<div class="fix-input">
					<input type="text" name="systemEntity.code" value="" id="SubmitSystemEntityForm_systemEntity_code" class="cui-noborder-input"/>
					</div>
					<span class="description" style="display:block;margin-top:-1px;margin-bottom:-8px">${getHtmlText('foundation.systemEntity.code.tip')}</span>
					</#if>
			</td>
			<td class="lab" style="width:10%;color:#B30303">${getHtmlText('foundation.systemCode.name')}</td>
			<td style="width:16%">
				<#if systemEntity??>
				...
				<@international name="systemEntity.name" key="${(systemEntity.name)!}" moduleCode="${modulecode}" isNew=true maxLength=80>${getText(systemEntity.name)}</@international>
			</td>
		</tr>
		<tr>
			<td class="lab">${getHtmlText('foundation.systemCode.listType')}</td>
			<td  style="*padding-right:1px;">
			    <select name="systemEntity.listType" id="listType" style="width:100%">
                    <option value="LIST">列表显示</option>
                    <option value="TREE">树形显示</option>
                </select>
			</td>
		</tr>
		<tr>
			<#if (systemEntity.moduleCode)??>
			<td class="lab">${getHtmlText('foundation.menuinfo.module')}</td>
			<td>
			<div class="fix-input-readonly">
			<input type="hidden" name="systemEntity.moduleCode" value="${modulecode}" id="codeEditId"/>
			<input id="SubmitSystemEntityForm_modulename" class="cui-noborder-input" type="text" readonly="true" value="${moduleName}" name="modulename">
			...
			<td>
				<div class="fix-input-readonly">
				<input type="hidden" name="systemEntity.cid" value="${getCurrent('companyId')}" id="SubmitSystemEntityForm_systemEntity_cid"/>
				<input type="text" name="companyName" value="${getCurrent('companyName')}" id="SubmitSystemEntityForm_companyName" class="cui-noborder-input" readOnly="true"/>
				</div>
			</td>
		</tr>
		<tr>
			<td class="lab v-align">${getHtmlText('foundation.systemCode.meno')}</td>
			<td class="cui-vte" colspan="3">
				<div class="fix-input">
				<textarea name="systemEntity.memo" cols="" rows="" id="SubmitSystemEntityForm_systemEntity_memo" class="cui-noborder-textarea" style="width:100%" onkeyup="systemEntityCheckLimit(this)">${systemEntity.memo!}</textarea>
				</div>
			</td>
		</tr>
	</table>
...
<script type="text/javascript" charset="UTF-8" language="javascript">
	$("#listType").setValue("${systemEntity.listType!}".toUpperCase());
	function systemEntityCheckLimit(obj){
		var limit=200;
