...
<body class="ec-config-page" style="background-color:#f2f2f2" layoutCode="${(ev.configMap.layout.layoutCode)!ecCodeInit('layout')}" page_onload="${((ev.configMap.layout.pageConfig.onload)!)?html}" >
	...
	<div class="layout grid-s4m0">
						...
						<ul class="main_properties_container">
							<#if (properties)?? && (properties)?size &gt; 0>
									<#list properties as p>
										<#if !(p.isCustom!false) || (p.projCustomInUse)!false>
										<#if !assMap?? || (assMap?? && !assMap[p.code]??)>
											<#if (p.type)?? && (p.type) != 'LAYER'>
		...
		<div class="col-main"> 	
			...
			<input type="hidden" id="isProj" value="${(isProj?c)!}" />
				...
				<div id="form_area" style="padding-left:3px;">
					<div id="main_form_tab" class="etv-navset">
						...
						<div style="background-color:#e3f1f9;" class="etv-content" >
							...
							<#--<#if !(ev.configMap)??>
								<div class="div-selected">
									<ul class="form_design_ul se-selected" sectionCode="${ecCodeInit('section')}" id="designarea" colNum="6">
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										...
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
										<li class="form_design_ul_li" cellCode="${ecCodeInit()}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="2" rowspan="1" namekey="ec.common.query" name="${getText('ec.common.query')}" showType="BUTTON"><dd><input type="button" style="width:70px!important" value="${getText('ec.common.query')}"/></dd><div></div></li>
									</ul>
							<#else>
							<#assign configMap = (ev.configMap.layout)>
							<#list (configMap.sections) as section>
								<#if (section.regionType)?? && (section.regionType)=='FASTQUERY'>
									<#assign hasRegion = "true">
									<#break>
								</#if>
							</#list>
							<#if !hasRegion?? || hasRegion =='false'>
								<div class="div-selected">
									<ul class="form_design_ul se-selected" sectionCode="${ecCodeInit('section')}" id="designarea" colNum="6">
								...
								<div class="div-selected">
									<#list (configMap.sections) as secatt>
										<#if (secatt.regionType)?? && (secatt.regionType)=='FASTQUERY'>
											...
											<ul class="form_design_ul se-selected" sectionCode="${sectionCode!}" id="designarea" colwidth="${(secatt.pageConfig.colwidth)!}" colNum="${(secatt.pageConfig.colNum)!6}" name="${(secatt.name)!}" isborder="${(secatt.isborder)!1}">
												...
												<#if (element.element)??>
													...
													<#if (element.element.showType) == 'LABEL'>
														...
														<li class="form_design_ul_li" cellCode="${cellCode!}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" name="${(element.element.name)!}" key="${(element.element.key)!}" propertyCode="${(element.element.propertyCode)!}" namekey="${(element.element.namekey)!}" mnecode='${(element.element.mnecode!false)?string('true','false')}'  isrefselect='${(element.element.isrefselect!false)?string('true','false')}' layRec="${(element.element.layRec)!}" propnullable="${(property.nullable)?string("true","false")}" nullable="${(element.element.nullable)?string("true","false")}" isreadonly="${(element.element.readonly)?string("true","false")}" exp="${(element.element.exp)!}" entityCode="${(element.element.entityCode)!}" layRec="${(element.element.layRec)!}" assTar="${(element.element.assTar)!}" assOrg="${(element.element.assOrg)!}" multable="${((element.element.multable)!false)?string('true','false')}" readonly="${(element.element.readonly)?string("true","false")}"  showType="${(element.element.showType)!}"  sourcepropertyname="${(element.sourcepropertyname)!}" callbackbody="${((element.callbackbody)!)?html}" callbackname="${((element.callbackname)!)?html}" funcname="${((element.funcname)!)?html}" funcbody="${((element.funcbody)!)?html}" selectCompType="${(element.element.selectCompType)!}" cssstyle="${((element.cssstyle)!"")?html}" referenceview="${(element.element.referenceview)!}" modelcode="${(element.element.modelcode)!}" columnType="${(property.type)!}" colspan="${(element.colspan)!1}" rowspan="${(element.rowspan)!1}" 
															showFormat="${(element.element.showFormat)!}" fill='${(property.fillcontent)!}'>
														<#else>
													...
													<#else>
														<#if (element.element.propertyCode)?? &&propertyMap?? && propertyMap['${propCode}']?? >
															...
															showType="<#if element.element.showTypeHasChanged!false>${(element.element.showType)!}<#else>${(property.fieldType!)}</#if>" showFormat="<#if element.element.showFormatHasChanged!false>${(element.element.showFormat)!}<#else>${(property.format!)}</#if>" fill='${(property.fillcontent)!}'>
														<#else>
															<li class="form_design_ul_li" cellCode="${cellCode!}" ondblclick="listec.cellProperty()" defaultValue="${(element.element.defaultValue!)?string}" propDefaultValue="${(element.element.defaultValue!)?string}" defaultValueHasChanged="${(element.element.defaultValueHasChanged!false)?string('true','false')}" onmousedown="listec.selectListLi(this)" partDepend="${(element.element.partDepend)!'common'}" columnType="${(property.type)!''}" name="${(element.element.name)!}" namekey="${(element.element.namekey)!}" key="${(element.element.key)!}" mnecode='${(element.element.mnecode!false)?string('true','false')}'  isrefselect='${(element.element.isrefselect!false)?string('true','false')}' propertyCode="${(element.element.propertyCode)!}" layRec="${(element.element.layRec)!}" nullable="${(element.element.nullable)?string("true","false")}" isreadonly="${(element.element.readonly)?string("true","false")}" exp="${(element.element.exp)!}" entityCode="${(element.element.entityCode)!}" layRec="${(element.element.layRec)!}" assTar="${(element.element.assTar)!}" assOrg="${(element.element.assOrg)!}" multable="${((element.element.multable)!false)?string('true','false')}" columnLong="${(element.element.columnLong)!}" readonly="${(element.element.readonly)?string("true","false")}" checkname="${ckname!}" showType="${(element.element.showType)!}"   sourcepropertyname="${(element.sourcepropertyname)!}" callbackbody="${((element.callbackbody)!)?html}" callbackname="${((element.callbackname)!)?html}" funcname="${((element.funcname)!)?html}" funcbody="${((element.funcbody)!)?html}" selectCompType="${(element.element.selectCompType)!}" cssstyle="${((element.cssstyle)!"")?html}" referenceview="${(element.element.referenceview)!}" modelcode="${(element.element.modelcode)!}" columnType="${(property.type)!}" colspan="${(element.colspan)!1}" rowspan="${(element.rowspan)!1}" 
															showFormat="${(element.element.showFormat)!}" fill='{<#if (element.element.fill)?has_content><#list (element.element.fill)?keys as fe><#if fe_index &gt; 0>,</#if>"${fe}":<#if (element.element.fill.fillType)?has_content && (element.element.fill.fillType?string) == '4' && fe == 'fillContent'>{<#if element.element.fill.fillOrder?has_content><#list element.element.fill.fillOrder?split(",") as ne><#if ne_index &gt; 0>,</#if>"${ne}":"${((element.element.fill.fillContent)[ne])?html}"</#list><#else><#list (element.element.fill.fillContent)?keys as ne><#if ne_index &gt; 0>,</#if>"${ne}":"${((element.element.fill.fillContent)[ne])?html}"</#list></#if>}<#else>"${((element.element.fill)[fe])?html}"</#if></#list></#if>}'>
														</#if>
														<dd>
															<#if (element.element.showType) == 'DATE' || (element.element.showType) == 'DATETIME'>
																<input type="text" style="color:gray" value="${ckname!''}"/><input type='button' class='cui-calpick' />
															<#elseif (element.element.showType) == 'TEXTFIELD'>
																<input type="text" style="color:gray" value="${ckname!''}"/>
															<#elseif (element.element.showType) == 'SELECTCOMP'>
																<input type="text" style="color:gray;margin-right:-5px;" value="${ckname!''}"/><input type='button' class='cui-search-click' />
															<#elseif (element.element.showType) == 'SELECT'>
																<select style="color:gray"><option>${ckname!''}</option></select>
												...
												<#else>
													<li class="form_design_ul_li" cellCode="${cellCode!}" ondblclick="listec.cellProperty()" onmousedown="listec.selectListLi(this)" colspan="1" rowspan="1"><dd></dd><div></div></li>
												</#if>
												</#list>
												</ul>
											<#break>
							...
							-->
								...
								<div class="operateDiv">
									...
									<div style="padding-left:20px;display:inline-block">
										...
										<#--<a class="cui-btn mr10 cui-btn-setting" href="#" onclick="listec.cellPageProperty()">${getHtmlText('ec.view.pageProperty')}</a>-->
								...
								<table cellspacing="0" cellpadding="0" class="ec_table_bottom">									
									...
									<tr>
										...
										<td>
											...
											<#if !(ev.configMap)??>
												<tr class="list_design_ul" operService="${serviceName!}" sectionCode="${ecCodeInit('section')}" id="list_design_section" isCheckBox="false" isFirstLoad="false" isExportExcel="true" selectFirstRow="false" isTransCondition="false" conditionContent="" isSuperTable="false" renderOver="" ptPageInit="" isdbcustom="false">
											...
											<#else>
												...
												<#if section.cells?? && (section.cells)?size <= 0>
													<tr class="list_design_ul" sectionCode="${ecCodeInit('section')}" operService="${serviceName!}" id="list_design_section" isTreeView="false" nodeExpanded="false" isCheckBox="false" isFirstLoad="false" isExportExcel="true" selectFirstRow="false" isTransCondition="false" conditionContent="" isSuperTable="false" renderOver="" ptPageInit="" isdbcustom="false">
													<#if attachment?? && attachment == 'true'>
													<td class="list_design_ul_li" cellCode="${ecCodeInit()}" key='bapAttachmentInfo' name="bapAttachmentInfo" namekey="foundation.upload.attachment" showType='ATTACHMENT' fill='' isOrderBy='false' sortType='' seqNumber='' propertyCode='' checkname='' formulas='' hide=false proWidth=250 textalign='center' ondblclick="listec.fieldProperty()" onmousedown="listec.selectListLi(this)"><dd>${getText('foundation.upload.attachment')}</dd></td>
													</#if>
													<tr>
														...
														<#list section.cells as column>
															...
															<#if !(column.none)??>
																...
																<#assign prop = propertyMap['${(propCode)!}'] >
																 ...
																 propertyCode='${(column.propertyCode)!}' hide=${(column.isHidden!false)?string("true","false")} proWidth=${(column.width)!100} style="width: ${(column.width)!100}px" ondblclick="listec.fieldProperty()" onmousedown="listec.selectListLi(this)" funcname="${((column.funcname)!)?html}" funcbody="${((column.funcbody)!)?html}"><dd>${getText('${(column.namekey)!(column.label)!}')}</dd></td>
																 <#else>
																 <td class="list_design_ul_li" cellCode="${cellCode!}" name='${(column.key)!}' displayDefaultType="${(column.displayDefaultType)!}" openPending=<#if !column.openPending?? && column.key?? && column.key == 'tableNo'>true<#else>${(column.openPending!false)?string("true","false")}</#if> isCount=${(column.isCount!false)?string("true","false")} isTotal=${(column.isTotal!false)?string("true","false")} namekey="${(column.namekey)!}" key='${(column.key)!}' <#if column.multable??>multable=${(column.multable)?string("true","false")}<#else>multable=false</#if> <#if column.isOrderBy??>isOrderBy=${(column.isOrderBy)?string("true","false")}<#else>isOrderBy=false</#if> sortType='${(column.sortType)!}' seqNumber='${(column.seqNumber)!}' <#if column.isLink??>isLink=${(column.isLink)?string("true","false")}</#if> assModels='${(column.assModels)!}' linkView='${(column.linkView)!}' assModelCode='${(column.assModelCode)!}' modelCode='${(column.modelCode)!}' mnecode='${(column.mnecode!false)?string('true','false')}' showType='${(column.showType)!}' layRec='${(column.layRec)!}' formulas="${((column.formulas)!'')?html}" textalign="${(column.textalign)!'center'}" decimalNum='${(column.decimalNum)!}'
																	fill='{<#if (column.fill)?has_content><#list (column.fill)?keys as fe><#if fe_index &gt; 0>,</#if>"${fe}":<#if (column.showType) == 'ENUMERATE' && fe == 'fillContent'>{<#list (column.fill)[fe]?keys as ne><#if ne_index &gt; 0>,</#if>"${ne}":"${((column.fill)[fe][ne])?html}"</#list>}<#else>"${((column.fill)[fe])?html}"</#if></#list></#if>}'
																 	...
																 	seniorSystemcode="${(column.seniorSystemcode!false)?string("true","false")}"
																 	<#if (column.ass)?? && column.ass?has_content><#list column.ass?keys as asstar><#if asstar == 'tar'>assTar<#else>assOrg</#if>='${(column.ass)[asstar]!}'</#list></#if>
																 propertyCode='${(column.propertyCode)!}' hide=${(column.isHidden!false)?string("true","false")} proWidth=${(column.width)!100} style="width: ${(column.width)!100}px" ondblclick="listec.fieldProperty()" onmousedown="listec.selectListLi(this)"><dd><#if (column.customSection)!false>自定义字段区域&nbsp;[${(column.modelNameInternational)!}]<#else>${getText('${(column.namekey)!(column.label)!}')}</#if></dd></td>
																</#if>
	...
	<div id="design-button" style="height:30px;width:100%;float:right;text-align:right;margin-top:20px">
		<#if !isReadOnlyMode>
		<button class="Dialog_button btn_pointer btn-primary" onclick="listec.saveListPT()" type="button" style="margin-right:20px">${getHtmlText('common.button.save')}</button>
		</#if>
		<button class="Dialog_button btn_pointer" onclick="window.close()" type="button" style="margin-right:20px">${getHtmlText('common.button.cancel')}</button>
		<#--
	...
	<div id="form_set_item_property_dlg" style="display:none">
		...
		<table id="tab-common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			...
			<tr>
				...
				<@international name="formProperty.showName" isNew=true isOldEdit=true moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-edit-field" />
				</td>
			</tr>
			...
			<tr id="ref_condition">
				<td class="la" width="20%" align="right" padding-right="10px"><label>${getHtmlText('ec.view.refCondition')}</label></td>
		...
		<table id="tab-style" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px;display:none">
			...
			<tr class="not-btn-set">
				<td class="la" style="width:20%" align="right" padding-right="10px">${getHtmlText('ec.view.customstyle')}</td>
 	...
 	<div id="orderby_sort_dlg" style="display:none;">
		...
		<div id="listContent" style="background-color:#f8f6f7;position:absolute;right:0px;top:0px;width:25%;border: 1px solid #efefef; height: 280px;">
			<div class="ec-list-btndiv"><div class="ec-list-topbtn" onclick="listec.firstRow('list')" id="firstMove" ></div></div>
			<div class="ec-list-btndiv"><div class="ec-list-prevbtn" onclick="listec.upRow('list')" id="upMove" ></div></div>
			<div class="ec-list-btndiv"><div class="ec-list-nextbtn" onclick="listec.downRow('list')" id="downMove"></div></div>
			<div class="ec-list-btndiv"><div class="ec-list-lastbtn" onclick="listec.lastRow('list')" id="lastMove"></div></div>
		</div>
 	</div>
 	<div id="listFieldDlg" style="display:none;">
		...
		<table id="field-tab-common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			...
			<tr class="config_relation_model_props">
				...
				<#--international_propertyshowName    international_propertyshowName_showName -->
					<@international name="property.showName" isNew=true isOldEdit=true moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-edit-field" />
				</td>
			...
			<#if !dataGrid.targetModel.entity.isBase && dataGrid.targetModel.isMain?? && dataGrid.targetModel.isMain>
...
<p class="baphelp-hint">注意：直接写css样式，不能设置宽度高度</p>
	...
	<div id="listPropertyDlg" style="display:none;">
		...
		<table id="tab_common" class="infoTable" cellpadding="0" cellspacing="0" border="0" width="80%" align="left" style="margin-top: 10px">
			...
			<tr>
				<td class="la" style="width:10%;padding-right:11px;">
				<input type="hidden" id="dataGrid_isPermission" name="dataGrid.isPermission" value="<#if dataGrid??&&dataGrid.isPermission??&&dataGrid.isPermission>true<#else>false</#if>"/>
				<input type="checkbox" id="isPermission_cbx" onclick="ec.config.isPermission();" <#if dataGrid??&&dataGrid.isPermission??&&dataGrid.isPermission>checked="checked"</#if> /></td>
				<td class="co" width="40%">${getHtmlText('ec.view.listpt.permission')}</td>
				<td class="la" width="10%" style="padding-right:10px;">${getHtmlText('ec.action.datagrid.properties.title')}</td>
				<td class="co" width="40%">
					<@international name="dataGrid.dataGridName" moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="${dataGrid.dataGridName!''}" style="width:100%;margin-right: -5px;" cssClass="cui-edit-field" />
				</td>
			</tr>
			<tr id="dataGrid_permission_tr1" style="display:none;">
				<td class="la permissionCode" style="width:17%" >${getHtmlText('ec.view.listpt.permission.code')}</td>
				<td class="co permissionCode" style="width:33%;">
				    <input type="text" name="dataGrid.permissionCode" value="${dataGrid.permissionCode!}" id="dataGrid_permissionCode" class="cui-edit-field"/>
				</td>
				<td class="la" style="width:17%" >${getHtmlText('ec.view.listpt.operate.name')}</td>
				<td class="co" style="width:33%;padding-right:10px;">
	
					<@international name="dataGrid.operateName" isNew=true isOldEdit=true moduleCode="${(dataGrid.targetModel.entity.module.artifact)!}" key="${(dataGrid.operateName)!}" isNew=true cssClass="cui-edit-field"></@international>
				</td>
			</tr>
			<tr>
				<td class="la" width="11%" style="padding-right:10px;"><input type="checkbox" id="isExportExcel" /></td>
				<td class="co" width="39%">${getHtmlText('ec.view.exportexcel')}</td>
...
<div id="renderOverHelpinforef" style="display:none">
	<p class="baphelp-info">DATATABLE数据请求执行接口</p>
	<p class="baphelp-example">范例</p>
<div class="baphelp-code">
<pre><code>alert ( " 示例 " );
</code></pre>
...
<div id="ptInitHelpinforef" style="display:none">
	<p class="baphelp-info">DATATABLE的初始化函数</p>
	<p class="baphelp-example">范例</p>
<div class="baphelp-code">
<pre><code>alert ( " 示例 " );
</code></pre>
...
<div id="dbclickHelpinforef" style="display:none">
	<p class="baphelp-info">DATATABLE行双击事件，配置后行默认的双击事件以该配置为准</p>
	<p class="baphelp-example">范例</p>
<div class="baphelp-code">
...
<p class="baphelp-hint">注意：event为事件对象,row为双击行的对象</p>
					...
					<script type="text/javascript">
						$('#dbclickHelpinfo').helptip({refElm: "#dbclickHelpinforef", html: true , isCustom :false, width: 370 , title :"说明"});
					</script>
			</td>
	...
	<div id="button_property_dlg" style="display:none">
		...
		<table class="infoTable" cellpadding="0" cellspacing="0" border="0" width="95%" align="left" style="margin-top: 10px">
			<tr>
				...
				<#--international_propertyshowName    international_propertyshowName_showName -->
				<@international name="buttonProperty.showName" isNew=true isOldEdit=true moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-edit-field" />
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="co" width="30%">
					<select id="operatetype"  class="cui-edit-field" style="width:100%">
						...
						<#if dataGrid.targetModel.entity?exists&&dataGrid.targetModel.entity.isBase>
						<option value="RESTORE">${getText('ec.view.resoperator')}</option>
						</#if>
						<#if dataGrid.targetModel.isMain!false>
			...
			<tr>
				...
				<td class="co confirmcontent" width="30%">
					...
					<@international name="confirmcontent" isNew=true isOldEdit=true moduleCode="${dataGrid.targetModel.entity.module.artifact}" key="" style="width:100%;margin-right: -5px;" cssClass="cui-edit-field" />
				</td>
			</tr>
			<tr>
...
<p class="baphelp-hint">注意：该时刻，DataGrid或DataTable可能还未加载完，不可再对这两个对象进行操作</p>
	...
	<script type="text/javascript">
		...
		var _viewAssModelCode = "${model.code}";
		$(function(){
...
<script type="text/javascript">	
	...
	$(function(){
		...
		var liArr = $("#dlg-div-field").find('li');
		liArr.each(function(index){
		    var objMe = $(this);
			var objId = $(objMe).attr('id');
		    $(this).unbind('click').bind('click',function(){
	...
	function openConfigRelationModelPropsDlg(){
		var configRelationModelPropsDlg = new CUI.Dialog({
			...
			width    :  550,
			height   :  422,
			url      :  "/msService/ec/view/configRelationModelPropsFrame?modelCode=${dataGrid.targetModel.code}",
			modal    :  true,
			dragable :  true,
	...
	*/
	ec.config.customProp.selectModel = function(e, obj) {
		...
		ec.config.customProp.showOverLayer(obj, "/msService/ec/view/config-select-model?model.code=${(model.code)!}");
	}
	...
	ec.config.isPermission = function(){
		var p_cbx = $("#isPermission_cbx");
		$("#dataGrid_isPermission").val(p_cbx.prop('checked'));
		if(p_cbx.prop('checked')){
			$('.permissionCode').show();
			$('#dataGrid_permission_tr1').show();
		} else {
			$('.permissionCode').hide();
			$('#dataGrid_permission_tr1').hide();
		}
	}
	...
	$(function(){
		...
		if(p_cbx.prop('checked')){
			$('.permissionCode').show();
			$('#dataGrid_permission_tr1').show();
			if(undefined != $("#dataGrid_permissionCode").val() && $("#dataGrid_permissionCode").val() != ""){
				$("#dataGrid_permissionCode").attr("readonly","true");
			}
		} else {
			$('.permissionCode').hide();
			$('#dataGrid_permission_tr1').hide();
