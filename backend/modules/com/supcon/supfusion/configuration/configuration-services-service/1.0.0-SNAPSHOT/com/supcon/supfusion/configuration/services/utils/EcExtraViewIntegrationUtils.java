...
public class EcExtraViewIntegrationUtils {
	...
	public String ecExtraViewIntegrationBuild(Object obj, Map<String, List<?>> infoMap) {
		...
		if (obj instanceof View) {
			...
			if (null != ev && null != ev.getConfig() && ev.getConfig().length() > 0) {
				config = ev.getConfig();
				if(isExtra){
					if(view.getFastQueryJson() != null && !view.getFastQueryJson().isEmpty()){
						infoMap.put("fastQueryJsons", view.getFastQueryJson());	
					}
					if(view.getAdvQueryJson() != null && !view.getAdvQueryJson().isEmpty()){
						infoMap.put("advQueryJsons", view.getAdvQueryJson());
					}
				}
	...
	private void ecSectionConfigBuilder(List<Map<String, Object>> sections, List<Field> fields, List<Button> buttons) {
		if (sections != null && !sections.isEmpty()) {
			for (Map<String, Object> section : sections) {
				...
				if (regionType.equals("EDIT") || regionType.equals("LISTPT") || regionType.equals("FASTQUERY")
						|| regionType.equals("DATAGRID") || regionType.equals("MNECODE")) {
					List<Map<String, Object>> cells = (List<Map<String, Object>>) section.get("cells");
					if (null != cells && !cells.isEmpty()) {
						ecCellConfigBuilder(cells, fields, buttons, RegionType.valueOf(regionType));
					}
				} else if (regionType.equals("BUTTON")) {
					List<Map<String, Object>> cells = (List<Map<String, Object>>) section.get("cells");
					if (null != cells && !cells.isEmpty()) {
						for (Map<String, Object> cell : cells) {
							if (cell.get("cellCode") != null && cell.get("cellCode").toString().length() > 0) {
								String cellCode = cell.get("cellCode").toString();
								if (null != buttons && !buttons.isEmpty()) {
									for (Button button : buttons) {
										if (button.getCellCode() != null && button.getCellCode().equals(cellCode)) {
											// 字段事件
											Set<Event> eventSet = button.getEvents();
											ecCellEventBuilder(cell, eventSet);

											Map<String, Object> buttonsMap = null;
											if (button.getConfig() != null && button.getConfig().length() > 0) {
												buttonsMap = (Map<String, Object>) SerializeUitls.deserialize(button.getConfig());
											}
											if (buttonsMap != null && !buttonsMap.isEmpty()) {
												Map<String, Object> buttonMap = (Map<String, Object>) buttonsMap.get("button");
												if (buttonMap != null && !buttonMap.isEmpty()) {
													if (buttonMap.get("selectType") != null) {
														cell.put("selectType", buttonMap.get("selectType"));
													}
												}
											}
											// 添加字段信息
											if (null != button.getName() && button.getName().length() > 0) {
												cell.put("id", button.getName());
											}
											if (null != button.getDisplayName() && button.getDisplayName().length() > 0) {
												cell.put("namekey", button.getDisplayName());
											}
											if (null != button.getButtonStyle() && button.getButtonStyle().length() > 0) {
												cell.put("buttonstyle", button.getButtonStyle());
											}
											if (null != button.getButtonOperationCode() && button.getButtonOperationCode().length() > 0) {
												cell.put("buttonstyle", button.getButtonOperationCode());
											}
											if (null != button.getIsUseMore()) {
												cell.put("useInMore", button.getIsUseMore());
											}
											if (null != button.getOperateType()) {
												cell.put("operatetype", button.getOperateType());
											}
											if (null != button.getViewSelect() && button.getViewSelect().getCode().length() > 0) {
												cell.put("viewselect", button.getViewSelect().getCode());
											}
											if (null != button.getIsCallback()) {
												cell.put("iscallback", button.getIsCallback());
											}
											if (null != button.getReleaseFelid()) {
												cell.put("releaseFelid", button.getReleaseFelid());
											}
											if (null != button.getIsCustomFunc()) {
												cell.put("iscustomfunc", button.getIsCustomFunc());
											}
											if (null != button.getIsPermission()) {
												cell.put("ispermission", button.getIsPermission());
											}
											if (null != button.getIsConfirm()) {
												cell.put("isconfirm", button.getIsConfirm());
											}
											if (null != button.getOperateUrl() && button.getOperateUrl().length() > 0) {
												cell.put("operateurl", button.getOperateUrl());
											}
											if (null != button.getConfirmContent() && button.getConfirmContent().length() > 0) {
												cell.put("confirmcontent", button.getConfirmContent());
											}
											if (null != button.getScriptCode() && button.getScriptCode().length() > 0) {
												cell.put("scriptCode", button.getScriptCode());
											}
											if (null != button.getIsHide()) {
												cell.put("ishide", button.getIsHide());
											}
											if (null != button.getPermissionCode()) {
												cell.put("permissionCode", button.getPermissionCode());
											}
											if (null != button.getButtonAlign()) {
												cell.put("buttonAlign", button.getButtonAlign());
											}
											if (null != button.getIsPublished()) {
												cell.put("isPublished", button.getIsPublished());
											}
											break;
										}
									}
	...
	private void ecCellConfigBuilder(List<Map<String, Object>> cells, List<Field> fields, List<Button> buttons, RegionType regionType) {
		...
		for (Map<String, Object> cell : cells) {
			...
			if (cell.get("cellCode") != null && cell.get("cellCode").toString().length() > 0) {
				...
				if (null != fields && !fields.isEmpty()) {
					for (Field field : fields) {
						if (field.getCellCode() != null && field.getCellCode().equals(cellCode)) {
							fieldExist = true;
							String fieldConfig = field.getConfig();
							if (regionType != RegionType.LISTPT) {
								// 字段事件
								Set<Event> eventSet = field.getEvents();
								ecCellEventBuilder(cell, eventSet);
							}

							if (regionType == RegionType.EDIT || regionType == RegionType.DATAGRID) {
								// 字段验证
								Set<Validate> validateSet = field.getValidates();
								ecCellValidateBuilder(cell, validateSet);
							}

							// 添加字段信息
							if (regionType != RegionType.BUTTON) {
								if (fieldConfig != null && fieldConfig.length() > 0) {
									Map fieldMap = (Map) SerializeUitls.deserialize(fieldConfig);
									if (fieldMap != null && !fieldMap.isEmpty()) {
										Map<String, Object> attrMap = (Map<String, Object>) fieldMap.get("field");
										if (attrMap != null && !attrMap.isEmpty()) {
											attrMap.remove("events");
											attrMap.remove("validates");
											attrMap.remove("cellCode");
											attrMap.remove("regionType");
											if (field.getRegionType().equals(RegionType.EDIT)
													|| field.getRegionType().equals(RegionType.FASTQUERY)) {
												cell.put("element", attrMap);
											} else if (field.getRegionType().equals(RegionType.LISTPT)
													|| field.getRegionType().equals(RegionType.DATAGRID)
													|| regionType.equals(RegionType.MNECODE)) {
			...
			// LISTPT或者DATAGRID中字段cellCode没有或者没有对应field的cell预备删除
			if (!fieldExist && (regionType == RegionType.LISTPT || regionType == RegionType.DATAGRID)) {
	...
	public Map<String, Object> getButtonAttributeMap(Button button) {
		...
		if (null != button.getViewSelect() && button.getViewSelect().getCode().length() > 0) {
			buttonMap.put("viewselect", button.getViewSelect().getCode());
		}
		if (null != button.getIsCallback()) {
	...
	public String modifyConfigStructure(String originalConfig) {
		if (originalConfig != null && originalConfig.length() > 0) {
			...
			if (null != originalMap && !originalMap.isEmpty() && originalMap.get("layout") == null) {// 排除已处理过的视图
				...
				layout.put("layoutCode", DateUtil.getEcConfigNodeCode("layout").get("layout"));
				...
				if (originalTabs != null && !originalTabs.isEmpty()) {
					...
					for (Map<String, Object> orgTab : originalTabs) {
						...
						if (orgTab.get("name") != null) {// 页签信息
							...
							tab.put("tabCode", DateUtil.getEcConfigNodeCode("tab").get("tab"));
						}
						...
						if (null != originalSections && !originalSections.isEmpty()) {
							...
							for (Map<String, Object> orgSection : originalSections) {
								...
								section.put("sectionCode", DateUtil.getEcConfigNodeCode("section").get("section"));
										...
										if (content != null && !content.isEmpty()) {
											...
											if (null != forms && !forms.isEmpty()) {
												...
												for (Map<String, Object> form : forms) {
													...
													cell.put("cellCode", DateUtil.getEcConfigNodeCode("cell", timestamp, random_num++));
			...
			if (fields != null && !fields.isEmpty()) {
				for (Field field : fields) {
					...
					if (field.getEvents() != null && !field.getEvents().isEmpty()) {
						fieldEventMap.put(field.getCellCode(), field.getEvents());
					}
					if (field.getValidates() != null && !field.getValidates().isEmpty()) {
						validateMap.put(field.getCellCode(), field.getValidates());
					}
					sb.append("</list-item>");
				}
			}
			if (buttons != null && !buttons.isEmpty()) {
				for (Button button : buttons) {
					...
					if (button.getEvents() != null && !button.getEvents().isEmpty()) {
						buttonEventMap.put(button.getCellCode(), button.getEvents());
					}
					sb.append("<list-item>");
					...
					if (null != button.getViewSelect() && button.getViewSelect().getCode().length() > 0) {
						sb.append("<viewselect><![CDATA[" + button.getViewSelect().getCode() + "]]></viewselect>");
					}
					if(null != button.getOperateType() && button.getOperateType().name().equals("REF")){
						for (Field f : fields) {
							if(f.getCode().contains(button.getReleaseFelid()!=null?button.getReleaseFelid():"")&& (f.getShowFormat() != null && ShowFormat.SELECTCOMP.name().equals(f.getShowFormat().name()))){
								sb.append("<objName><![CDATA[" + f.getDataGrid().getName()+f.getKey() + "]]></objName>");
							}
						}
						sb.append("<viewurl><![CDATA[" + button.getViewSelect().getUrl() + "]]></viewurl>");
						sb.append("<viewname><![CDATA[" + button.getViewSelect().getTitle() + "]]></viewname>");
					}
					if (null != button.getIsCallback()) {
	...
	public static void main(String[] args) throws IOException {
		...
		reader.close();
		String config = modifyConfigStructure(sb.toString());
		System.out.println("full:" + config);
		Map<String, Object> map = ecSplitConfig(config);
