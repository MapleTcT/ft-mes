...
public class AnnoFilter implements Filter {
                ...
                for (Field field : fields) {
                    try {
                        ...
                        if (null != fieldSystemConfigAnno) {
                            //识别注解值为开头为 % 的标识
                            String value = fieldSystemConfigAnno.value();
                            if (!value.startsWith("$") || !value.contains(":") || !value.contains(CONFIG_SPLIT)) {
                                continue;
                            }
                            ...
                            //String initValue = originValue.substring(2, originValue.length() - 1);
                         ...
                         /*   if (!initValue.contains(CONFIG_SPLIT)) {
                                continue;
                            }*/
                        ...
                        log.error("注解过滤器解析字段发生错误，{}", e.getMessage());
                    }
                }
            }
        } catch (Exception e) {
            log.error("注解过滤器发生错误，{}", e.getMessage());
        } finally {
    ...
    private void setTargetFieldValue(Object bean, Field field, Object value) {
        try {
            ...
            /*try {
                Field CGLIB_CALLBACK = bean.getClass().getDeclaredField("CGLIB$CALLBACK_0");
                CGLIB_CALLBACK.setAccessible(true);
                Object dynamicAdvisedInterceptor = CGLIB_CALLBACK.get(bean);
                ...
                log.debug("过滤器 NoSuchFieldException");
            }*/
