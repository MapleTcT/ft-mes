...
public class BAPUrlBase64 {
	...
	private static byte[] decode(byte[] source, int off, int len, int options) {
		...
		for (i = off; i < off + len; i++) { // Loop through source
			...
			if (sbiDecode >= WHITE_SPACE_ENC) {
				...
				throw new IllegalArgumentException(String.format("Bad Base64 input character '%c' in array position %d", source[i],
						i));
			}
		}
	...
	public static void main(String[] args) {
		String pcStr = "1==";
		String _pc = new String(encode(pcStr.getBytes()));
		System.out.println(_pc);
		String pc = new String(decode(_pc.getBytes()));
