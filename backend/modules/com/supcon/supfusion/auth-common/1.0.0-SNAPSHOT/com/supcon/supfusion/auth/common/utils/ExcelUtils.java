...
import com.supcon.supfusion.auth.common.model.ExcelTitle;
...
public class ExcelUtils {
    ...
    public static final String USER_FILE = "用户模板";
    public static final String EXPLIAN = "说明";

    public static final List<String> DEPARTMENT_TEMPLATE_EXPLAIN = new ArrayList<String>();

    public static final Set<String> DEPARTMENT_IMPORT_HEAD = new HashSet<>();
    ...
    static {
        //部门导入模板说明
        DEPARTMENT_TEMPLATE_EXPLAIN.add("用户名:必填项，用于登录supOS的用户名，可以是字母、数字和下划线（_）组合而成，且在整个平台内唯一");
        DEPARTMENT_TEMPLATE_EXPLAIN.add("人员密码:必填项，是指该用户对应的登录密码，此处填写录入在系统中的密码");
        DEPARTMENT_TEMPLATE_EXPLAIN.add("人员姓名:必填项，是指该用户对应的实际人员，此处填写录入在系统中的姓名");
        DEPARTMENT_TEMPLATE_EXPLAIN.add("人员编码:必填项，是指该用户对应的实际人员的编码，在整个平台内唯一");
        DEPARTMENT_TEMPLATE_EXPLAIN.add("用户描述：非必填项，用于对该用户进行描述");

        DEPARTMENT_IMPORT_HEAD.add("*用户名称");
        DEPARTMENT_IMPORT_HEAD.add("*用户密码");
        DEPARTMENT_IMPORT_HEAD.add("*人员名称");
        DEPARTMENT_IMPORT_HEAD.add("*人员编号");
        DEPARTMENT_IMPORT_HEAD.add("用户描述");
    ...
    public static void createExplainSheet(List<String> explainMessages, Workbook workbook) {
        ...
        for (int i = 0; i < explainMessages.size(); i++) {
            ...
            cell.setCellValue(i + "、" + explainMessages.get(i));
        }
    }
    ...
    public static void createFolder(String folderPath) {
        ...
        if (!dir.exists()) {
            ...
            if (!mkFlag) {
                throw new UserException(UserErrorEnum.EXCEL_FILE_CREATE_ERROR);
            }
        }
    }
