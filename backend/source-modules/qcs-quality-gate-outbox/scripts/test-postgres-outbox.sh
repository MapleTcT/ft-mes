...
CONTAINER="ft-mes-qcs-gate-outbox-test-$$"
...
docker exec -i "$CONTAINER" psql -v ON_ERROR_STOP=1 -U adp_test -d adp_test \
    < "$MODULE_DIR/src/test/resources/qcs-quality-gate-outbox-test-schema.sql"
docker exec -i "$CONTAINER" psql -v ON_ERROR_STOP=1 -U adp_test -d adp_test \
    < "$ROOT_DIR/deploy/docker/postgres/init/176-wom-bpi-production-context-outbox.sql"
docker exec -i "$CONTAINER" psql -v ON_ERROR_STOP=1 -U adp_test -d adp_test \
    < "$ROOT_DIR/deploy/docker/postgres/init/193-qcs-bpi-quality-gate-outbox.sql"
docker exec -i "$CONTAINER" psql -v ON_ERROR_STOP=1 -U adp_test -d adp_test \
    < "$MODULE_DIR/src/test/resources/qcs-quality-gate-outbox-postgres-acceptance.sql"
