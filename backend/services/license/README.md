# license

- 原始 JAR: `bap-server/base-Server/license/supfusion-license.jar`
- Main-Class: `org.springframework.boot.loader.JarLauncher`
- Start-Class: `com.supcon.supfusion.license.bootstrap.LicenseApplication`
- Spring Boot: `2.1.5.RELEASE`
- 嵌入依赖数量: `215`

## 配置入口
- JAR 内: `BOOT-INF/classes/application.yml`
- JAR 内: `META-INF/maven/com.supcon.supfusion/license-bootstrap/pom.properties`
- JAR 内: `BOOT-INF/classes/application-dev.yml`
- JAR 内: `BOOT-INF/classes/bootstrap.yml`

## Nacos Data IDs
- `supfusion-datasource-system.properties`
- `supfusion-jwt-common.properties`
- `supfusion-license.properties`
- `supfusion-mybatis-common.properties`
- `supfusion-redis-system.properties`
- `supfusion-registry.properties`

## 端口和应用名
- `supfusion-license.properties`: `30035`
- `supfusion-license.properties`: `forceChange:supplant-license`
