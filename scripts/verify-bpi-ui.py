...
def main() -> int:
    ...
    if not failures:
        ...
        for required in ("console", "pageerror", "requestfailed", "/tmp/bpi-console-desktop.png", "/tmp/bpi-console-candidate-rejected.png", "/tmp/bpi-console-batch-lifecycle.png", "/tmp/bpi-console-end-boundary.png", "/tmp/bpi-console-rule-published.png", "/tmp/bpi-console-rule-application-applied.png", "CLOSED_RAW", "END_BOUNDARY_CONFIRMED", "rule-runtime-readiness", "DEGRADED", "runtimeReadinessStatus", "document.documentElement.scrollWidth"):
        ...
        if browser_tests < 8 or summary.get("pass") != browser_tests or summary.get("fail") != 0:
            failures.append("BPI UI acceptance must record at least eight browser tests with every test passing")
