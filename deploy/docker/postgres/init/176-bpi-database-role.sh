...
psql --set ON_ERROR_STOP=1 \
  ...
  --set catalog_publisher_password="$BPI_CATALOG_PUBLISHER_DATABASE_PASSWORD" <<'SQL'
...
\gexec
SELECT format('GRANT CONNECT ON DATABASE %I TO bpi_migrator, bpi_service, bpi_materializer, bpi_catalog_publisher', :'bpi_database')
\gexec
SQL
...
GRANT USAGE ON SCHEMA public TO bpi_migrator, bpi_service, bpi_materializer, bpi_catalog_publisher;
SQL
