var loadFlag=false;
//资质参照
function companyCertAdd(data,companyId) {
	var certCompanyDg = ReactAPI.getComponentAPI("SupDataGrid").APIs("Qualify_1.0.0_companyCertificate_companyCertEditdg1575277481175");
	var pDataObjs = certCompanyDg.getDatagridData();//编辑页面表体数据
	...
	for(var i=0; i<data.length; i++) {
		...
		if(pDataObjs.length > 0) {
			for(var lRows=0; lRows<pDataObjs.length; lRows++) {
				var levelId = certCompanyDg.getValueByKey(lRows,'certLvId.id');
		...
		if(!flag) {
			var lastRow = certCompanyDg.addLine().rowIndex;//添加行，获取行索引
			//资质
			var certId = {
			...
			var date = new Date();
			//设置行数据
			certCompanyDg.setRowData(lastRow,{
			...
			//有效期提醒日期
			certCompanyDg.setDatagridCellAttr(lastRow, "vremindDate", { readonly : true } );
			//复审提醒日期
			certCompanyDg.setDatagridCellAttr(lastRow, "rremindDate", { readonly : true } );
		}
	}
