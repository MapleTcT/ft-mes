...
var ticketType = new Map([["WTS_workType/fireWork", "动火安全作业"], ["WTS_workType/limitSpaceWork", "受限空间安全作业"], ["WTS_workType/blockWork", "盲板抽堵安全作业"], ["WTS_workType/heightWork", "高处安全作业"], ["WTS_workType/liftWork", "吊装安全作业"], ["WTS_workType/electricityWork", "临时用电安全作业"], ["WTS_workType/soilWork", "动土安全作业"], ["WTS_workType/breakWork", "断路安全作业"]]);

var LvGroup = new Map([["fireWork", [{ key: "workAppointment_ticketLv/fireLv1", value: "一级动火" }, { key: "workAppointment_ticketLv/fireLv2", value: "二级动火" }, { key: "workAppointment_ticketLv/fireLvS", value: "特级动火" }]], ["heightWork", [{ key: "workAppointment_ticketLv/highLv1", value: "一级高处" }, { key: "workAppointment_ticketLv/highLv2", value: "二级高处" }, { key: "workAppointment_ticketLv/highLv3", value: "三级高处" }, { key: "workAppointment_ticketLv/highLvS", value: "特级高处" }]], ["liftWork", [{ key: "workAppointment_ticketLv/liftLv1", value: "一级吊装" }, { key: "workAppointment_ticketLv/liftLv2", value: "二级吊装" }, { key: "workAppointment_ticketLv/liftLv3", value: "三级吊装" }]], ["other", [{ key: "workAppointment_ticketLv/noneLv", value: "无" }]]]);
...
function onChangeTicket(value) {
    ...
    if (value) {
        ...
        if (CurrentTicketsApi.getDatagridData().filter(function (element) {
            return element.rowIndex != thisRow && element.workType != null;
        }).map(function (o) {
            return o.workType.id;
        }).includes(value)) {
            ReactAPI.showMessage("w", "作业类型不允许重复！");
            setTimeout(function () {
                CurrentTicketsApi.setValueByKey(thisRow, "workType", null);
            });
            return false;
        }
...
function setLvOptions(ticketType, rowIndex) {
    ...
    if (thisOptions[0].key == 'workAppointment_ticketLv/noneLv') {
        //无--->直接赋值
    ...
    } else if (currentValue && !thisOptions.includes(currentValue.id)) {
        //如果当前级不在范围内

        ReactAPI.getComponentAPI("SupDataGrid").APIs(CurrentTicketsKey).setValueByKey(rowIndex, "ticketLv", null);
    }
    //设置候选值
    ReactAPI.getComponentAPI("SupDataGrid").APIs(CurrentTicketsKey).setCellOptions(rowIndex, "ticketLv", thisOptions);
}
...
function actionCustomCheck() {
    ...
    if (tickets.length == 0) {
        ...
        var errorTickets = tickets.filter(function (o) {
        ...
        if (errorTickets.length > 0) {
            ...
            errorMsg += "第" + errorTickets.map(function (o) {
                return o.rowIndex + 1 + "";
            }).join(",") + "行作业票信息内外部作业单位至少填写一个！</br>";
        }
    }
