...
TARGET_VIEW_CODES: Sequence[str] = (
    ...
    "HierarchicalMod_1.0.0_factoryWare_factoryRepairList",
    ...
    "WOM_1.0.0_prepareMaterialNeed_demandBreakdownList",
    ...
    "WOM_1.0.0_prePraOrder_preOrderScheduleList",
    "WOM_1.0.0_prePraOrder_preOrderDepartList",
    "WOM_1.0.0_prePraOrder_recordingList",
    "WOM_1.0.0_prePraOrder_recordingReceiveList",
    ...
    "WOM_1.0.0_batchMaterialNeed_batchMaterialNeed",
    "WOM_1.0.0_batchMaterialNeed_makeBatOrdEdit",
    "WOM_1.0.0_batchMaterial_batchMaterilList",
...
# The recovered WOM package contains the preparation and batching scripts, but
# all of their operator buttons were exported as unpublished. Publish only the
# original business actions needed for the material handoff workflow. Summary
# and detail ledgers intentionally remain query-only, and technical recovery
# actions such as "secondSyn" stay hidden.
WOM_MATERIAL_PREPARATION_PUBLISHED_ACTIONS: Dict[str, Tuple[str, ...]] = {
    "HierarchicalMod_1.0.0_factoryWare_factoryRepairList": ("addRepir",),
    "WOM_1.0.0_prepareMaterialNeed_demandBreakdownList": (
        "instruction",
        "complete",
        "adjustment",
    ...
    "WOM_1.0.0_prePraOrder_preOrderDepartList": ("prePare",),
    "WOM_1.0.0_prePraOrder_recordingList": (
        "submit",
        "recall",
        "deliver",
    ),
    "WOM_1.0.0_prePraOrder_recordingReceiveList": ("receive",),
    "WOM_1.0.0_batchMaterialNeed_batchMaterialNeed": (
        "addBatchMaterial",
        "complete",
    ),
...
WOM_BATCH_MATERIAL_LIST_VIEW_CODE = (
    "WOM_1.0.0_batchMaterial_batchMaterilList"
)
WOM_BATCH_MATERIAL_LIST_GRID_CODE = (
    "WOM_1.0.0_batchMaterial_batchMaterilList_batchMateril_sdg"
)

# The consolidated Sample Management navigation still uses the PostgreSQL-safe
# compatibility layouts generated by this script.  The original packaged
# layouts carry the authoritative button scripts, but the compatibility views
# were emitted with empty button arrays.  Keep the target list explicit so a
# hidden or unpublished laboratory action is never exposed by accident.
LIMS_SAMPLE_ACTION_SPECS: Dict[str, Dict[str, Any]] = {
    "LIMSSample_5.0.0.0_sample_sampleRegisterLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_sampleRegisterLayout",
        "parent_qualified": True,
        "button_ids": ("register", "setTestItem", "cancel"),
    },
    "LIMSBasic_1.0.0_testPlan_planSetSampleList": {
        "source_view": "LIMSBasic_1.0.0_testPlan_planSetSampleList",
        "parent_qualified": False,
        "button_ids": ("createSample",),
    },
    "LIMSSample_5.0.0.0_sample_sampTaskAlcatLayOut": {
        "source_view": "LIMSSample_5.0.0.0_sample_sampTaskAlcatLayOut",
        "parent_qualified": True,
        "button_ids": ("taskAlcat",),
    },
    "LIMSSample_5.0.0.0_sample_collectListLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_collectListLayout",
        "parent_qualified": True,
        "button_ids": ("collect", "collectInfoEdit"),
    },
    "LIMSSample_5.0.0.0_sample_receiveListLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_receiveListLayout",
        "parent_qualified": True,
        "button_ids": ("receive", "setTestItem"),
    },
    "LIMSSample_5.0.0.0_sample_sampleSweepReceive": {
        "source_view": "LIMSSample_5.0.0.0_sample_sampleSweepReceive",
        "parent_qualified": False,
        "button_ids": ("receiveSubmit", "del"),
    },
    "LIMSSample_5.0.0.0_sample_makeListLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_makeListLayout",
        "parent_qualified": True,
        "button_ids": ("make",),
    },
    "LIMSSample_5.0.0.0_sample_handoverListLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_handoverListLayout",
        "parent_qualified": True,
        "button_ids": ("handover",),
    },
    "LIMSSample_5.0.0.0_sample_recordBySingleSample": {
        "source_view": "LIMSSample_5.0.0.0_sample_recordBySingleSample",
        "parent_qualified": False,
        "button_ids": ("devRecord",),
    },
    "LIMSSample_5.0.0.0_sample_recordBySample": {
        "source_view": "LIMSSample_5.0.0.0_sample_recordBySample",
        "parent_qualified": False,
        "button_ids": ("devRecord",),
    },
    "LIMSSample_5.0.0.0_sample_recordCheckByTest": {
        "source_view": "LIMSSample_5.0.0.0_sample_recordCheckByTest",
        "parent_qualified": False,
        "button_ids": ("refresh",),
    },
    "LIMSSample_5.0.0.0_sample_sampleCheck": {
        "source_view": "LIMSSample_5.0.0.0_sample_sampleCheck",
        "parent_qualified": False,
        "button_ids": ("viewSample",),
    },
    "LIMSSample_5.0.0.0_sample_sampleDealListLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_sampleDealListLayout",
        "parent_qualified": True,
        "button_ids": (
            "sampleInfoModify",
            "setTestItem",
    ...
    "LIMSSample_5.0.0.0_sample_retainListLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_retainListLayout",
        "parent_qualified": True,
        "button_ids": ("retain",),
    },
    "LIMSSample_5.0.0.0_sample_sampleInfoLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_sampleInfoLayout",
        "parent_qualified": True,
        "button_ids": ("sampleInfoCheck", "dealInfoCheck"),
    },
    "LIMSSample_5.0.0.0_sample_remainSampleLayout": {
        "source_view": "LIMSSample_5.0.0.0_sample_remainSamplePart",
        "parent_qualified": True,
        "button_ids": ("back", "destroy"),
        "button_overrides": {
            "back": {
                "label": "归还",
                "buttonoperationcode": (
                    "remainSamplePart_recover_recover_"
                    "LIMSSample_5.0.0.0_sample_remainSamplePart"
                ),
            },
            "destroy": {
                "label": "销毁",
                "buttonoperationcode": (
                    "remainSamplePart_delete_del_"
                    "LIMSSample_5.0.0.0_sample_remainSamplePart"
                ),
            },
        },
...
class ViewDef:
    ...
    dependency_model_codes: Tuple[str, ...] = ()
...
def normalize_packaged_runtime_payload(value: Any) -> Any:
    ...
    def visit(node: Any) -> None:
        ...
        if operation_code:
            ...
            if onclick and button_funcname_requires_repair(raw_funcname):
...
def canonical_button_onclick(funcname: str) -> str:
    ...
    malformed_function = re.fullmatch(
        r"(?:function|nction|ction)\s+"
        r"([A-Za-z_$][A-Za-z0-9_$]*\s*\([^)]*\))\s*;?",
        value,
    )
    if malformed_function:
        return malformed_function.group(1).strip()
    return value


def button_funcname_requires_repair(funcname: str) -> bool:
    return bool(
        re.search(
            r"(?:^|['\"])(?:function|nction|ction)\s+",
            (funcname or "").strip(),
...
def button_payload(button_item: ET.Element, views: Optional[Dict[str, "ViewDef"]] = None) -> Optional[Dict[str, Any]]:
    ...
    if onclick:
        if button_funcname_requires_repair(str(result.get("funcname") or "")):
...
def extract_view_model_dependencies(
    view_element: ET.Element, entity_code: str
) -> Tuple[str, ...]:
    """Collect child-table models used by embedded edit datagrids.

    Legacy BAP edit views keep their main model in ``assModel`` while editable
    table bodies point at part models through ``modelcode`` and
    ``targetPropertyCode``. Those part models need their own PostgreSQL tables.
    """
    ...
    def add(candidate: str) -> None:
        ...
        if not candidate or candidate in seen:
            return
        if entity_code and not candidate.startswith(entity_code + "_"):
            return
        seen.add(candidate)
        dependencies.append(candidate)

    for node in view_element.iter():
        if node.tag in {"modelCode", "modelcode"}:
            add(node.text or "")
            continue
        if node.tag not in {"targetPropertyCode", "propertyCode"}:
            continue
        property_code = (node.text or "").split("||", 1)[0].strip()
        if "_" in property_code:
            add(property_code.rsplit("_", 1)[0])
...
def parse_view(source: Path, view_element: ET.Element) -> Optional[ViewDef]:
    ...
    return ViewDef(
        ...
        dependency_model_codes=extract_view_model_dependencies(
            view_element, entity_code
        ),
    )
...
def iter_layout_datagrids(payload: Any) -> List[Dict[str, Any]]:
...
def collect_runtime_buttons(payload: Any) -> List[Dict[str, Any]]:
    buttons: List[Dict[str, Any]] = []
    ...
    def visit(node: Any) -> None:
        ...
        if not isinstance(node, dict):
            return
        nested_buttons = node.get("buttons")
        if isinstance(nested_buttons, list):
            buttons.extend(
                button for button in nested_buttons if isinstance(button, dict)
            )
        for child in node.values():
            visit(child)
...
def apply_lims_sample_action_buttons(
    view: ViewDef,
    payload: Any,
    views: Dict[str, ViewDef],
    packaged_view_json: Optional[Dict[str, Dict[str, Any]]] = None,
) -> None:
    spec = LIMS_SAMPLE_ACTION_SPECS.get(view.code)
    if not spec:
        return

    target = find_layout_datagrid(view, payload)
    if target is None:
        raise RuntimeError(f"LIMS sample action grid not found: {view.code}")

    source_view_code = str(spec["source_view"])
    source_buttons: List[Dict[str, Any]] = []
    packaged_source = (packaged_view_json or {}).get(source_view_code)
    if packaged_source is not None:
        source_buttons.extend(
            collect_runtime_buttons(normalize_packaged_runtime_payload(packaged_source))
        )

    source_view = views.get(source_view_code)
    if source_view is not None:
        packaged_ids = {
            str(button.get("id") or "").strip() for button in source_buttons
        }
        source_buttons.extend(
            button
            for button in extract_buttons(source_view, views)
            if str(button.get("id") or "").strip() not in packaged_ids
        )

    source_by_id: Dict[str, Dict[str, Any]] = {}
    for button in source_buttons:
        button_id = str(button.get("id") or "").strip()
        if button_id and button_id not in source_by_id:
            source_by_id[button_id] = button

    restored_buttons: List[Dict[str, Any]] = []
    overrides = spec.get("button_overrides") or {}
    for button_id in spec["button_ids"]:
        source_button = source_by_id.get(str(button_id))
        if source_button is None:
            raise RuntimeError(
                f"LIMS sample action button not found: {source_view_code}#{button_id}"
            )

        button = copy.deepcopy(source_button)
        button_override = overrides.get(button_id) or {}
        label = str(
            button_override.get("label")
            or button.get("showname")
            or button.get("name")
            or button.get("namekey")
            or button_id
        ).strip()
        original_namekey = str(
            button.get("i18nKey") or button.get("namekey") or ""
        ).strip()
        if original_namekey and original_namekey != label:
            button["i18nKey"] = original_namekey
        for key in ("name", "showname", "namekey", "NAME"):
            button[key] = label

        relative_operation_code = str(
            button_override.get("buttonoperationcode")
            or button.get("buttonoperationcode")
            or ""
        ).strip()
        if not relative_operation_code:
            raise RuntimeError(
                f"LIMS sample action operation code not found: {source_view_code}#{button_id}"
            )
        permission_operation_code = relative_operation_code
        if bool(spec.get("parent_qualified")):
            permission_operation_code = f"{view.code}_{relative_operation_code}"

        button["buttonoperationcode"] = relative_operation_code
        button["CODE"] = relative_operation_code
        button["pc"] = button_power_code(permission_operation_code)
        button["isPublished"] = True
        button["ispermission"] = True
        button["isHide"] = False

        raw_funcname = str(button.get("funcname") or button.get("onclick") or "")
        onclick = canonical_button_onclick(raw_funcname)
        if onclick:
            button["funcname"] = f"onclick='{onclick}'"
            button["onclick"] = onclick
            button["ONCLICK"] = onclick

        button_style = default_button_style(button)
        button["buttonstyle"] = button_style
        button["ICONCLS"] = "cui-btn-" + button_style
        button["USEINMORE"] = bool_text(
            str(
                button.get("useInMore")
                if button.get("useInMore") is not None
                else button.get("USEINMORE") or "false"
            )
        )
        button["SEPARATENUM"] = "0"
        normalize_xml_scalar_types(button)
        restored_buttons.append(button)

    target["buttons"] = restored_buttons
...
def publish_wom_material_preparation_actions(
    view: ViewDef,
    payload: Any,
    views: Optional[Dict[str, ViewDef]] = None,
) -> None:
    allowed_ids = WOM_MATERIAL_PREPARATION_PUBLISHED_ACTIONS.get(view.code)
    if not allowed_ids:
        return

    target = find_layout_datagrid(view, payload)
    if target is None:
        raise RuntimeError(f"WOM material preparation action grid not found: {view.code}")
    ...
    buttons_by_id = {
        str(button.get("id") or "").strip(): button
        for button in existing_buttons
        if isinstance(button, dict) and str(button.get("id") or "").strip()
    }
    source_buttons = {
        str(button.get("id") or "").strip(): button
        for button in extract_buttons(view, views)
    ...
    for button in existing_buttons:
        if not isinstance(button, dict):
            continue
        button_id = str(button.get("id") or "").strip()
        if button_id and button_id not in allowed_id_set:
            button["isPublished"] = False
            button["isHide"] = True

    for button_id in allowed_ids:
        button = buttons_by_id.get(button_id)
        if button is None:
            source_button = source_buttons.get(button_id)
            if source_button is None:
                raise RuntimeError(
                    f"WOM material preparation action not found: {view.code}#{button_id}"
                )
            button = copy.deepcopy(source_button)
        ...
        button["isPublished"] = True
        button["isHide"] = False
        # These package actions were designed without a separate button-level
        # permission registration. Menu access and backend authorization still
        # apply; keeping the source contract avoids an admin-only UI regression.
        button["ispermission"] = False

        onclick = canonical_button_onclick(
            str(button.get("funcname") or button.get("onclick") or "")
        )
        if onclick:
            button["funcname"] = f"onclick='{onclick}'"
            button["onclick"] = onclick
    ...
    target["buttons"] = existing_buttons


def repair_wom_batch_material_render_over(view: ViewDef, payload: Any) -> None:
    if view.code != WOM_BATCH_MATERIAL_LIST_VIEW_CODE:
        return

    target = find_layout_datagrid(view, payload)
    if target is None:
        raise RuntimeError(f"WOM batch material grid not found: {view.code}")
    ...
    for key in ("renderOver", "renderOver_es5"):
        script = str(target.get(key) or "")
        if not script:
            continue
        script = script.replace(unsafe_probe + ";", "").replace(unsafe_probe, "")
        for declaration in (
            f"let data = {data_expression};",
            f"var data = {data_expression};",
        ):
            guard = declaration + "\n    if (!data || !data.length) { return; }"
            if declaration in script and guard not in script:
                script = script.replace(declaration, guard, 1)
...
def repair_wom_material_preparation_action_scripts(view: ViewDef, payload: Any) -> None:
    if view.code == "WOM_1.0.0_batchMaterialNeed_makeBatOrdEdit":
        repair_wom_make_batch_order_dialog(payload)
        return
    if view.code != "WOM_1.0.0_batchMaterialNeed_batchMaterialNeed":
        return

    target = find_layout_datagrid(view, payload)
    if target is None:
        raise RuntimeError(f"WOM batch material need grid not found: {view.code}")

    action_ids = set(WOM_MATERIAL_PREPARATION_PUBLISHED_ACTIONS[view.code])
    for button in target.get("buttons") or []:
        if not isinstance(button, dict) or button.get("id") not in action_ids:
            continue
        for key in ("funcbody", "funcbody_es5"):
            script = str(button.get(key) or "")
            if not script:
                continue
            script = script.replace(
                "selectedData[i].needState.id",
                "(selectedData[i].needState && selectedData[i].needState.id)",
            )
            if button.get("id") == "addBatchMaterial":
                validation_marker = "//选了多条数据，有"
                validation = """// 拒料测试记录等不完整数据不能转成正常生产配料指令。
    for (var invalidIndex = 0; invalidIndex < selectedData.length; invalidIndex++) {
        var invalidRow = selectedData[invalidIndex];
        if (!invalidRow.needState || !invalidRow.needState.id ||
            !invalidRow.batchSite || !invalidRow.batchSite.id ||
            !invalidRow.materialId || !invalidRow.materialId.id) {
            ReactAPI.showMessage("w", "当前记录不是有效的生产配料需求，请从制造指令生成配料需求");
            return false;
        }
    }
    """
                if validation_marker in script and validation not in script:
                    script = script.replace(
                        validation_marker,
                ...
                script = script.replace(
                    'ReactAPI.international.getText("WOM.custom.randon1596012314973")',
                    '"创建配料指令"',
                ).replace(
                    'ReactAPI.international.getText("Button.text.confirm")',
                    '"确认"',
                ).replace(
                    'ReactAPI.international.getText("Button.text.close")',
                    '"关闭"',
                )
            elif button.get("id") == "complete":
                validation_marker = "//如果全部已更新无需再操作"
                validation = """for (var invalidIndex = 0; invalidIndex < selectedData.length; invalidIndex++) {
        if (!selectedData[invalidIndex].needState || !selectedData[invalidIndex].needState.id) {
            ReactAPI.showMessage("w", "当前记录缺少配料需求状态，不能完成");
            return false;
        }
    }
    """
...
def repair_wom_make_batch_order_dialog(payload: Any) -> None:
    target = None
    for candidate in iter_layout_datagrids(payload):
        if candidate.get("DataGridCode") == (
            "WOM_1.0.0_batchMaterialNeed_makeBatOrdEditdg1582856369922"
        ):
            target = candidate
            break
    if target is None:
        raise RuntimeError("WOM make-batch-order dialog grid not found")

    target["modelCode"] = "WOM_1.0.0_batchMaterialNeed_MakeBatOrdPart"
    for button in target.get("buttons") or []:
        if not isinstance(button, dict) or button.get("id") not in {"copy", "delete"}:
            continue
        button["isPublished"] = True
        button["isHide"] = False
        button["ispermission"] = False
        for key in ("funcbody", "funcbody_es5"):
            script = str(button.get(key) or "")
            script = script.replace(
                "editTable.getDatagridData()[newline.rowIndex].batchSite.id",
                "(editTable.getDatagridData()[newline.rowIndex].batchSite || {}).id",
            ).replace(
                "if (res.code = 200)",
                "if (res && Number(res.code) === 200 && Array.isArray(res.data))",
    ...
    for key in ("renderOver", "renderOver_es5"):
        ...
        if not script:
            continue
        script = re.sub(
            r"\s*var offerNum = \"\";\s*if \(\(?planNum - offerNum\)? > 0\) \{\s*offerNum = planNum = offerNum;\s*\}",
            "\n    offerNum = Math.max(Number(planNum) - Number(offerNum), 0);",
            script,
        )
        script = script.replace(
            "batchNum = res.data;",
            "batchNum = res && res.data != null ? res.data : offerNum;",
        )
        script = script.replace(
            "var batchSite = dataGrid[j].batchSite.id;",
            "var batchSite = dataGrid[j].batchSite && dataGrid[j].batchSite.id;\n"
            "    if (!batchSite) { continue; }",
        )
        script = script.replace(
            "if (res.code = 200)",
            "if (res && Number(res.code) === 200 && Array.isArray(res.data))",
...
def view_json(
    ...
    publish_wom_material_preparation_actions(view, payload, views)
    repair_wom_batch_material_render_over(view, payload)
    repair_wom_material_preparation_action_scripts(view, payload)
    apply_lims_sample_action_buttons(
        view,
        payload,
        views,
        packaged_view_json,
...
def resolve_required_models(models: Dict[str, ModelDef], views: Sequence[ViewDef]) -> List[ModelDef]:
    ...
    for view in views:
        ...
        candidate_codes.extend(getattr(view, "dependency_model_codes", ()))
