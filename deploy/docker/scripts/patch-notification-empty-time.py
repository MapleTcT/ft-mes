...
PATCH_ROOT = DOCKER_DIR / "patches" / "notification-empty-time"
SOURCE_FILE = (
    PATCH_ROOT
    / "src"
    / "com"
    ...
    / "webapi"
    / "controller"
    / "NotcieTaskController.java"
)
CLASSES_DIR = PATCH_ROOT / "classes"
CLASS_FILE = CLASSES_DIR / "com/supcon/supfusion/notification/admin/webapi/controller/NotcieTaskController.class"
TARGET_CLASS = "com/supcon/supfusion/notification/admin/webapi/controller/NotcieTaskController.class"
NOTIFICATION_WEBAPI_JAR = "BOOT-INF/lib/admin-webapi-1.0.0-SNAPSHOT.jar"
...
def service_jar(runtime_root: Path) -> Path:
    service_dir = runtime_root / "base-Server" / "notification-admin"
...
def extract_nested_libs(jar_path: Path, target_dir: Path) -> list[Path]:
    ...
    with zipfile.ZipFile(jar_path, "r") as app_jar:
        for info in app_jar.infolist():
            ...
            libs.append(out)
    if not any(path.name == Path(NOTIFICATION_WEBAPI_JAR).name for path in libs):
        raise FileNotFoundError(f"{jar_path} does not contain {NOTIFICATION_WEBAPI_JAR}")
    return libs
...
def build_class(jar_path: Path, force: bool) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="notification-libs-") as tmp:
...
def replace_class_in_nested_jar(nested_bytes: bytes, class_bytes: bytes) -> tuple[bytes, bool]:
    replaced = False
    with tempfile.TemporaryDirectory(prefix="notification-nested-") as tmp:
...
def patch_service_jar(jar_path: Path, class_file: Path, backup_suffix: str) -> None:
    ...
    patched = False
    with tempfile.TemporaryDirectory(prefix="notification-service-") as tmp:
        output = Path(tmp) / jar_path.name
        with zipfile.ZipFile(jar_path, "r") as zin, zipfile.ZipFile(output, "w") as zout:
            for info in zin.infolist():
                data = zin.read(info.filename)
                if info.filename == NOTIFICATION_WEBAPI_JAR:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Patch notification task controller to tolerate empty time filters."
    )
    parser.add_argument("--runtime-root", type=Path, default=runtime_root_default())
    parser.add_argument("--backup-suffix", default=".pre-notification-empty-time.bak")
    ...
    if args.restore:
        ...
        print(f"{'restored' if restored else 'missing backup for'} notification-admin: {jar_path}")
        return
    ...
    print(f"patched notification-admin: {jar_path}")
