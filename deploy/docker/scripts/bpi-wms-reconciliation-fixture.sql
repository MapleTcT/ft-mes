...
VALUES
    (:'commands_flag_id'::uuid, '1000', 'PLANT', 'PLANT-01', 'bpi.commands', true, 1,
     :'marker', true, :'marker' || ' controlled WMS reconciliation acceptance'),
...
VALUES
    (:'quality_gate_id'::uuid, '1000', :'batch_id'::uuid,
     ...
     md5(:'marker'), 'ACCEPTED', 12.345000, 't', :'marker' || '_MATERIAL',
...
INSERT INTO bpi.bpi_outbox_events
    (id, tenant_id, plant_id, line_id, aggregate_type, aggregate_id,
     ...
     attempt_count, total_attempt_count, next_attempt_at, published_at,
     revision, created_at, updated_at)
VALUES
    (:'outbox_id'::uuid, '1000', 'PLANT-01', 'LINE-S07-01',
     'BATCH_INSTANCE', :'batch_id'::uuid, 'WMS_COMPLETION_INBOUND_COMMAND',
     'bpi.wms.completion-inbound-command.v1', '1000|' || :'batch_id',
     convert_to(:'marker', 'UTF8'), jsonb_build_object('acceptanceMarker', :'marker'),
     'PUBLISHED', 1, 1, now() - interval '10 minutes', now() - interval '10 minutes',
     1, now() - interval '10 minutes', now() - interval '10 minutes');
