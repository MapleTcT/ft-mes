...
set bap_home=%~dp0..\..\
set ServerName=supfusion-redis-middleware
set server_description=»ù´¡×é¼þredis
set sys_exist_flag=no
set script_path=%bap_home%\bap-server\assembly\Redis-x64-5.0.9
set nssm_path=%~dp0\..\nssm
...
if %sys_exist_flag%==no (
	...
	%nssm_path%\nssm install %ServerName% %script_path%\startRedis.bat
