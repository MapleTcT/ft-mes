...
set bap_home=%~dp0..\..\
set ServerName=supfusion-kafka-middleware
set server_description=»ù´¡×é¼þkafka
set sys_exist_flag=no
set script_path=%bap_home%\bap-server\assembly\kafka_2
set nssm_path=%~dp0\..\nssm
...
if %sys_exist_flag%==no (
	...
	%nssm_path%\nssm install %ServerName% %script_path%\startKafka.bat
