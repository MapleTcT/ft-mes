...
set bap_home=%~dp0..\..\
set nssm_path=%~dp0\..\nssm
set ServerName=supfusion-zookeeper-middleware
set server_description=»ù´¡×é¼þzookeeper
set sys_exist_flag=no
set script_path=%bap_home%\bap-server\assembly\zookeeper\bin
...
if %sys_exist_flag%==no (
	...
	%nssm_path%\nssm install %ServerName% %script_path%\zkServer.bat
