...
def check_no_placeholders(value: Any, path: str, failures: list[str]) -> None:
    ...
    if isinstance(value, str) and value.strip() in PLACEHOLDER_VALUES:
        fail(failures, f"{path} must not be a placeholder for strict MinIO migration readiness")
...
def check_artifact_reference(phase_id: str, field: str, value: str, failures: list[str]) -> None:
    ...
    if "$" in value or "\n" in value:
        return
    if any(value.startswith(prefix) for prefix in ("make ", "mc ", "aws ", "curl ", "docker ", "sha256sum ")):
        return
    relative = Path(value)
...
def check_real_evidence_reference(phase_id: str, field: str, value: Any, failures: list[str]) -> None:
    if is_template_evidence_ref(value):
        fail(
            failures,
            f"{phase_id}.{field} must reference real production MinIO migration evidence, not a template/example/sample asset",
        )
...
def check_ready_summary(phase_id: str, summary: dict[str, Any], failures: list[str]) -> None:
    if phase_id in {"source-bucket-inventory", "target-bucket-inventory"}:
        require_int_summary(phase_id, summary, "buckets", failures, minimum=1)
        require_int_summary(phase_id, summary, "objects", failures, minimum=1)
        require_int_summary(phase_id, summary, "bytes", failures, minimum=1)
        ...
        total = require_int_summary(phase_id, summary, "totalObjects", failures, minimum=1)
        match = require_int_summary(phase_id, summary, "match", failures, minimum=1)
        if match != total:
            fail(failures, f"{phase_id}.summary.match must equal totalObjects")
        for key in ("missing", "mismatch", "errors"):
        ...
        require_int_summary(phase_id, summary, "commands", failures, minimum=1)
        failed = require_int_summary(phase_id, summary, "failedCommands", failures)
        if failed != 0:
            fail(failures, f"{phase_id}.summary.failedCommands must be 0")
        if summary.get("resumable") is not True:
            fail(failures, f"{phase_id}.summary.resumable must be true")
    elif phase_id == "sample-download-verification":
        if summary.get("status") != "PASS":
            fail(failures, f"{phase_id}.summary.status must be PASS")
        samples = require_int_summary(phase_id, summary, "samples", failures, minimum=1)
        ...
        if passed != samples:
            fail(failures, f"{phase_id}.summary.passed must equal samples")
        if failed != 0:
        ...
        if summary.get("sourceReadFallback") is not True:
            fail(failures, f"{phase_id}.summary.sourceReadFallback must be true")
...
def check_phase(index: int, phase: Any, failures: list[str], strict_ready: bool) -> str | None:
    ...
    if phase_id not in REQUIRED_PHASE_IDS:
        fail(failures, f"unknown MinIO migration phase id: {phase_id}")
    ...
    if status == "READY":
        ...
        if strict_ready:
            fail(failures, f"{phase_id} must be READY for strict MinIO migration readiness")
...
def check_document(data: Any, failures: list[str], strict_ready: bool) -> None:
    if not isinstance(data, dict):
        fail(failures, "MinIO migration evidence must be a JSON object")
        return
    ...
    if strict_ready:
        if data.get("status") != "READY":
            fail(failures, "top-level status must be READY for strict MinIO migration readiness")
        for field in ("generatedAt", "owner", "repoCommit", "migrationWindow", "sourceEndpointRef", "targetEndpointRef"):
            if is_placeholder(data.get(field)):
                fail(failures, f"{field} must not be a placeholder for strict MinIO migration readiness")
    ...
    for index, phase in enumerate(phases):
        ...
        if phase_id:
            if phase_id in seen_ids:
                fail(failures, f"duplicate MinIO migration phase id: {phase_id}")
    ...
    if missing:
        fail(failures, f"missing MinIO migration phases: {', '.join(sorted(missing))}")
...
def main() -> int:
    ...
    parser.add_argument("--evidence", required=True, help="MinIO migration evidence JSON")
    ...
    if failures:
        print(f"MinIO migration evidence validation failed: {len(failures)} issue(s).", file=sys.stderr)
        return 1

    print("MinIO migration evidence validation passed.")
    return 0
