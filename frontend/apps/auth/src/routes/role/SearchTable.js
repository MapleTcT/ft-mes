...
class RoleSearchTable extends React.Component {
  state = {
    dataSource: [],
    pagination: {
      current: 1,
  ...
  handleExport(all) {
    const params = {};
    const { searchKeyword: keyword } = this.props;
  ...
  searchRoleList() {
    ...
    fetchRoleUser({ ...pagination, keyword }).then((res) => {
      const {
  ...
  render() {
    ...
    const { searchKeyword, intl } = this.props;
    const title = (
      <span>
        <b style={{ marginRight: 5 }}>{searchKeyword}</b>
    ...
    return (
      <>
        <SupTable
          ...
          tableKey="roleSearchTable"
          operationBarTitle={title}
...
export default injectIntl(RoleSearchTable);
