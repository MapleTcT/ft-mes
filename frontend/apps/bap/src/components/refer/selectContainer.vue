...
export default {
  ...
  data() {
    return {
      ...
      propBind: {
        ...
        // url: '/foundation/user/common/getStaffDetailRefInfo?departmentId=1001',
        listUrl: `${this.params.listUrl}&departmentId=${this.params.groupId}`,
        isSingleCheck: this.params.isSingleCheck
      }
    };
  },
  ...
  mounted() {
    ...
    $this.propBind_O.updatePanel =
      this.$refs.groupSlideChild && this.$refs.groupSlideChild.updatePanel;
  },
  methods: {
    //根据key查找对应的数值
    ...
    linkcallBack: function(params) {
      ...
      url = url[0] + "&departmentId=" + list.id;
      let flag = list.isChecked;
      let pageSize = 20;
      // 单选的时候刷新下拉列表，多选的时候不刷新
      this.$refs.groupSlideChild.pageNum["page.pageNo"] = 1;
      if (isAll) {
        pageSize = 99999;
      }
      if (isHead) flag = 2;
      this.$refs.groupSlideChild.pageNum["page.pageSize"] = pageSize;
