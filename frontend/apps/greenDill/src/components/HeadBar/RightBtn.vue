<template>
  <div v-if="hidden && (viewType=='EDIT' || viewType=='VIEW') && !isRefer && showRightBtn">
    <div
      class="m_rightbtn"
      v-if="(!urlParams.id && referenceCopy) || (editButtonsList && editButtonsList.length > 0) || (urlParams.id && isAllowProxy) || hasAttachment"
      ref="rightbtn"
      ...
      >
        ...
        >
          <ul class="btn_ul">
            ...
            >
              <i class="btn-icon copy-icon"></i>{{$t('button.referCopy.text')}}
            </li>
            ...
            <li
              ...
              @click="editBtnList.func(), clickEvent()"
            >
              <i
                class="btn-icon"
...
export default {
  ...
  mounted() {
    ...
    //是否显示头部todo
    if (window.pageConfig) {
      $this.hidden = window.pageConfig.hidden ? window.pageConfig.hidden : false;
    }
  ...
  methods: {
    ...
    openReferCopy: function (referenceCopy, event) {
      ...
      var callbackFunc = function (list) {
        ...
        // if (window.pageConfig && window.pageConfig.interfaceApi) { //获取表格数据地址
        //   let dataGridDataUrl = prefix + window.pageConfig.interfaceApi.datagridData;
        ...
        //     basciUrl = `${dataGridDataUrl}${dtCode}?datagridCode=${datagridKey}&id=${list.id}`;
        //     vue.refreshDatagridData(basciUrl, true, dtCode);//刷新datagrid数据 
        //   }
        // }
      ...
      vue.openRefSelecFunc(data, callbackFunc, goBack);
