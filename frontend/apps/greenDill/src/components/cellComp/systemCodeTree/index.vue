<template>
  <div
    class="systemCodeTree"
    v-if="isShowSystemCode"
  >
    <header
      class="m_head"
    ...
    >
      <div class="m_hbox">
        ...
        <h2 class="m_title">{{$t('cellComp.systemcode.title')}}</h2>
      </div>
    </header>
    ...
    >
      ...
      <a
        ...
        v-on:click="btnClickEvent('confirm')"
      >{{$t('button.ok.text')}}</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    closeTouch: function () {
      ...
      document.getElementsByClassName("m_head")[0].style.display = "none"; //解决ios头部遮盖的问题
    },

    openTouch: function () {
      document.getElementsByClassName("m_centerBox")[0].style.overflow = "auto";
      document.getElementsByClassName("m_head")[0].style.display = "";
    }
  }
};
</script>
