  ...
  <div id="workflow">
    ...
    <div class="bg-white">
      <div class="mt20 list-space">
        <template v-if="sortByType && sortByType['1'] && (sortByType['1'].length > 1 || sortByType['1'][0].selectStaff != '0' || routerData.type == '7' || (sortByType['4'] && sortByType['4'].length > 0))">
          <div
          ...
          >
            ...
            <p
              v-if="sortByType['1'] && sortByType['1'].length > 1 && index != sortByType['1'].length-1"
              class="m-border"
            ></p>
          </div>
        </template>
        ...
        <!-- 循环会签 -->
        <template v-if="routerData && routerData.type == '7' && routerData.loopCountersign">
          <p class="m-border"></p>
          <div class="m-work-list">
            <selectPersonnel
              v-on:update="updatePerson"
              :params="{personnelTitle: $t('workflow.route.forward'), chooseSwitch: routerData.loopCountersign, changeData: routerData}"
            ></selectPersonnel>
            <div class="ms_check">
    ...
    >
      <div class="mt20 list-space">
        ...
        >
          ...
          <p
            v-if="sortByType['2'] && sortByType['2'].length > 1 && index != sortByType['2'].length-1"
            class="m-border"
          ></p>
        </div>
      </div>
    ...
    <!-- 作废 3 -->
    <div
      v-if="sortByType && sortByType['3'] && pendingId"
      class="bg-white"
    >
      <div class="mt20 list-space">
        <div
          v-for="(item, index) in sortByType['3']"
          class="m-work-list"
        >
          <selectPersonnel
            v-on:update="updatePerson"
          ...
          <p
            v-if="sortByType['3'] && sortByType['3'].length > 1 && index != sortByType['3'].length-1"
            class="m-border"
          ></p>
        </div>
      </div>
    ...
    >
      <div class="mt20 list-space">
        ...
        >
          ...
          <p
            v-if="sortByType['4'] && sortByType['4'].length > 1 && index != sortByType['4'].length-1"
            class="m-border"
          ></p>
    ...
    >
      ...
      <textarea
        ref="comments"
        :placeholder="$t('workflow.comment.placeholder')"
        id="workflow_comments"
        name="workFlowVar.comment"
      ></textarea>
    </div>

    <div
      v-if="routerData && routerData.outgoingTransitions"
      class="m_foot"
    >
      <a
...
export default {
  ...
  mounted() {
    ...
    publicGetAxios({
      ...
      this.$nextTick(() => {
        ...
        const outgoingTransitions = this.routerData.outgoingTransitions;
        const { '1': submitTransi = [] } = this.sortByType;
        if (submitTransi.length === 1 || (outgoingTransitions && outgoingTransitions.length == 1)) {
          if (submitTransi[0].selectStaff.toString() !== '0') {
            const changeDataCode = submitTransi[0].code;
  ...
  methods: {
        ...
        switch (type) { //有按钮的路由添加默认值
          case "1": // 普通迁移线
            if (data && (data.length == 1 && data[0].selectStaff == '0' && taskType.toString() != '7')) {
              if (!($this.sortByType['4'] && $this.sortByType['4'].length > 0)) { //有普通路由和通知路由同时存在时
    ...
    routeParamValidate: function (operateType, status, lines, outcomeCode) {
      ...
      let commentVal;
      if ($this.showComment) {
        commentVal = $this.$refs.comments && $this.$refs.comments.value;
      } else {
        commentVal = "";
      }
      if (requiredStaffError && requiredStaffError.length > 0) {
        let message = outcomeMapJson[0].dec + requiredStaffError[0];
      ...
      const workFlowVar = {
        ...
        outcome: outcomeCode,
        comment: commentVal,
        outcomeType: outcomeMapJson && outcomeMapJson[0] && outcomeMapJson[0].type,
        assignUser: outcomeMapJson && outcomeMapJson[0] && outcomeMapJson[0].assignUser
      };
      const param = { workFlowVar };
      const isloop = $this.isLoopCountersign(); // 是否为循环会签
      if (isloop) {
        // 循环会签参数组织
        const selects = $this.getAssignUser($this.routerData.code);
        param.workFlowVar.countersignUsers = selects;
        param.countersignUsersAddIds = selects;
        param.countersignUsersMultiIDs = selects;
        param.pendingActivityType = 'countersign';
      }
    ...
    // 是否为循环会签
    isLoopCountersign: function () {
      const { loopCountersign: loop, type: taskType } = this.routerData;
      return taskType.toString() === '7' && loop && String(loop) !== '0';
    },

    // 获取人员选择的数据
    getAssignUser: function (code) {
      const routerData = this.routerData;
      const members = routerData.members;
      let assignUser = '';
    ...
    validatorComment: function (dealSet, commentVal) {
      ...
      if (dealSet == "1" && (!commentVal || commentVal == "")) {
        $this.myToast(this.$t('notice.comment.edit'));
        return false;
      }
            ...
            if (code === tag) {
              ...
              if (item.members && item.members.length > 0) {
                ...
                for (let j = 0; j < item.members.length; j++) {
                  ...
                  element.id && memberTemp.push(element.id);
                }
              ...
              if (requiredStaff && info.assignUser === '') {
                const errorStr = this.$t('workflow.validate.staffRequire');
    ...
    handelBtnClick: function (type) {
      ...
      switch (operateType) {
        case "save":
          ...
          break;
        case "submit":
          if (!$this.routeCode && !$this.cancelCode && taskType && taskType.toString() !== "5") { // 除通知活动其他活动必须有一条出去的迁移线;
            $this.myToast(this.$t('notice.route.select'));
            $this.routerToTop();
            ...
            if (taskType.toString() !== "5") {
              ...
              lines = [this.$t('workflow.route.notice')];
            }
        ...
        case "reject": //驳回
          if (!$this.rejectCode) {
            $this.myToast(this.$t('notice.rejectRoute.select'));
            $this.routerToTop();
          } else {
            let lines = [];
    ...
    submitData: function (operateType, status, workParams) {
      const $this = this;
      let editForm = $this.$parent && $this.$parent.$refs.editForm && $this.$parent.$refs.editForm;
      let pending = editForm.pending;
      if (pending) {
        workParams.workFlowVar.activityName = pending.activityName;
        workParams.workFlowVar.activityType = pending.activityType;
      }
      if (operateType == 'save') {
        editForm.submitData({ status: status, operateType: operateType, saveCommentParams: workParams }); // 提交        
      } else {
        editForm.submitData({ status: status, operateType: operateType, workParams: workParams }); // 提交        
      }
    },
