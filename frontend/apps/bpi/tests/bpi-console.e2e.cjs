...
test('team completes a shadow run acceptance and critical data quality blocks approval until resolved', async () => {
  ...
  await trainingCoverage.getByText('0 / 7', { exact: true }).waitFor();
  await trainingCoverage.getByText('0 / 100', { exact: true }).waitFor();
  await trainingCoverage.getByText('0 / 10', { exact: true }).waitFor();
  ...
  await page.locator('[data-training-data-coverage]').getByText('10 / 200', { exact: true }).waitFor();
  await page.locator('[data-training-data-coverage]').getByText('6 / 7', { exact: true }).waitFor();
  await page.locator('[data-training-data-coverage]').getByText('10 / 100', { exact: true }).waitFor();
  await page.locator('[data-training-data-coverage]').getByText('0 / 10', { exact: true }).waitFor();
  await page.getByText('TRAINING_REJECTED_START_LABELS_BELOW_MINIMUM', { exact: true }).waitFor();
