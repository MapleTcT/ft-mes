...
import {getSysCode} from '../../services/changePass'
...
class EditForm extends React.Component {
  ...
  componentWillMount(){
    const {staffInfo: {gender}} = this.props
    getSysCode({entityCode: 'sys_gender', code: gender}).then((res)=> {
      const {data:{list}} = res
      const value = gender ? list.filter((item) => {
        return item.code === gender.substring(gender.lastIndexOf('/') + 1)
      }) : [];
      this.setState({
        genderValue: value.length ? value[0].displayName : null
      })
    })
  }
  ...
  render() {
    ...
    const { name, code,  status, directLeader, grandLeader, phone, email, description, positions, mainPosition, avatarUrl } = staffInfo;
