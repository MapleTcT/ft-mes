...
const NODE_TYPE_VIEW = 'view';

const NODE_TYPE_TREE = [NODE_TYPE_MODULE, NODE_TYPE_ENTITY, NODE_TYPE_VIEW];

class ViewManageTree extends React.Component {
  state = {
    treeData: [],
    selectedKeys: []
  };
  ...
  handleSelect = (selectedKeys, event) => {
    ...
    if (!isParent) {
      const { handleClickView } = this.props;
      handleClickView(selectedKeys[0]);
    }
  };
...
export default injectIntl(ViewManageTree);
