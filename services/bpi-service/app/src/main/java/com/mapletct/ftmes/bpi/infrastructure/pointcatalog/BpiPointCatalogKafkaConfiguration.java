...
public class BpiPointCatalogKafkaConfiguration {
    private static final int KAFKA_ENVELOPE_OVERHEAD_BYTES = 1024 * 1024;

    @Bean("bpiPointCatalogProducerFactory")
    ProducerFactory<byte[], byte[]> bpiPointCatalogProducerFactory(BpiPointCatalogKafkaProperties properties) {
        ...
        configuration.put(ProducerConfig.MAX_REQUEST_SIZE_CONFIG, maxMessageBytes(properties));
        return new DefaultKafkaProducerFactory<>(configuration);
    }

    @Bean("bpiPointCatalogKafkaTemplate")
    KafkaTemplate<byte[], byte[]> bpiPointCatalogKafkaTemplate(
            @Qualifier("bpiPointCatalogProducerFactory") ProducerFactory<byte[], byte[]> producerFactory) {
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean("bpiPointCatalogKafkaErrorHandler")
    DefaultErrorHandler bpiPointCatalogKafkaErrorHandler(
            BpiPointCatalogKafkaProperties properties,
            @Qualifier("bpiPointCatalogKafkaTemplate") KafkaTemplate<byte[], byte[]> kafkaTemplate) {
        ...
        handler.addNotRetryableExceptions(
                PointCatalogKafkaRecordRejectedException.class,
                BpiConflictException.class);
        handler.setCommitRecovered(true);
        return handler;
    }

    @Bean("bpiPointCatalogKafkaListenerContainerFactory")
    ConcurrentKafkaListenerContainerFactory<byte[], byte[]> bpiPointCatalogKafkaListenerContainerFactory(
            BpiPointCatalogKafkaProperties properties,
            @Qualifier("bpiPointCatalogKafkaErrorHandler") DefaultErrorHandler errorHandler) {
        Map<String, Object> configuration = new HashMap<>();
        ...
        configuration.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, 10);
        configuration.put(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG, maxMessageBytes(properties));
        configuration.put(ConsumerConfig.FETCH_MAX_BYTES_CONFIG, maxMessageBytes(properties));
    ...
    private static int maxMessageBytes(BpiPointCatalogKafkaProperties properties) {
        return properties.maxPayloadBytes() + KAFKA_ENVELOPE_OVERHEAD_BYTES;
    }
}
