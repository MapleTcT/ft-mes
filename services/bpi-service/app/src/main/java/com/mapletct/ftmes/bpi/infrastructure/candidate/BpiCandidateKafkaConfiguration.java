...
public class BpiCandidateKafkaConfiguration {

    @Bean("bpiCandidateProducerFactory")
    ProducerFactory<byte[], byte[]> bpiCandidateProducerFactory(BpiCandidateKafkaProperties properties) {
    ...
    @Bean("bpiCandidateKafkaTemplate")
    KafkaTemplate<byte[], byte[]> bpiCandidateKafkaTemplate(
            @Qualifier("bpiCandidateProducerFactory") ProducerFactory<byte[], byte[]> producerFactory) {
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean("bpiCandidateKafkaErrorHandler")
    DefaultErrorHandler bpiCandidateKafkaErrorHandler(
            BpiCandidateKafkaProperties properties,
            @Qualifier("bpiCandidateKafkaTemplate") KafkaTemplate<byte[], byte[]> kafkaTemplate) {
        ...
        handler.addNotRetryableExceptions(
                CandidateKafkaRecordRejectedException.class,
                BpiConflictException.class);
        handler.setCommitRecovered(true);
        return handler;
    }

    @Bean("bpiCandidateKafkaListenerContainerFactory")
    ConcurrentKafkaListenerContainerFactory<byte[], byte[]> bpiCandidateKafkaListenerContainerFactory(
            BpiCandidateKafkaProperties properties,
            @Qualifier("bpiCandidateKafkaErrorHandler") DefaultErrorHandler errorHandler) {
        Map<String, Object> configuration = new HashMap<>();
