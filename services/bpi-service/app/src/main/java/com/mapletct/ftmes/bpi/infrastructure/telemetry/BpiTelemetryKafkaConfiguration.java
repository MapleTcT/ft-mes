...
public class BpiTelemetryKafkaConfiguration {
    private static final int KAFKA_ENVELOPE_OVERHEAD_BYTES = 1024 * 1024;

    @Bean("bpiTelemetryProducerFactory")
    ProducerFactory<byte[], byte[]> bpiTelemetryProducerFactory(BpiTelemetryKafkaProperties properties) {
    ...
    @Bean("bpiTelemetryKafkaTemplate")
    KafkaTemplate<byte[], byte[]> bpiTelemetryKafkaTemplate(
            @Qualifier("bpiTelemetryProducerFactory") ProducerFactory<byte[], byte[]> producerFactory) {
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean("bpiTelemetryKafkaErrorHandler")
    DefaultErrorHandler bpiTelemetryKafkaErrorHandler(
            BpiTelemetryKafkaProperties properties,
            @Qualifier("bpiTelemetryKafkaTemplate") KafkaTemplate<byte[], byte[]> kafkaTemplate) {
        ...
        handler.addNotRetryableExceptions(
                TelemetryKafkaRecordRejectedException.class,
                BpiConflictException.class);
        handler.setCommitRecovered(true);
        return handler;
    }

    @Bean("bpiTelemetryKafkaListenerContainerFactory")
    ConcurrentKafkaListenerContainerFactory<byte[], byte[]> bpiTelemetryKafkaListenerContainerFactory(
            BpiTelemetryKafkaProperties properties,
            @Qualifier("bpiTelemetryKafkaErrorHandler") DefaultErrorHandler errorHandler) {
        Map<String, Object> configuration = new HashMap<>();
        ...
        configuration.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, properties.autoOffsetReset());
    ...
    private static int maxMessageBytes(BpiTelemetryKafkaProperties properties) {
