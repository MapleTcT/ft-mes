...
@Validated
@ConfigurationProperties(prefix = "bpi.data-quality-kafka")
public record BpiDataQualityKafkaProperties(
        boolean enabled,
    ...
    public BpiDataQualityKafkaProperties {
        if (topic != null && topic.equals(dlqTopic)) {
            throw new IllegalArgumentException("Data-quality Kafka source and DLQ topics must differ.");
        }
        ...
        if (retryBackoff == null || retryBackoff.isNegative() || retryBackoff.isZero()) {
            throw new IllegalArgumentException("Data-quality Kafka retry backoff must be positive.");
        }
    }
    ...
    private static Set<String> normalizedScope(Set<String> values, String scope) {
        if (values == null) {
            throw new IllegalArgumentException("Data-quality Kafka " + scope + " scope is required.");
        }
        Set<String> normalized = new LinkedHashSet<>();
        for (String value : values) {
            if (value == null || value.isBlank()) {
                throw new IllegalArgumentException("Data-quality Kafka " + scope + " scope cannot be blank.");
            }
