...
public class BpiSourceSequenceKafkaConfiguration {
    private static final int KAFKA_ENVELOPE_OVERHEAD_BYTES = 64 * 1024;

    @Bean("bpiSourceSequenceProducerFactory")
    ProducerFactory<byte[], byte[]> bpiSourceSequenceProducerFactory(
    ...
    @Bean("bpiSourceSequenceKafkaTemplate")
    KafkaTemplate<byte[], byte[]> bpiSourceSequenceKafkaTemplate(
            @Qualifier("bpiSourceSequenceProducerFactory")
            ProducerFactory<byte[], byte[]> producerFactory) {
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean("bpiSourceSequenceKafkaErrorHandler")
    DefaultErrorHandler bpiSourceSequenceKafkaErrorHandler(
            BpiSourceSequenceKafkaProperties properties,
            @Qualifier("bpiSourceSequenceKafkaTemplate") KafkaTemplate<byte[], byte[]> kafkaTemplate) {
        ...
        handler.addNotRetryableExceptions(
                SourceSequenceEvidenceKafkaRecordRejectedException.class,
                BpiConflictException.class);
        handler.setCommitRecovered(true);
        return handler;
    }

    @Bean("bpiSourceSequenceKafkaListenerContainerFactory")
    ConcurrentKafkaListenerContainerFactory<byte[], byte[]> bpiSourceSequenceKafkaListenerContainerFactory(
            BpiSourceSequenceKafkaProperties properties,
            @Qualifier("bpiSourceSequenceKafkaErrorHandler") DefaultErrorHandler errorHandler) {
        Map<String, Object> configuration = new HashMap<>();
    ...
    private static int maxMessageBytes(BpiSourceSequenceKafkaProperties properties) {
