...
@Repository
public class RuleApplicationPostgresRepository {
    private final NamedParameterJdbcTemplate jdbc;
    private final ObjectMapper objectMapper;

    public RuleApplicationPostgresRepository(
    ...
    public RuleApplicationTarget lockTarget(String tenantId, UUID publicationId) {
        try {
            RuleApplicationTarget target = jdbc.queryForObject("""
                    SELECT o.id AS publication_id, o.aggregate_id AS rule_id,
                           o.tenant_id, o.plant_id, o.line_id, o.status AS publication_status,
                           o.revision AS publication_revision, o.application_status,
                    ...
                    (rs, rowNum) -> new RuleApplicationTarget(
        ...
        int updated = jdbc.update("""
                UPDATE bpi.bpi_outbox_events
                   SET application_status = :applicationStatus,
                       ...
                       application_deployment_id = :deploymentId,
                       application_observed_at = :observedAt,
                       application_received_at = now(),
                       application_error_code = :errorCode,
                       application_error_detail = :errorDetail,
                       revision = revision + 1,
                       updated_at = now()
                 WHERE tenant_id = :tenantId
                   AND id = :publicationId
                   AND revision = :expectedRevision
                   AND application_status = :expectedApplicationStatus
                """, new MapSqlParameterSource()
                .addValue("applicationStatus", event.getStatus().name())
                .addValue("applicationEventId", event.getEventId())
                .addValue("deploymentId", event.getDeploymentId())
                .addValue("observedAt", new Timestamp(event.getObservedAtMs()))
                .addValue("errorCode", blankToNull(event.getErrorCode()))
                .addValue("errorDetail", blankToNull(event.getDetail()))
                ...
                .addValue("expectedApplicationStatus", target.applicationStatus()));
        if (updated != 1) {
            throw new BpiConflictException(
                    "Rule application state changed concurrently.", target.publicationRevision());
        }
    ...
    public void insertAudit(
        ...
        jdbc.update("""
                ...
                .addValue("action", "RULE_PUBLICATION_" + event.getStatus().name())
                ...
                .addValue("detail", writeJson(Map.of(
                        "applicationEventId", event.getEventId(),
    ...
    private String writeJson(Object value) {
        try {
            ...
            throw new IllegalStateException("Could not write rule application audit JSON", error);
        }
    }
