...
@Validated
@ConfigurationProperties(prefix = "bpi.rule-application-kafka")
public record BpiRuleApplicationKafkaProperties(
        boolean enabled,
    ...
    public BpiRuleApplicationKafkaProperties {
        ...
        if (retryBackoff == null || retryBackoff.isNegative() || retryBackoff.isZero()) {
            throw new IllegalArgumentException("Rule application retry backoff must be positive.");
        }
    }
    ...
    private static Set<String> normalizedScope(Set<String> values, String scope) {
        if (values == null) {
            throw new IllegalArgumentException("Rule application " + scope + " scope is required.");
        }
        Set<String> normalized = new LinkedHashSet<>();
        for (String value : values) {
            if (value == null || value.isBlank()) {
                throw new IllegalArgumentException("Rule application " + scope + " scope cannot be blank.");
            }
