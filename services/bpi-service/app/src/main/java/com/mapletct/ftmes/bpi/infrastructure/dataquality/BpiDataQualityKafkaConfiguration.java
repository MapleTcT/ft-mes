...
public class BpiDataQualityKafkaConfiguration {

    @Bean("bpiDataQualityProducerFactory")
    ProducerFactory<byte[], byte[]> bpiDataQualityProducerFactory(BpiDataQualityKafkaProperties properties) {
    ...
    @Bean("bpiDataQualityKafkaTemplate")
    KafkaTemplate<byte[], byte[]> bpiDataQualityKafkaTemplate(
            @Qualifier("bpiDataQualityProducerFactory") ProducerFactory<byte[], byte[]> producerFactory) {
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean("bpiDataQualityKafkaErrorHandler")
    DefaultErrorHandler bpiDataQualityKafkaErrorHandler(
            BpiDataQualityKafkaProperties properties,
            @Qualifier("bpiDataQualityKafkaTemplate") KafkaTemplate<byte[], byte[]> kafkaTemplate) {
        ...
        handler.addNotRetryableExceptions(
                DataQualityKafkaRecordRejectedException.class,
                BpiConflictException.class);
        handler.setCommitRecovered(true);
        return handler;
    }

    @Bean("bpiDataQualityKafkaListenerContainerFactory")
    ConcurrentKafkaListenerContainerFactory<byte[], byte[]> bpiDataQualityKafkaListenerContainerFactory(
            BpiDataQualityKafkaProperties properties,
            @Qualifier("bpiDataQualityKafkaErrorHandler") DefaultErrorHandler errorHandler) {
        Map<String, Object> configuration = new HashMap<>();
