...
public class WmsAdapterKafkaConfiguration {

    @Bean("bpiWmsAdapterProducerFactory")
    ProducerFactory<byte[], byte[]> bpiWmsAdapterProducerFactory(
        ...
        configuration.put(ProducerConfig.CLIENT_ID_CONFIG, properties.clientId() + "-producer");
    ...
    @Bean("bpiWmsAdapterKafkaTemplate")
    KafkaTemplate<byte[], byte[]> bpiWmsAdapterKafkaTemplate(
            @Qualifier("bpiWmsAdapterProducerFactory") ProducerFactory<byte[], byte[]> producerFactory) {
        return new KafkaTemplate<>(producerFactory);
    }

    @Bean("bpiWmsAdapterErrorHandler")
    DefaultErrorHandler bpiWmsAdapterErrorHandler(
            BpiWmsAdapterProperties properties,
        ...
        DefaultErrorHandler handler = new DefaultErrorHandler(
                recoverer,
                new FixedBackOff(
                        properties.retryBackoff().toMillis(), properties.maxAttempts() - 1L));
        handler.addNotRetryableExceptions(WmsCommandRejectedException.class);
        handler.setCommitRecovered(true);
        return handler;
    }

    @Bean("bpiWmsAdapterListenerContainerFactory")
    ConcurrentKafkaListenerContainerFactory<byte[], byte[]> bpiWmsAdapterListenerContainerFactory(
            BpiWmsAdapterProperties properties,
            @Qualifier("bpiWmsAdapterErrorHandler") DefaultErrorHandler errorHandler) {
        Map<String, Object> configuration = new HashMap<>();
        ...
        configuration.put(ConsumerConfig.MAX_POLL_RECORDS_CONFIG, 20);
