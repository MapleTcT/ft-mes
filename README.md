...
| BPI 浏览器状态交互 | [BPI UI 验收](metadata/bpi-ui-acceptance.json)、[质量与库存 UI 验收](metadata/bpi-quality-inventory-ui-acceptance.json) | 18/18 确定性模拟浏览器，覆盖两步强制结束、影子验收和批次质量/WMS 六态、局部失败/重试、关闭竞态与移动端；模拟层不替代真实 Java/PostgreSQL 目标验收 |
...
| QCS 放行与 WMS 完工入库 | [目标全链验收](metadata/bpi-quality-release-wms-target-acceptance.json)、[本地 PostgreSQL 验收](metadata/bpi-quality-release-wms-inbound-acceptance.json)、[红字单本地合同](metadata/material-wms-completion-inbound-reversal-contract-acceptance.json)、[产品页验收](metadata/bpi-quality-inventory-ui-acceptance.json) | Flyway V23 合同 4/4、受控 QCS、目标 Kafka command/receipt、query-first adapter、material 单据/库存、durable receipt、QCS/Kafka 重放、真实页面和清理通过；内部追加式红字单本地 12/12 PASS，但 BPI 冲销审批/Kafka、目标 PostgreSQL、外部 QCS 主动事件及外部 ERP/WMS 冲销/宕机补偿仍 BLOCKED，全部开关默认关闭 |
