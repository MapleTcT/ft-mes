...
最近一次 RM Web 配方编辑复验时间：`2026-07-17T12:04:48.223Z`。`material-service` 与 `process-analysis` 已有 Nacos `prod` 健康实例，并已分别通过真实 marker 落库验收。生产列表导出在当前测试入口 `http://10.11.100.17:18080` 完成 6/6 真实浏览器点击、运行时元数据、目标 sourceAudit 和有效工作簿响应复验，状态为 `READY`，证据为 `metadata/production-export-readiness-smoke.json`。RM 批控配方列表现有真实可见 `Web编辑` 入口，创建、幂等、两次更新、投递失败/重试和六张 PostgreSQL 表已连续三轮通过，故 `PROD-010` 已移出 blocker；现场 Batch/DCS 端点签字仍是生产切换前置条件，不重新计为页面编辑 blocker。
