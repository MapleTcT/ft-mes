...
流式模块自动测试为 **14/14 PASS**，其中 5 项使用 Flink 官方
`KeyedBroadcastOperatorTestHarness`，不是自制上下文 mock。

## 状态与恢复契约

- keyed window state 使用 `ValueState<byte[]>`，magic=`BPIS`、codec version=1；
- broadcast rule state 使用 `MapState<String, byte[]>`，magic=`BPIR`、codec version=1；
...
- 当前迟到事件进入 `LATE_EVENT_UNSUPPORTED` issue；allowed lateness 修正状态尚未实现。
- 规则更新流目前是已解码的 `BoundaryRuleUpdate`，Kafka 规则命令格式和 schema registry 尚未接线。
- 尚未完成 IoT 真实回放、BPI inbox/PostgreSQL marker、候选确认和浏览器 E2E。
