...
| 实体/模型 PostgreSQL 自定义字段同步 | PASS | dedicated-marker-required | `metadata/entity-model-field-persistence-acceptance.json`、`deploy/docker/scripts/adp-entity-model-field-persistence-acceptance.js`、`metadata/entity-model-field-type-matrix-acceptance.json`、`deploy/docker/scripts/adp-entity-model-field-type-matrix-acceptance.js`、`metadata/persistence-acceptance.json` | `PG_FIELD` marker 以 `33/33 PASS` 证明索引/唯一/nullable 生命周期；`ADP_E2E_20260721075749_PG_TYPE_MATRIX` 以 `36/36 PASS` 证明 24 个实例覆盖全部 23 类标量 DbColumnType、原生 PostgreSQL 类型、整行 marker 回读、安全转换及失败回滚。自动删列仍关闭，`OBJECT` 关联字段留给独立 fixture |
...
字段级路径先使用 marker `ADP_E2E_20260721071510_PG_FIELD` 完成 `33/33` 约束生命周期验收。真实页面一次导航 `200`，23 个正常写请求均为 `200`；PostgreSQL 证明 `E2E_TXT_071510 varchar(64)`、托管普通索引和唯一约束的创建、幂等、改名和停用，外部唯一索引保护，nullable/NOT NULL 双向切换，以及危险变更同事务回滚。证据见 `metadata/entity-model-field-persistence-acceptance.json`。

随后 marker `ADP_E2E_20260721075749_PG_TYPE_MATRIX` 在同一真实页面和 API 链上完成 `36/36`。24 个实例覆盖全部 23 类标量 `DbColumnType`，PostgreSQL 直接确认 `varchar/integer/numeric/date/time/timestamp/bytea/boolean/text/bigint` 物理类型和完整 marker 行回读；`PROPERTYATTACHMENT` 只落元数据而不建列。`BOOLEAN <-> INTEGER`、`INTEGER -> BIGINT/NUMERIC`、`DATE -> DATETIME` 保值通过；非法整数 `2 -> BOOLEAN` 和 `numeric(19,2) -> numeric(19,6)` 容量缩减均返回预期 `500`，元数据与 DDL 同事务回滚。清理后 property/model/entity/table/marker 为 `0/0/0/0/0`，证据见 `metadata/entity-model-field-type-matrix-acceptance.json`。

边界仍要说清：当前已经关闭标量字段类型和字段约束生命周期。自动 `DROP COLUMN` 仍明确关闭；`OBJECT` 依赖被关联模型和目标属性，必须用独立双模型 fixture 验收，不能从标量矩阵外推。
