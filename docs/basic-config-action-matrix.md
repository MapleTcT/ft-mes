...
| 实体/模型 PostgreSQL 自定义字段同步 | PASS | dedicated-marker-required | `metadata/entity-model-field-persistence-acceptance.json`、`deploy/docker/scripts/adp-entity-model-field-persistence-acceptance.js`、`metadata/persistence-acceptance.json` | marker `ADP_E2E_20260721071510_PG_FIELD` 已以 `33/33 PASS` 证明 TEXT 列、托管普通索引和唯一约束创建/停用/幂等/改名、外部唯一索引保护、nullable/NOT NULL 启停、重复值与 NULL 拒绝、marker 行、安全扩容、危险 NOT NULL/类型转换事务回滚和受控清理；自动删列及更广类型矩阵仍不在该动作范围 |
...
字段级路径随后使用 marker `ADP_E2E_20260721071510_PG_FIELD` 完成 `33/33` 独立验收。真实页面一次导航 `200`，23 个正常写请求均为 `200`；PostgreSQL 证明 `E2E_TXT_071510 varchar(64)`、托管普通索引和 `UQ_DS_E2EF_071510_E2E_TXT_071510` 唯一约束创建，约束幂等重放并拒绝重复 marker，字段改名扩容后唯一约束与其索引同步改名，关闭唯一约束后恢复普通索引，普通索引也通过往返改名验证。外部唯一索引 `EXT_DS_E2EF_071510_E2E_TEXT_071510` 可同时满足索引和属性唯一性且不会被接管或重复创建；nullable/NOT NULL 双向切换和幂等重放通过，物理库拒绝 NULL。已有 NULL 时设置 NOT NULL 与 `TEXT -> INTEGER` 危险转换均按预期返回 `500`，`ec_property` 元数据和物理 DDL 同事务回滚；marker 行全程保持，清理后 property/model/entity/table 为 0/0/0/0。链路为 `PropertyController -> ModelServiceImpl -> FieldSyncDBUtils -> PostgresFieldSyncSupport`，证据见 `metadata/entity-model-field-persistence-acceptance.json`。

边界仍要说清：当前关闭的是 TEXT 字段新增、托管普通索引和唯一约束生命周期、外部唯一索引保护、nullable/NOT NULL、安全扩容、幂等和失败回滚。自动 `DROP COLUMN` 仍明确关闭，更广字段类型矩阵还需分批验收，不能从本报告外推。
