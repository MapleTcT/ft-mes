...
| 实体/模型 PostgreSQL 自定义字段同步 | PASS | dedicated-marker-required | `metadata/entity-model-field-persistence-acceptance.json`、`deploy/docker/scripts/adp-entity-model-field-persistence-acceptance.js`、`metadata/persistence-acceptance.json` | marker `ADP_E2E_20260721063213_PG_FIELD` 已以 `16/16 PASS` 证明 TEXT 列和托管普通索引创建、停用、重复停用、重启用、随字段改名、外部唯一索引保护、marker 行、安全扩容、幂等重放、危险类型转换事务回滚和受控清理；自动删列、唯一/非空约束变更及更广类型矩阵仍不在该动作范围 |
...
字段级路径随后使用 marker `ADP_E2E_20260721063213_PG_FIELD` 完成 `16/16` 独立验收。真实页面导航为 `200`，实体/模型/字段创建、托管索引停用/重复停用/重启用、字段重命名与安全扩容、外部唯一索引场景和幂等重放均返回 `200`；PostgreSQL 证明 `E2E_TXT_063213 varchar(64)` 与 `IDX_DS_E2EF_063213_E2E_TXT_063213` 创建，托管索引可安全停用并重建，字段改名为 `E2E_TEXT_063213` 时索引同步规范化为 `IDX_DS_E2EF_063213_E2E_TEXT_063213`。外部唯一索引 `EXT_DS_E2EF_063213_E2E_TEXT_063213` 在停用托管索引时保持存在，重启用时作为等价索引被复用且未产生重复；marker 行全程保持不变。`TEXT -> INTEGER` 危险转换按预期返回 `500`，`ec_property` 和物理列一起回滚，清理后 property/model/entity/table 均为 0。链路为 `PropertyController -> ModelServiceImpl -> FieldSyncDBUtils -> PostgresFieldSyncSupport`，证据见 `metadata/entity-model-field-persistence-acceptance.json`。

边界仍要说清：当前关闭的是 TEXT 字段新增、托管普通索引创建/停用/幂等停用/重启用/改名、外部唯一索引保护、安全扩容、幂等和不兼容转换回滚。自动 `DROP COLUMN` 明确关闭，唯一/非空约束本身的变更及更广字段类型矩阵仍需分别验收，不能从本报告外推。
