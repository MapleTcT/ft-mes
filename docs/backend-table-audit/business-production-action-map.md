...
| WOM 投入明细/消耗记录专项分析 | [`wom-consumption-record-analysis.md`](wom-consumption-record-analysis.md)；`metadata/wom-consumption-record-analysis.json`；历史 `0/0` 已定位为源码执行记录缺口，当前 `...CLOSED_10` 回读执行/消耗 `1/1`，由服务层补丁关闭且未引入数据库触发器 |
...
| PROD-ACTION-006 | 历史 `remainMaterialView/save` 后只有 `wom_putin_details`，缺少活动执行和消耗记录 | 已由 WOM 服务层补建执行记录并复用既有消耗生成器关闭；`...CLOSED_10` 真实页面/API/PostgreSQL 证明明细/执行/消耗 `1/1/1`，验收脚本已把缺失设为硬失败 |
