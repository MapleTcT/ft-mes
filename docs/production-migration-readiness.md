...
8 个 `PLANNED`、1 个 `BLOCKED`，生产模块 blocker 为 0 个、产品 backlog 为 0 个；
material/ProcessAnalysis 依赖和生产列表导出已经 READY，WOM 二维码与可见手工新建指令单也已闭合。旧 public `produceTaskCreated` 已按退役契约 `NOT_APPLICABLE` 关闭；现场 Batch/DCS 联调和真实生产业务签字仍必须作为切换前置风险保留。
...
| 业务 smoke 签字 | BLOCKED | 平台和 53 个业务页面 smoke 有证据；生产模块测试矩阵当前 44 条，`44 PASS / 0 BLOCKED / 0 NOT_RUN`，44 条 route smoke 全部 PASS。material/WMS、ProcessAnalysis、生产列表导出、WOM 二维码、可见手工新建指令单和独立不良数量均已完成真实页面/API/PostgreSQL 或文件响应验收；二维码浏览器和 API/PostgreSQL 分别 10/10 PASS，手工新建为 9/9 PASS，不良数量 marker `ADP_E2E_20260717141017_WOM_BAD_QTY` 已证明 WOM/QCS 双入口、10/2/8 数量分配、幂等、冲销和清理。`metadata/business-dependency-readiness-smoke.json` 证明 material-service 与 process-analysis 为 2/2 READY；`metadata/production-module-blockers.json` 和 `metadata/production-module-backlog.json` 当前均为 0 个未闭合项。旧 public `produceTaskCreated` 已由 marker `ADP_E2E_20260721104747_PUBLIC_PRODUCE_RETIRED` 按 `NOT_APPLICABLE` 关闭；`deploy/business-smoke/production-migration/` 已提供签字证据模板和 READY 校验器 | 现场 Batch/DCS 端点联调、回滚确认和真实生产业务签字尚未闭合，不能切换生产 |
...
| `docs/production-module-backlog.md` | 生产模块 backlog 账本说明 | 当前 active backlog 为 0；`PROD-ACTION-007`、WOM 二维码、可见手工新建指令单、RM Web 编辑和独立不良数量均在关闭记录中保留证据 |
| `metadata/production-module-backlog.json` | 生产模块 backlog 机器账本 | 当前 0 条未闭合 item；`make production-module-backlog-check` 会校验新出现的落库 `FAIL/BLOCKED`、生产 `BLOCKED` 和动作级缺口，并锁定旧 public endpoint 的退役契约 |
