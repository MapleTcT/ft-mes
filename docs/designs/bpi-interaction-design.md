...
点击产线打开右侧“实时证据”抽屉，包含 PostgreSQL 最新点位事实、最近 15 分钟真实遥测、
服务端运行判据和未解决数据质量事件。窗口没有数值样本时必须明确显示无数据，不允许浏览器补造趋势。
点击批次号进入批次详情；点击待处理数跳到已带筛选的候选页。

刷新采用增量轮询或 SSE，表格行不因刷新重新排序。用户切换“仅异常”后只显示有候选、
required 信号异常或上下文过期的产线。没有生产时显示最后心跳和最近一次批次，不留白页。

**主要 API：** `getBpiOverview`、`getCurrentLineState`、`getLineLiveEvidence`。
