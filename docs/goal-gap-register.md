...
| Field | Value |
| --- | --- |
| Generated At | `2026-07-21T08:14:53+00:00` |
...
| `G-012` | `PARTIAL` | 基础配置 | QCS reportShowIndexRange、RM.MQ brokerUrl、BaseSet.isEnable、customProperty 模型映射、实体/模型元数据 CRUD、PostgreSQL 基础物理模型表生命周期，以及字段约束生命周期和 23 类标量类型矩阵均已完成 marker、回读、落库和回滚/清理；对应 QCS/RM/BaseSet 代表性业务回归已复跑 PASS。身份、授权、密钥、密码类配置仍禁止通用 smoke 改值；其他 BaseSet/RM/QCS 配置项、自动删列、OBJECT 关联字段、Nacos/Keycloak 生产 export/diff、realm 迁移、secret 轮换和回退演练仍需继续验收。 | 继续把 QCS/RM/BaseSet 当前地址报告链路证据纳入后续生产或 rehearsal 业务 smoke 签字，但不要把测试环境证据直接冒充生产签字。 |
