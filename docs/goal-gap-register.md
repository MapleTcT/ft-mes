...
| Field | Value |
| --- | --- |
| Generated At | `2026-07-17T00:57:09+00:00` |
...
| `G-013` | `BLOCKED` | 生产模块完整功能 | 剩余 4 条生产用例仍 BLOCKED：RM 外部 Batch 客户端、PROD-019/PROD-021 独立不良数量范围确认和生产列表导出。material/WMS 完工入库与 ProcessAnalysis 追溯均已 PASS；WOM printManage 二维码接口仍是独立运行端点 backlog。 | 保持制造-质量-material/WMS-ProcessAnalysis marker 回归；完成独立不良数量范围产品决定、外部 Batch 客户端、二维码端点和生产导出。 |
...
| `G-018` | `PARTIAL` | 业务模块完整测试用例 | 剩余 4 条生产用例、独立不良数量等产品范围项、二维码/导出和业务负责人签字未完成；PATROL 统计未验收，完整隐患整改/复查/销项依赖尚未安装的真实 SESH；ProcessAnalysis 已闭合。 | 保持 material/WMS、ProcessAnalysis、PATROL 配置/执行/异常隐患链回归，继续 PATROL 统计；若要求整改、复查和销项先取得真实 SESH；继续产品范围、二维码、外部客户端和导出验收，并补齐 before/after SQL、文件响应和业务签字。 |
