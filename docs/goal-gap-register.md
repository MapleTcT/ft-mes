...
| Field | Value |
| --- | --- |
| Generated At | `2026-07-17T05:58:11+00:00` |
| Repo Commit | `5b314c535a446a0e547ffe365f52fbc9a67537a8` |
...
| `G-013` | `BLOCKED` | 生产模块完整功能 | 剩余 3 条生产用例仍 BLOCKED：RM 外部 Batch 客户端和 PROD-019/PROD-021 独立不良数量范围确认。material/WMS 完工入库、ProcessAnalysis 追溯与生产列表导出均已 PASS；WOM printManage 二维码接口仍是独立运行端点 backlog。 | 保持制造-质量-material/WMS-ProcessAnalysis 与生产列表导出回归；完成独立不良数量范围产品决定、外部 Batch 客户端和二维码端点。 |
...
| `G-018` | `PARTIAL` | 业务模块完整测试用例 | 剩余 3 条生产用例、独立不良数量等产品范围项、二维码和业务负责人签字未完成；生产列表导出已 6/6 PASS。PATROL Kafka/认证/TagManagement 技术链已闭合，但采集误差分析仍需真实误差基准项、测点历史和非空 gather_data，GIS 定位/轨迹依赖缺失的 SESGISConfig，完整隐患整改/复查/销项依赖尚未安装的真实 SESH；ProcessAnalysis 已闭合。 | 保持 material/WMS、ProcessAnalysis、生产列表导出、PATROL 配置/执行/异常隐患、统计聚合和 Kafka 消费回归；接入有历史值的真实 TagManagement 测点并配置误差基准巡检项，复验中位数落库和误差图表；取得 SESGISConfig 后恢复 GIS 定位/轨迹，若要求整改、复查和销项则先取得真实 SESH；继续产品范围、二维码和外部客户端验收，并补齐 before/after SQL 和业务签字。 |
