...
`CONTROLLED_MARKER_REQUIRED`；Nacos/Keycloak 测试环境运行态 smoke 已纳入机器校验；实体/模型 PostgreSQL 基础物理表自动创建和受控字段生命周期已通过；内置敏感配置写操作、其他业务运行配置、自动删列/索引删除/唯一与非空约束变更/更广字段类型矩阵，以及 Nacos/Keycloak 生产配置链路还不能按
整体基础配置完成来交付。
...
| 实体/模型 PostgreSQL 自定义字段同步 | PASS | `/msService/ec/engine/msManage`；areaId `configuration-physical-model-field` | marker `ADP_E2E_20260721053537_PG_FIELD` 已通过真实页面、字段 API 和 PostgreSQL 证明 TEXT 列与索引创建、marker 行写入、字段重命名、安全扩容、幂等重放、危险类型转换事务回滚和受控清理 | `ec_property`、`information_schema.columns`、`pg_index`、`DS_E2EF_053537`；`metadata/entity-model-field-persistence-acceptance.json`、`metadata/persistence-acceptance.json` | 当前范围无阻断；自动删列、索引删除、唯一/非空约束变更及更广类型矩阵仍需独立验收 |
