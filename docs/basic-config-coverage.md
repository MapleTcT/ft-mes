...
`CONTROLLED_MARKER_REQUIRED`；Nacos/Keycloak 测试环境运行态 smoke 已纳入机器校验；实体/模型 PostgreSQL 基础物理表自动创建、受控字段、普通索引、唯一约束和 nullable/NOT NULL 生命周期已通过；内置敏感配置写操作、其他业务运行配置、自动删列、更广字段类型矩阵，以及 Nacos/Keycloak 生产配置链路还不能按
整体基础配置完成来交付。
...
| 实体/模型 PostgreSQL 自定义字段同步 | PASS | `/msService/ec/engine/msManage`；areaId `configuration-physical-model-field` | marker `ADP_E2E_20260721071510_PG_FIELD` 已通过真实页面、字段 API 和 PostgreSQL `33/33` 证明 TEXT 列、托管普通索引与唯一约束创建/停用/幂等/改名，外部唯一索引保护，nullable/NOT NULL 启停，重复值与 NULL 拒绝，marker 行写入，字段安全扩容，以及危险 NOT NULL/类型转换事务回滚和受控清理 | `ec_property`、`information_schema.columns`、`pg_index`、`pg_constraint`、`DS_E2EF_071510`；`metadata/entity-model-field-persistence-acceptance.json`、`metadata/persistence-acceptance.json` | 当前范围无阻断；自动删列及更广字段类型矩阵仍需独立验收 |
