...
元数据 readiness smoke，并已完成 customProperty 映射、实体/模型元数据 CRUD 和 PostgreSQL 基础物理模型表生命周期专项验收；基础配置动作矩阵已把身份、授权、AK/SK、密码策略目录标为
`READ_ONLY_GUARDED`，把 QCS/RM/BaseSet 运行配置标为
`CONTROLLED_MARKER_REQUIRED`；Nacos/Keycloak 测试环境运行态 smoke 已纳入机器校验；实体/模型 PostgreSQL 基础物理表自动创建已通过；内置敏感配置写操作、其他业务运行配置、任意自定义字段 DDL 和 Nacos/Keycloak 生产配置链路还不能按
整体基础配置完成来交付。
