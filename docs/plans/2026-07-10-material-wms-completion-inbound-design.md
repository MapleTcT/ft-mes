...
- `POST /material/wms/completion-inbound-reversals`：BPI 受信来源的追加式完工入库红字冲销。
- `GET /material/wms/completion-inbound-reversals/by-idempotency`：按 BPI 幂等键精确查询红字单、原单和流水。
...
- `wms_stock_documents`：库存业务单据，类型为 `COMPLETION_INBOUND`、
  `COMPLETION_INBOUND_REVERSAL` 或 `PRODUCTION_ISSUE`；红字单引用原蓝字单。
...
来源系统、单据类型、来源单据 ID 和仓库构成单据头幂等键；来源明细 ID 构成实际记账幂等键；库存流水使用事件键防止重复记账。WOM 生产用料单会用同一个 `srcID` 按明细多次调用，因此已有单据头必须继续接收尚未记账的 `srcPartId`，不能整单提前返回。相同来源明细若数量或库存维度不一致则返回冲突。数量使用 `numeric(20,6)`。空仓库、空物料和非正数量明确失败。首期最初不支持红字冲销；迁移 `198` 后只为受信 BPI 来源开放单明细追加式冲销，要求新事件、新幂等键和与原单完全一致的库存事实，不生成半张单据。
...
完工入库冲销不删除也不覆盖原单和原流水，而是在同一事务中新建红字单、红字明细和负向库存流水，
成功后再把原蓝字单标记为 `REVERSED`。同一原单最多一个红字单；库存已被领用或冻结状态变化造成
可用量不足时，红字单及原单状态更新整体回滚。
