...
- 跨地点规范测试前端：`http://100.99.133.43:18080`
- 跨地点规范 SSH / DB / Docker 主机：`v6@100.99.133.43`
- 当前公司内网直连前端：`http://10.11.100.17:18080`
- 当前公司内网 SSH / DB / Docker 主机：`v6@10.11.100.17`
- 两条地址指向同一测试机；报告必须记录实际使用的入口。本轮 PATROL 使用公司内网地址，既有 BPI/平台 smoke 保留其 Tailscale 证据地址。
- 默认测试账号：`admin / 123456`
- PostgreSQL 数据库来自 Docker profile；常规排查优先只读，会改变业务数据的验收必须使用唯一 marker 并记录清理/回滚边界。
