...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 总设计、交互设计、API 目录和工程测试计划已落库；Java 8 适配器、Java 17 PostgreSQL 服务、BPI 操作台、Protobuf/OpenAPI、遥测/规则/拓扑/候选/影子批次、Flink 事件时间/生产上下文/Broadcast State/checkpoint、规则 transactional outbox、失败重试和审计已有本地验证；规则应用 `WAITING -> REJECTED -> APPLIED` 已通过真实 PostgreSQL 16.13、Flyway V1-V8 和确定性浏览器验收；Embedded Kafka 3.8.1 + PostgreSQL 联合测试进一步覆盖 `read_committed`、事务回滚不可见、listener restart 精确重放幂等、APPLIED 终态防回退和 poison payload DLQ | 联合测试仍由事务生产者模拟 Flink sink；尚缺真实 Flink job checkpoint/restart 回执、浏览器到 Java 服务联合验收、目标测试环境部署、IoT exporter 与 MES production context 现场联调、7-14 天影子运行、QCS/WMS 写回和训练数据阶段 |
