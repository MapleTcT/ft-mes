...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1902 个 CI/治理/runtime 依赖资产，其中 1902 tracked、0 untracked、0 missing references。本轮纳入 BPI Flyway V35 最新遥测投影和 Phase 3C-F 实时运行证据实现，并保留 V26-V34 数据集、MLflow、工艺窗口、MQTT 落表、遥测窗口索引与权限基线。Phase 3C-F 已完成本地 PostgreSQL、模拟器和真实浏览器验收，当前目标环境部署证据仍待补齐；连续生产启用、物理现场窗口与数据量、整站灾备、MLflow 生产治理和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
