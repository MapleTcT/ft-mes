...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `256 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V33 已分别闭合 manifest、Parquet、Iceberg/Polaris、Object Lock 恢复包、MLflow Dataset Input、训练准备度评估、受控工艺窗口与权限加固的目标页面、API、PostgreSQL、外部存储、失败重试/恢复、最小权限、重启与定向清理。V31 marker 证明 19 项门槛按当时真实数据失败关闭为 8 个 blocker；V32/V33 marker 又证明 2 个窗口形成 6 条不可变事实、迟到/冻结后排除、桌面/移动零错误和精确清理。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `259` 项，其中 `256 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V33 均有真实落库证据；V31 readiness、V32 process-window facts、V33 function ACL、审计/幂等和不可变 trigger 分别保存在 PostgreSQL，并与 MLflow/MinIO/Object Lock 证据对账，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 V26-V33 数据集链已有真实目标证据。V31 marker `ADP_E2E_BPI_READINESS_20260723_091500_A1` 保留两次不可变 `BLOCKED` 历史评估；V32/V33 release `f7db2f98e82d` 又由 marker `ADP_E2E_BPI_WINDOWS_20260723_1235_A1` 从真实页面创建 2 个窗口、固化 6 条不可变事实（2 READY/4 BLOCKED），证明 flow mean 20、pump ratio 0.5、迟到/冻结后排除、V33 最小权限、桌面/移动与 19 类清理。MLflow/model 路径仍为 0/false。 | Phase 3C-C 受控窗口实现已关闭，但 fixture 不是物理 DEVICE/GATEWAY、正式证书或数据资格。仍需正式校准下的现场窗口、至少 200 个复核批次、7 个生产日、切分组/类别覆盖、连续 7-14 天、外部 ERP/WMS、整站灾备与 MLflow 生产 RBAC/SSO/TLS/HA；下一步接真实 `MapleTcT/iot` 来源并重跑 v2 readiness，不得把 G-021 改为 READY。 |
