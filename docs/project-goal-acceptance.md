...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`make sustainable-check` 会校验生成物忽略规则并阻断误跟踪缓存/构建产物；`metadata/ci-required-file-inventory.json` 当前记录 742 个 CI/治理/runtime 依赖资产，其中 742 tracked、0 untracked、0 missing references，覆盖部署、smoke、PostgreSQL、BPI 事件/API 契约、Java 17/PostgreSQL 影子服务、BPI 操作台浏览器验收与模拟验收、迁移、运行补丁、业务模块、ProcessAnalysis 验收、核心链回归、blocker/backlog 和 verifier；`make ci` 包含 strict tracking，必需文件游离在 Git 外会直接失败 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=972`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 972 条已分类引用、0 条未分类引用；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
