...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1636 个 CI/治理/runtime 依赖资产，其中 1636 tracked、0 untracked、0 missing references。本轮纳入 PostgreSQL QCS 显示兼容迁移 194-196、配置字段同步迁移 197-198、默认关闭的 Java 8 QCS quality-gate sidecar、精确 BPI batch resolver、BPI Flyway V25、query-first WMS adapter、目标 QCS/Kafka/material-wms/PostgreSQL/browser 历史验收、QCS 列表/编辑/只读页和双 LIMS provider 冷启动证据、WMS 原命令核对与完工入库冲销（含目标正式身份双会话证据）、material-wms 故障恢复、受控强制结束及正式双管理员 ADP 会话、真实目标 QCS 页面到 Kafka/BPI 的首发和幂等重放、BPI Service/Adapter/Flink 真实负载整栈回切，以及实体模型字段生命周期、23 类 PostgreSQL 标量类型矩阵、独立 `OBJECT` 关联、受控字段删除和 WOM public endpoint 退役验收资产。受控 QCS 激活已验收并恢复关闭；连续生产启用和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `210 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。独立不良数量、PATROL、material/WMS、ProcessAnalysis、生产导出、二维码、手工新建、RM Web 编辑、旧 public endpoint 退役及 BPI/QCS 链均有真实证据；实体模型字段类型 `36/36`、`OBJECT` 关联 `10/10`、受控字段删除 `18/18` 及目标正式身份完工入库冲销已补齐页面/API/PostgreSQL/回滚/清理链。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准、7-14 天影子验收及外部 ERP/WMS 查单/冲销/补偿演练。 |
