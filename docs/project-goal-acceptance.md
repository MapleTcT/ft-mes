...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；拓扑/规则产品化已通过 Flyway V1-V9 和目标页面验收；2026-07-15 又升级到 Flyway V12，以 marker `ADP_E2E_20260715_POINTCAT_02` 完成真实页面点位快照导入、幂等重放、拓扑快照绑定、四项失败关闭、PostgreSQL 审计和重启读取；`ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；`MapleTcT/iot@357a285b` 目标 EventBus marker 已到 Kafka/Flink source；MES production context 已通过真实 WOM 页面、outbox、Kafka/Flink join 和影子批次验收 | 试点设备仍未注册/激活且 metadata/标定/sequence 未就绪；IoT + MES context 同 marker candidate/batch、版本比较/审批/产品级回退、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
...
1. 把已通过的 BPI 同一 marker 联合验收、目标环境 Flyway V12 点位目录和 topology/rule 产品化验收固化为发布回归基线，继续补版本比较、审批和产品级回退。
