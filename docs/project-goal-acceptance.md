...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；本轮纳入 BPI Flyway V35 最新遥测投影、Phase 3C-F 真实页面证据、Phase 3C-G Parquet v2/Iceberg exact-version 证据、最小输送单元真实整链证据、MES 制造到追溯总流程脚本/迁移/机器证据、工厂架构新增产线真实页面/API/PostgreSQL/WOM 引用证据、果糖喷射液化至糖化试运行整链资产、2026-07-28 标准核心链路验收报告/机器记录、2026-07-29/30 QCS 质检交互及质量标准参照证据、2026-07-30 组织岗位及人员新增入口证据、QCS 手工新增/WTS 配置回归、QCS/WTS 工作流编辑器兼容回归、QCS 检验报告配置回归、WOM 手工新增鉴权边界回归、WTS 作业票工作流配置入口弹窗拦截回退回归、审批配置与 JSA 工作流模块级证据和当前实现版产品交互说明书，并保留 V26-V35 数据集、MLflow、工艺窗口、MQTT 落表、遥测投影与权限基线。`metadata/ci-required-file-inventory.json` 当前记录 2027 个必需文件：2027 tracked、0 untracked、0 missing references。连续生产启用、物理现场窗口与数据量、整站灾备、MLflow 生产治理和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=1635`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`，PostgreSQL 迁移数为 `223`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 1635 条已分类引用、0 条未分类引用。新增工厂架构迁移 211-216 只使用 PostgreSQL 幂等路径；迁移 217 仅更新飞天品牌主题配置；迁移 218 通过 PostgreSQL 大对象运行元数据恢复四个 QCS 布局；迁移 219-221 恢复来料检验运行时、产品/来料手工新增入口，并将 WTS 全模块设计态与项目草稿态元数据转换为 PostgreSQL 大对象兼容格式；迁移 222 将 QCS 配置扩展视图转换为有效 PostgreSQL 大对象引用；迁移 223 修复 QCS 检验报告已发布及草稿配置中 Hibernate `@Lob` 字段，均未引入 Oracle 默认路径；V34 文档政策引用均归类为 `documentation-or-workflow`，验收 SQL 的 PostgreSQL `decode(..., 'hex')` 归类为 `postgres-conversion-tooling`；既有 Spring JDBC `rowNum` 回调仍归类为 `java-row-index-noise`，不是 Oracle 默认路径。PATROL 运行元数据引用归类为 `postgres-compat-reference`，真实 SQL 方言仍被阻断；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `288 PASS / 0 FAIL / 0 BLOCKED / 18 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V35 已闭合数据集/遥测链。工具栏最新复验 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`（task `9000006343993284`，`2026-06-22T13:21:24.930Z`）继续保留。2026-07-27 最小输送单元以五个持续信号闭合 BPI 影子批次；MES 以 `...CLOSED_10` 保留完全不续跑基线，并以最新 `...CLOSED_11` 在透明记录预启动分页恢复后完成制造指令、工序、投料及消耗、产出及产出台账、请检、合格、完工、WMS 入库和追溯八步真实页面/API/PostgreSQL 闭环。2026-07-28 marker `ADP_E2E_20260728042954_FACTORY_LINE` 又以纯可见页面交互和 `13/13 PASS` 完成工厂架构新增产线、提交接口、PostgreSQL 和 WOM 生产线参照联合验收；marker `ADP_E2E_FRUCTOSE_LINE_01` 进一步以 `TEST_ONLY_DRAFT` 完成淀粉浆、液化液、糖化液、喷射液化、糖化、九项检验定义、工序交接、QCS 放行、35.1 t 完工入库、批次谱系和 BPI 信号的五页面真实复验。2026-07-29 QCS 首轮交互复验恢复申请列表动作与编辑布局，并验证报告、处置、放行及空态检验记录页面；2026-07-30 又通过真实公网页面闭合产品/来料检验手工新增入口、组织新增部门入口和 WTS 作业许可、审批配置和 JSA 工作流/菜单读取，并顺序通过 WTS 23/23 个实体菜单接口；同日 QCS/WTS 工作流编辑器兼容回归恢复 Linux 混合大小写语言资源与 PostgreSQL 大对象 XML 解码，分别渲染 20/88 个流程图元素，源码/设计切换、缩放和重开通过且数据库指纹不变。随后 QCS 检验报告 8 个配置入口全部恢复，迁移幂等且浏览器无异常；WOM `新建指令单` 修复文档请求 401，同时保持数据接口鉴权，marker `ADP_E2E_20260730_1235_WOM_MANUAL_AUTH` 以 `9/9 PASS` 完成创建、查库、提交和软删除回滚；WTS 作业票 `fireWorkWF` 配置入口在正常新页和弹窗拦截回退两种路径下均打开动火安全作业流程，未选择行时仍保留提示，API 全部 200 且数据库指纹不变。配置读取不写业务数据，记为 `NOT_APPLICABLE`；WOM 新增真实落库记为 `PASS`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；果糖质量限值仍需工艺和 QA/QC 正式批准；检验记录当前无受控业务行，设置检测日期、跳批和删除的真实写入尚待专项；现场 Batch/DCS 切换确认、连续运行和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `306` 项，其中 `288 PASS / 0 FAIL / 0 BLOCKED / 18 NOT_APPLICABLE`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V35、最小输送单元 START/END 影子批次、`...CLOSED_11` 的 WOM/QCS/WMS/ProcessAnalysis 主链、`...FACTORY_LINE` 的产线新增/参照链和 `ADP_E2E_FRUCTOSE_LINE_01` 的两工序物料谱系、质量放行、WMS/BPI 闭环均有真实落库证据；投料路径生成活动执行和物料消耗事实，产出路径精确生成两条产出事实。迁移 100/206 通过行锁保护运行视图更新并回收替换的大对象，迁移 211-216 闭合工厂树、编辑、节点类型参照、助记码别名与运行元数据，迁移 218 恢复四个 QCS 运行布局，迁移 219-221 恢复来料检验运行时、产品/来料手工新增入口，并闭合 WTS 23 个实体的设计态 LOB/project 元数据兼容；迁移 222 闭合 QCS 配置扩展视图 PostgreSQL 大对象兼容；迁移 223 将 QCS 检验报告已发布及草稿配置中 Hibernate `@Lob` 字段转换为有效 PostgreSQL 大对象引用，并通过 8/8 配置页及幂等重放。WTS 作业票工作流配置入口本轮仅读取工作流元数据，浏览器/API/数据库指纹复验均无写入。当前迁移账本为 223 个脚本、0 个高风险语句、87 个受控观察项、0 个观察项安全问题，且 mapper 审计错误为 0。Phase 3C-G 复用 V32/V28 表契约，不新增 Oracle 路径或重复业务表。 | 当前已登记 PostgreSQL 缺口已进入幂等 SQL 或明确产品边界；不能证明未来发现项会自动闭环，需要持续记录 |
