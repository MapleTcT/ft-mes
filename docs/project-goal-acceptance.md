...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `229 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26 manifest、V27 Parquet 和 V28 Iceberg/Polaris 已分别闭合目标页面、API、PostgreSQL、对象存储、失败重试/恢复和定向清理。V28 三组 marker 又证明真实桌面/移动发布、同任务失败重试、post-commit fencing 恢复到同一单一 snapshot。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准、7-14 天影子验收、WORM/MLflow/模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `232` 项，其中 `229 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束、标量/OBJECT 字段生命周期、BPI V26/V27/V28 均有真实落库证据；V28 目录发布事实、审计、幂等和 fencing 独立保存在 PostgreSQL，并与精确 MinIO/Polaris snapshot 对账，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 Phase 3A/3B 数据集链已有真实目标证据。clean V28 release `b7356aa07496` 已由 marker `ADP_E2E_BPI_ICEBERG_20260722_175829_A1` 闭合真实桌面/移动发布、精确 MinIO version/SHA 和 Iceberg snapshot `1863646729883880222` 的 1 row/1 file 对账；retry marker 从 `FAILED/r3/SOURCE_OBJECT_ERROR` 复用同一任务到 `READY/r7/attempt2`；fencing marker 在 commit 后 exit 86，原版 publisher 将 `COMMITTING/r2` 恢复到 `READY/r6/attempt2`，始终只有 snapshot `3771508441673321637` 一条。浏览器错误为 0，publisher drop 为 403，marker/对象/table 已清零且 sidecar/七开关默认关闭。 | Phase 3B-B 纵切已闭合，但物理 DEVICE/GATEWAY、正式证书、连续 7-14 天、外部 ERP/WMS、WORM/Object Lock、MLflow、Phase 4 模型、容量/备份恢复和生产激活仍未完成；G-021 不得改为 READY。 |
