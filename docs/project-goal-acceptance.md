...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `217 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。独立不良数量、PATROL、material/WMS、ProcessAnalysis、生产导出、二维码、手工新建、RM Web 编辑、旧 public endpoint 退役及 BPI/QCS 链均有真实证据；实体模型字段类型 `36/36`、`OBJECT` 关联 `10/10`、受控字段删除 `18/18`，以及目标正式身份完工入库冲销、内部 material-wms 蓝红 durable-receipt 和 V26 数据集清单目标整链均已补齐页面/API/PostgreSQL/清理证据。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准、7-14 天影子验收及外部 ERP/WMS 查单/冲销/补偿演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `220` 项，其中 `217 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束 `33/33`、23 类标量 `36/36`、`OBJECT` 关联 `10/10`、受控字段删除 `18/18` 和 BPI V26 数据集清单均已通过；字段删除使用应用自有 `DROP COLUMN RESTRICT`，并证明依赖/漂移保护及元数据失败时 DDL/数据回滚，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已通过明确拒绝和 marker `0 -> 0` 按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
