...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1875 个 CI/治理/runtime 依赖资产，其中 1875 tracked、0 untracked、0 missing references。本轮纳入 PostgreSQL QCS 显示兼容迁移 194-196、应用持有的字段同步迁移 197、追加式 material-wms 完工入库冲销迁移 198、默认关闭的 Java 8 QCS quality-gate sidecar、精确 BPI batch resolver、BPI Flyway V26 数据集清单、V27 Parquet materializer、V28 Iceberg/Polaris catalog publisher、V29 Object Lock retention archiver、V30 MLflow Dataset Input registrar、V31 离线训练准备度评估、V32 工艺信号窗口与 V33 函数权限加固，以及真实页面/API/PostgreSQL/MinIO/Polaris/MLflow 验收资产。内部 WMS 受控激活已验收并恢复关闭；V26-V33 已闭合 manifest、Parquet、Iceberg 页面/fencing、Object Lock 恢复包、MLflow Dataset Input、19 项训练前门槛、受控 point-in-time 窗口、最小权限、隔离恢复/物理清除和零残留退场；连续生产启用、物理现场窗口与数据量、整站灾备、MLflow 生产治理和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=1624`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`，PostgreSQL 迁移数为 `198`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 1624 条已分类引用、0 条未分类引用。V26 的政策文档引用和 V29“Oracle 不参与 Object Lock 恢复链”说明归类为 `documentation-or-workflow`，`DatasetPostgresRepository` 的 8 条 Spring JDBC `rowNum` 回调参数和 V27 `DatasetMaterializationPostgresRepository` 的 2 条同类回调均归类为 `java-row-index-noise`，不是 Oracle 默认路径；`ShadowRunPostgresRepository`、`FeatureFlagPostgresRepository`、来源序列、点位校准、`BatchReleasePostgresRepository`、`WmsInboundReversalPostgresRepository` 及 `ProcessSignalWindowPostgresRepository` 的 Spring JDBC `rowNum` 回调参数同样归类为 `java-row-index-noise`；PATROL 运行元数据引用归类为 `postgres-compat-reference`，JavaScript `rowNum` / `Number(...)` 均按非 SQL 噪声分类，真实 SQL 方言仍被阻断；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `260 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V33 已分别闭合 manifest、Parquet、Iceberg/Polaris、Object Lock 恢复包、MLflow Dataset Input、训练准备度评估、受控工艺窗口与权限加固的目标页面、API、PostgreSQL、外部存储、失败重试/恢复、最小权限、重启与定向清理。Phase 3C-D marker `ADP_E2E_BPI_FIELD_COVERAGE_20260723_1525_A3` 又证明六项固定来源均为 `2/2`，训练数据如实保持 `0/200、0/7、0/100、0/10`，桌面/移动零错误、取消落库和 12 类精确清理通过。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `263` 项，其中 `260 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V33 均有真实落库证据；V31 readiness、V32 process-window facts、V33 function ACL、Phase 3C-D 来源/训练覆盖、审计/幂等和不可变 trigger 分别由 PostgreSQL 复验，并与 MLflow/MinIO/Object Lock 证据对账，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 V26-V33 数据集链已有真实目标证据。V31 marker `ADP_E2E_BPI_READINESS_20260723_091500_A1` 保留两次不可变 `BLOCKED` 历史评估；V32/V33 release `f7db2f98e82d` 又由 marker `ADP_E2E_BPI_WINDOWS_20260723_1235_A1` 固化 6 条工艺窗口事实。Phase 3C-D release `a73a53a0f4f2` 和 marker `ADP_E2E_BPI_FIELD_COVERAGE_20260723_1525_A3` 将固定来源与训练数据量拆开：来源六项 `2/2/READY`，训练覆盖 `0/200、0/7、0/100、0/10/BLOCKED`；真实页面、API、PostgreSQL、移动几何、取消审计/幂等和 12 类清理全部通过。MLflow/model 路径仍为 0/false。 | Phase 3C-C/D 的受控窗口和覆盖投影实现已关闭，但 fixture 不是物理 DEVICE/GATEWAY、正式证书或真实数据资格。仍需正式校准下的现场窗口、至少 200 个不同复核批次、7 个生产日、足量 accepted/rejected START 标签、切分组/类别覆盖、连续 7-14 天、外部 ERP/WMS、整站灾备与 MLflow 生产 RBAC/SSO/TLS/HA；下一步接真实 `MapleTcT/iot` 来源并重跑 v2 readiness，不得把 G-021 改为 READY。 |
