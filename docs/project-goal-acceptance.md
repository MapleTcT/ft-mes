...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`make sustainable-check` 会校验生成物忽略规则并阻断误跟踪缓存/构建产物；`metadata/ci-required-file-inventory.json` 当前记录 1012 个 CI/治理/runtime 依赖资产，其中 1012 tracked、0 untracked、0 missing references，覆盖部署、smoke、PostgreSQL、全部可维护 source-module、BPI 事件/API 契约、Java 17/PostgreSQL 影子服务、Java 8 认证适配层、BPI 操作台浏览器验收与模拟验收、遥测 replay staging 与落库验收、事件时间边界规则运行时验收、Java 17/Flink 流式回放和 checkpoint 恢复验收、候选 Protobuf 完整证据落库确认验收、Kafka candidate consumer/DLQ/PostgreSQL 验收、规则发布事务 outbox/PostgreSQL/Kafka 联合验收、失败发布管理员重试审计、规则应用 Kafka/PostgreSQL restart/replay/DLQ、Kafka 4.2/Flink MiniCluster checkpoint/restart 验收和发布/应用状态可见性、受控 Kafka/Flink/PostgreSQL marker 联合回放脚本、IoT 信号到 Protobuf/确认/PostgreSQL 的跨模块运行时验收、规则版本化 watermark/迟到/timeout 语义验收、规则发布 Protobuf/AsyncAPI、point-in-time 生产上下文、keyed 双输入等待、Kafka 元数据解码、规则生命周期、作用域索引路由、稳定 UID、candidate/data-quality/rule-application 三事务 sink、可部署 shaded job、独立 Kafka/Flink Compose、独立 BPI runtime Compose、目标环境验收资产、容量预检、集群 smoke 和 marker-scoped read-committed replay 验收、迁移、运行补丁、业务模块、ProcessAnalysis 验收、核心链回归、blocker/backlog 和 verifier；`make ci` 包含 strict tracking，必需文件游离在 Git 外会直接失败 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=980`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 980 条已分类引用、0 条未分类引用；本轮 README 重写移除了旧文档引用，运行依赖没有增加；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-013 | 生产模块完整功能 | BLOCKED | 生产模块测试矩阵当前 `38 PASS / 5 BLOCKED / 0 NOT_RUN`；WOM/QCS/WTS/WAPS/RM/craftGraph 已覆盖多条真实前端/API 和 PostgreSQL 落库证据；WOM `makeTaskList` 工具栏整排最新真实行点击证据为 `metadata/wom-toolbar-row-smoke.json`：`generatedAt=2026-06-22T13:21:24.930Z`、`status=PASS_WITH_KNOWN_BLOCKERS`、marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`，API 使用 `http://100.99.133.43:18080`，浏览器 E2E 使用 `http://222.88.185.146:18080`，PostgreSQL 回查仍指向同一测试环境；无选中时 `开始/保持/重启/结束/提前放料/请检/生产过程追溯/生成二维码` 8 个按钮均显示 `请先选择一条指令单！` 且不发起行级业务请求；左侧筛选全部兜底、查询、仅查待办、清空、开始、保持、重启、结束、提前放料、请检均已复验，PostgreSQL 回读 `wom_produce_tasks.version=5`、`task_run_state=WOM_runState/runing`、`wom_wait_put_records.proc_report_id=758002551887104`；生产过程追溯和生成二维码已由前端守卫成中文依赖提示，但仍因缺 ProcessAnalysis 服务和 WOM `printManage` 二维码接口保持 BLOCKED；material/ProcessAnalysis 缺包、生产导出缺口和业务包扫描证据仍由对应 metadata 账本校验。 | 剩余 5 条生产用例仍 BLOCKED，另有 RM 外部 Batch 客户端、ProcessAnalysis、独立不良数量范围、导出、WOM 手工创建/导入范围、public `produceTaskCreated` 已显式禁用/待产品确认、WOM 二维码运行包接口等未闭合 backlog。 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个业务页面 smoke 已有证据；生产模块测试矩阵当前 `38 PASS / 5 BLOCKED / 0 NOT_RUN`；落库验收总账当前 `122 PASS / 0 FAIL / 3 BLOCKED / 1 NOT_APPLICABLE`；WOM `makeTaskList` 工具栏动作由 `metadata/wom-toolbar-action-coverage.json`、`metadata/wom-toolbar-row-smoke.json` 和 `make wom-toolbar-action-coverage-check` 持续校验，最新整排真实点击证据 marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、`generatedAt=2026-06-22T13:21:24.930Z`、`status=PASS_WITH_KNOWN_BLOCKERS`，且新增无选中 8 个按钮统一中文提示守卫证据；8 个截图工具栏动作当前 `6 PASS / 2 BLOCKED / 0 NOT_VERIFIED`，其中追溯缺 ProcessAnalysis、二维码缺 WOM `printManage` 接口；business-package-scan、business-dependency-readiness 和 production-export-readiness 继续提供缺包/导出 blocker 可复跑证据。 | 剩余 5 条生产用例、3 个动作级 backlog、生成二维码运行包接口缺失、业务负责人签字未完成。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `127` 项，其中 `123 PASS / 0 FAIL / 3 BLOCKED / 1 NOT_APPLICABLE`；`metadata/production-module-backlog.json` 用 7 个 backlog 项覆盖全部未闭合落库行和工具栏动作 blocker，其中 `0 FAIL_BACKLOG / 7 BLOCKED`；material/WMS 与 ProcessAnalysis 均已通过源码模块和 marker 验收；其余 migration/watch 与模块准入门禁继续由 CI 校验 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 总设计、交互设计、API 目录和工程测试计划已落库；Java 8 adapter、Java 17 PostgreSQL 服务、BPI 操作台、Protobuf/OpenAPI、遥测/规则/拓扑/候选/影子批次、Flink 事件时间/生产上下文/Broadcast State/checkpoint、transactional outbox、失败重试和审计已有本地验证；2026-07-14 目标测试环境已部署独立 runtime 与 streaming Compose，真实 ADP 会话到 BPI 概览的浏览器只读链 PASS，三 broker Kafka、8 topic、Flink 30/30 task、MinIO checkpoint、唯一 marker 单候选回放和带负载 TaskManager 重启恢复 PASS；证据见 `metadata/bpi-test-environment-acceptance.json` | 目标浏览器只读链和 Kafka/Flink 数据面仍是分段证据；尚缺同一 marker 的浏览器规则发布、outbox、Flink application receipt、PostgreSQL APPLIED/audit、候选确认和 batch/evidence/audit 完整写链，以及 IoT exporter、MES production context、7-14 天影子运行、QCS/WMS 写回和训练数据阶段 |
...
1. 在已部署的 BPI 目标环境闭合同一 marker 的 `UI -> Outbox -> Kafka -> Flink checkpoint -> application receipt -> PostgreSQL/audit -> candidate confirm -> batch/evidence/audit`，补真实浏览器、offset、checkpoint、数据库和幂等证据。
2. 为拓扑/规则补可审计创建或导入入口，再接入一条产线的 IoT exporter 与 MES production context，完成点位语义、单位、质量码和 sequence 核对后进入 7-14 天影子运行。
...
- BPI 真实联合验收、目标环境和影子运行门槛未完成前，`G-021` 必须保持 `PARTIAL` 或 `BLOCKED`。
