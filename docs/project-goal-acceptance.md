...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 数据面、transactional outbox、回执和审计已建立；目标环境已完成 Flyway V16。规则退役、savepoint、延迟 candidate、重复目录观测、准入撤销、单 broker 故障和应用组件双向回退均已闭合。2026-07-19 又通过 JetLinks 官方 API 注册激活 `bpi-pilot-product-01` / `bpi-pilot-device-01` 并补齐 `instantFlow` metadata；自动 revision `sha256:8b4fd91...e3148022` 落入 MES，marker `ADP_BPI_UNIT_ALIAS_20260719_1325` 证明 `m³/h` / `m3/h` 等价，真实页面、API、PostgreSQL 和浏览器错误门禁通过。Phase 1 仍保持 shadow-only。 | 目标点位仍为 1 点/0 READY，但平台注册/metadata/单位缺口已关闭；当前只剩现场标定、强制且连续单调 DEVICE/GATEWAY source sequence、真实来源遥测 + MES context 同 scope candidate/batch、真实业务负载下跨组件整体回切、7-14 天影子运行、QCS/WMS 写回和训练数据阶段。 |
