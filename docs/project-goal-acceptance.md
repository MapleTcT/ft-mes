...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1520 个 CI/治理/runtime 依赖资产，其中 1520 tracked、0 untracked、0 missing references。本轮纳入 BPI Flyway V23、material 迁移 192、query-first WMS adapter、受保护升级、Java 8 release/原单核对路由、目标 QCS/Kafka/material-wms/PostgreSQL/browser 全链验收、WMS 原命令核对验收、受保护 material-wms 故障恢复验收资产、截图和新版 verifier。Phase 2 仍默认关闭，外部 QCS 主动事件与外部 ERP/WMS 补偿未完成前不得激活。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台和 Flink 数据面已建立；Phase 1 继续保持 shadow-only。marker `BPI_LIVE_20260720_123058` 已用真实 WOM start/stop、受控 MQTT、Kafka/Flink/PostgreSQL 和真实浏览器闭合同一影子批次。Flyway V23 的 QCS required-inspection、`WAIT_QA/RELEASED/REJECTED`、WMS transactional outbox、durable receipt 和 fail-closed 合同已完成本地与目标回归。受控认证 QCS 已通过目标 Kafka 在 `material-wms` 创建真实单据/明细/事务/库存，并由 durable receipt 将批次推进到 `INBOUNDED/r4`；相同 QCS 重放、强制 Kafka command 重放和真实页面均通过。marker `ADP_E2E_20260721_015610_WMS_RECON` 闭合原 event/key 的管理员核对、幂等和并发控制；marker `ADP_E2E_20260720193226_WMS_OUTAGE` 又真实停止内部 `material-wms`，证明首次命令 DLQ +1 且零物料单据，恢复后由页面重排同一 event/key，只生成一套 `12.345 kg` 完工入库事实并进入 `INBOUNDED/r4`，双库清理为 0。证据在 `metadata/bpi-quality-release-wms-target-acceptance.json`、`metadata/bpi-wms-reconciliation-target-acceptance.json` 和 `metadata/bpi-wms-outage-recovery-target-acceptance.json`。验收后 Phase 2/WMS 开关全关且 allowlist 为 deny-all。 | 当前仍缺物理 DEVICE/GATEWAY 与真实现场证书、真实连续 7-14 天影子运行、外部 QCS 实例主动事件与检验记录所有权联接、外部 ERP/WMS 查单协议、响应丢失、冲销/补偿、真实负载跨组件整体回切和受控生产激活；Phase 3/4 训练数据与建议型模型未开始。 |
