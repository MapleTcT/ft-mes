...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；2026-07-14 目标 runtime/streaming Compose、ADP 会话桥接、三 broker Kafka、Flink/MinIO、TaskManager 恢复通过；同一 marker `ADP_E2E_20260714_091536_BPI_JOINT` 已闭合真实浏览器模拟/发布、outbox、Kafka/Flink `APPLIED`、唯一候选、浏览器确认、影子批次/证据/审计，并完成 typed inactive、marker 清理、消费者关闭和清理后只读复验；证据见 `metadata/bpi-browser-kafka-postgres-joint-acceptance.json` | 受控 Phase 1 技术链已通过，但拓扑/规则产品化创建或导入、真实 `MapleTcT/iot` exporter、MES production context、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
...
2. 接入一条产线的 `MapleTcT/iot` exporter 与 MES production context，完成点位语义、单位、质量码和 sequence 核对后进入 7-14 天影子运行。
