...
| G-007 | 用户/人员/岗位/公司/组管理 | READY | 组织部门/组/岗位/公司/人员 CRUD 已落库；人员勾选创建账号已落库；独立用户管理账号新增/编辑/锁定/解锁/删除已落库；角色新增/编辑/删除、角色绑定/解绑用户已落库 | 保持回归 smoke 和后续数据资源权限补测 |
...
| G-009 | 权限/RBAC | PARTIAL | RBAC authority smoke PASS；角色权限页和用户权限页已用 marker 验收 `personmanage/addPerson` 菜单操作权限新增/删除，`rbac_rolepermission`、`rbac_userpermission` 均有 PostgreSQL 证据 | 数据资源权限 `saveRoleSourceAuth/saveUserSourceAuth` 尚未做 marker 落库验收 |
...
| G-016 | PostgreSQL 运行与迁移治理 | PARTIAL | 74 个 init SQL、PostgreSQL migration index、mapper audit 0 error；`073-auth-user-lock-status-compat.sql` 已覆盖 `auth_user.error_count` 兼容缺口；`074-rbac-roleuser-valid-default.sql` 已覆盖 `rbac_roleuser.valid` 默认值兼容缺口 | 生产数据迁移和每个业务动作落库仍未完成 |
...
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 已纳入 CI；本轮 RBAC 验收发现并闭合 `rbac_roleuser.valid` 默认值缺口 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
2. 补 RBAC 数据资源权限页面的真实前端保存动作与 PostgreSQL 落库验收。
3. 补基础配置页面的写动作落库验收。
