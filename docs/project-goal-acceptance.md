...
| G-013 | 生产模块完整功能 | BLOCKED | 生产模块测试矩阵当前 `37 PASS / 6 BLOCKED / 0 NOT_RUN`；WOM/QCS/WTS/WAPS/RM/craftGraph 已覆盖多条真实前端/API 和 PostgreSQL 落库证据；WOM `makeTaskList` 工具栏整排最新真实行点击证据为 `metadata/wom-toolbar-row-smoke.json`：`generatedAt=2026-06-22T10:31:52.950Z`、`status=PASS_WITH_KNOWN_BLOCKERS`、marker `ADP_E2E_20260622103057_WOMSTART_HOLD_RESTART`、taskId `9000006242571780`，API 使用 `http://100.99.133.43:18080`，浏览器 E2E 使用 `http://222.88.185.146:18080`，PostgreSQL 回查仍指向同一测试环境；无选中时 `开始/保持/重启/结束/提前放料/请检/生产过程追溯/生成二维码` 8 个按钮均显示 `请先选择一条指令单！` 且不发起行级业务请求；左侧筛选全部兜底、查询、仅查待办、清空、开始、保持、重启、结束、提前放料、请检均已复验，PostgreSQL 回读 `wom_produce_tasks.version=5`、`task_run_state=WOM_runState/runing`、`wom_wait_put_records.proc_report_id=757960994657536`；生产过程追溯和生成二维码已由前端守卫成中文依赖提示，但仍因缺 ProcessAnalysis 服务和 WOM `printManage` 二维码接口保持 BLOCKED；material/ProcessAnalysis 缺包、生产导出缺口和业务包扫描证据仍由对应 metadata 账本校验。 | 剩余 6 条生产用例仍 BLOCKED，另有 RM 外部 Batch 客户端、material 物料/库存、ProcessAnalysis、独立不良数/库存边界、导出、WOM 手工创建/导入范围、public `produceTaskCreated` 已显式禁用/待产品确认、WOM 二维码运行包接口等未闭合 backlog。 |
