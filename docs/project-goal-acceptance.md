...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；2026-07-14 目标 runtime/streaming Compose、ADP 会话桥接、三 broker Kafka、Flink/MinIO、TaskManager 恢复通过；同一 marker `ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；`MapleTcT/iot@be89aecf` 已实现失败关闭的 JetLinks 属性事件 exporter，9/9 测试和 38 模块 standalone 打包通过 | 受控 Phase 1 与 exporter 工程链已通过，但拓扑/规则产品化、IoT 试点配置/部署、MES production context、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
...
2. 基于 `MapleTcT/iot@be89aecf` 配置并部署一条试点产线，实现 MES production context outbox，完成点位语义、单位、质量码和 source epoch/sequence 核对后进入 7-14 天影子运行。
