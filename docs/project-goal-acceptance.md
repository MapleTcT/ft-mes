...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`make sustainable-check` 会校验生成物忽略规则并阻断误跟踪缓存/构建产物；`metadata/ci-required-file-inventory.json` 当前记录 948 个 CI/治理/runtime 依赖资产，其中 948 tracked、0 untracked、0 missing references，覆盖部署、smoke、PostgreSQL、全部可维护 source-module、BPI 事件/API 契约、Java 17/PostgreSQL 影子服务、Java 8 认证适配层、BPI 操作台浏览器验收与模拟验收、遥测 replay staging 与落库验收、事件时间边界规则运行时验收、Java 17/Flink 流式回放和 checkpoint 恢复验收、候选 Protobuf 完整证据落库确认验收、Kafka candidate consumer/DLQ/PostgreSQL 验收、IoT 信号到 Protobuf/确认/PostgreSQL 的跨模块运行时验收、规则版本化 watermark/迟到/timeout 语义验收、规则发布 Protobuf/AsyncAPI、point-in-time 生产上下文、keyed 双输入等待、Kafka 元数据解码、规则生命周期、作用域索引路由、稳定 UID、双事务 sink、可部署 shaded job、独立 Kafka/Flink Compose、容量预检、集群 smoke 和 marker-scoped read-committed replay 验收、迁移、运行补丁、业务模块、ProcessAnalysis 验收、核心链回归、blocker/backlog 和 verifier；`make ci` 包含 strict tracking，必需文件游离在 Git 外会直接失败 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
