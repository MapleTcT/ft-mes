...
| G-013 | 生产模块完整功能 | PARTIAL | 生产模块测试矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`、44 条 route smoke 全部 PASS，模块 blocker 与 active backlog 均为 0。marker `ADP_E2E_20260727_MES_FULL_CLOSED_10` 已从空 marker 连续闭合制造指令、工序、投料、产出、请检、合格、完工、WMS 入库和追溯；PostgreSQL 证明 2/2 活动执行、投料明细/物料消耗 `1/1`、产出明细 2、QCS `1/1`、WMS `1/2/1` 和追溯快照 3，页面网络、console、page error 与可见 warning/error 均为 0。工具栏最新复验 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`（task `9000006343993284`，`2026-06-22T13:21:24.930Z`）继续保留；独立不良数量、material/WMS、ProcessAnalysis、生产导出、二维码、可见手工新建指令单和 RM Web 配方编辑保持 PASS，旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 退役。 | 仓库内生产主链与投料消耗台账已闭合，但真实现场 Batch/DCS 投递确认、连续运行、回滚演练和业务负责人签字仍是生产切换前置，因此不能称为正式生产完成。 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `280 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V35 已闭合数据集/遥测链。工具栏最新复验 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`（task `9000006343993284`，`2026-06-22T13:21:24.930Z`）继续保留。2026-07-27 最小输送单元以五个持续信号闭合 BPI 影子批次；同日 marker `ADP_E2E_20260727_MES_FULL_CLOSED_10` 又从空 marker、不续跑历史状态，完成制造指令、工序、投料及消耗、产出、请检、合格、完工、WMS 入库和追溯八步真实页面/API/PostgreSQL 闭环。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认、连续运行和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `283` 项，其中 `280 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V35、最小输送单元 START/END 影子批次和 `...CLOSED_10` 的 WOM/QCS/WMS/ProcessAnalysis 主链均有真实落库证据；投料路径现在生成活动执行和物料消耗事实，运行按钮通过迁移 207 兼容当前响应包。Phase 3C-G 继续复用 V32/V28 表契约，不新增 Oracle 路径或重复业务表。 | 当前已登记 PostgreSQL 缺口已进入幂等 SQL 或明确产品边界；不能证明未来发现项会自动闭环，需要持续记录 |
...
3. 保持 `ADP_E2E_20260727_MES_FULL_CLOSED_10` 的制造到追溯八步及投料消耗闭环为发布回归；随后用真实主数据、现场 Batch/DCS、连续运行和业务签字复跑，不扩散到低优先级模块。
