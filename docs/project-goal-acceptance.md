...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；本轮纳入 BPI Flyway V35 最新遥测投影、Phase 3C-F 真实页面证据、Phase 3C-G Parquet v2/Iceberg exact-version 证据和当前实现版产品交互说明书，并保留 V26-V35 数据集、MLflow、工艺窗口、MQTT 落表、遥测投影与权限基线。`metadata/ci-required-file-inventory.json` 当前记录 1924 个必需文件：1924 tracked、0 untracked、0 missing references。连续生产启用、物理现场窗口与数据量、整站灾备、MLflow 生产治理和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `266 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V35 已闭合数据集/遥测链，Phase 3C-G 又从真实页面把两个过程窗口交付到 1-row Parquet v2 和指定 Iceberg snapshot，PostgreSQL、exact object、浏览器和精确清理均通过。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `269` 项，其中 `266 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V35 均有真实落库证据；Phase 3C-G 复用 V32/V28 表契约，把冻结的 PostgreSQL 窗口事实交付到 Parquet v2/Iceberg，不新增 Oracle 路径或重复业务表。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 V26-V35 数据/遥测链已有真实目标证据。V31 保留不可变 `BLOCKED` 历史评估，V32-V35 已闭合工艺窗口、现场覆盖、受控 MQTT 和实时事实。Phase 3C-G revision `007616b290ef` 又从真实数据集页将两个冻结窗口交付到 `bpi.dataset-parquet.v2` 和指定 Iceberg snapshot：Parquet 1 row/27 fields、Iceberg 1 row/33 fields，值为 `20.000000/0.500000`，exact version/SHA/semantic checksum、PostgreSQL 审计/幂等和浏览器均通过；表、namespace、7 个对象版本和 20 类 marker 投影清零，五个开关 false。 | Phase 3C-C/D/E/F/G 的受控实现已关闭，但 fixture 和临时校准不是物理 DEVICE/GATEWAY、正式证书、容量或真实数据资格。仍需物理来源断电重连、正式校准、多点/多产线容量、至少 200 个复核批次、7 个生产日、正负标签、连续 7-14 天、外部 ERP/WMS、整站灾备与 MLflow 生产 RBAC/SSO/TLS/HA；Phase 3D-A 对当前数据必须失败关闭，不得把 G-021 改为 READY。 |
