...
| G-012 | 基础配置 | PARTIAL | 系统编码与普通 app 系统配置 CRUD 已完成 marker 落库；内置目录 7/7 只读 smoke 通过，QCS/RM/BaseSet 各有受控单项配置保存、回读、回滚和代表性业务回归；customProperty 模型映射已完成启用/编辑/恢复/禁用落库；`ADP_E2E_20260721_1240_ENTITY_MODEL_PHYSICAL_TABLE` 已完成实体/模型、物理表和 12 列幂等验收；`ADP_E2E_20260721053537_PG_FIELD` 又在 `10.11.100.17` 真实 `/msService/ec/engine/msManage` 页面完成 TEXT 列/索引创建、marker 行、重命名、安全扩容、幂等重放、危险转换事务回滚及 0/0/0/0 清理，证据 `metadata/entity-model-field-persistence-acceptance.json`，实现提交 `c1bbaae8`；动作矩阵为 `8 PASS / 4 READ_ONLY_GUARDED / 3 CONTROLLED_MARKER_REQUIRED / 2 PLANNED`，覆盖账本为 `5 PASS / 2 PARTIAL` | 身份、授权、密钥、密码类配置仍只读；其他 BaseSet/RM/QCS 配置项、自动删列、索引删除、唯一/非空约束变更和更广字段类型矩阵仍需专用 marker；Nacos/Keycloak 生产 export/diff、realm 迁移、secret 轮换、登录 smoke 和回退演练未完成 |
