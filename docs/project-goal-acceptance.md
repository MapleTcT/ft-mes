...
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `283` 项，其中 `280 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V35、最小输送单元 START/END 影子批次和 `...CLOSED_11` 的 WOM/QCS/WMS/ProcessAnalysis 主链均有真实落库证据；投料路径生成活动执行和物料消耗事实，产出路径精确生成两条产出事实。迁移 100/206 通过行锁保护运行视图更新并回收替换的大对象，运行按钮继续兼容当前响应包。Phase 3C-G 复用 V32/V28 表契约，不新增 Oracle 路径或重复业务表。 | 当前已登记 PostgreSQL 缺口已进入幂等 SQL 或明确产品边界；不能证明未来发现项会自动闭环，需要持续记录 |
