...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；2026-07-14 目标 runtime/streaming Compose、ADP 会话桥接、三 broker Kafka、Flink/MinIO、TaskManager 恢复通过；marker `ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；`MapleTcT/iot@357a285b` 已通过 14 个 Java 测试、7 个部署脚本测试和 40 模块打包，目标 JetLinks EventBus marker `ADP_BPI_E2E_20260714_145738_757314` 已到 Kafka partition 4 offset `3 -> 4`、Flink source offset `4/4`、lag `0`；MES production context 已通过真实 WOM 页面、outbox、Kafka/Flink join 和影子批次验收 | 受控 Phase 1、IoT source 与 MES context 分段链已通过，但拓扑/规则产品化、真实网关/协议设备点位、IoT + MES context 同 marker candidate/batch、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
...
1. 把已通过的 BPI 同一 marker 联合验收固化为发布回归基线，并补 topology/rule 产品化创建或导入入口。
2. 基于 `MapleTcT/iot@357a285b` 和已通过的 MES production context 配置一条试点产线；受控 EventBus 到 Flink source 已通过，下一步接入真实网关/协议设备点位，完成点位语义、单位、质量码、source epoch/sequence、WOM scope/state 和 locality group 对账，并用同一 marker 闭合 candidate/batch 后进入 7-14 天影子运行。
