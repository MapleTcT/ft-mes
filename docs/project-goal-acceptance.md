...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；拓扑/规则产品化已通过 Flyway V1-V9；Flyway V12 点位准入先以 marker `ADP_E2E_20260715_POINTCAT_02` 完成页面写链，随后由 `MapleTcT/iot@786d153a` 与 MES `79ee289d` 以 revision `sha256:2a218d12...151ce5` 闭合 JetLinks/Kafka/PostgreSQL/DLT/重启/浏览器自动目录同步；`ADP_E2E_20260715_0532_BPI_SOURCE_SEQUENCE` 又以本地 PostgreSQL 16.13、8 条浏览器 E2E、目标 PostgreSQL 15.18 和真实 ADP 页面证明设备/网关来源序列是 READY 和拓扑发布的硬门槛，目标点位保持 0 READY；`ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；MES production context 已通过真实 WOM 页面、outbox、Kafka/Flink join 和影子批次验收 | 自动同步控制链已 PASS，但试点设备仍未注册/激活且 metadata/标定/sequence 未就绪；真实遥测 + MES context 同 marker candidate/batch、版本比较/审批/产品级回退、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
