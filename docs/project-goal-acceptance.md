...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1790 个 CI/治理/runtime 依赖资产，其中 1790 tracked、0 untracked、0 missing references。本轮纳入 PostgreSQL QCS 显示兼容迁移 194-196、应用持有的字段同步迁移 197、追加式 material-wms 完工入库冲销迁移 198、默认关闭的 Java 8 QCS quality-gate sidecar、精确 BPI batch resolver、BPI Flyway V26 数据集清单、V27 Parquet materializer、V28 Iceberg/Polaris catalog publisher、V29 Object Lock retention archiver 与隔离 recovery rehearsal、私有版本化 MinIO 编排、query-first WMS adapter，以及真实页面/API/PostgreSQL/MinIO/Polaris 验收资产。内部 WMS 受控激活已验收并恢复关闭；V26-V29 已闭合 manifest、Parquet、Iceberg 页面/fencing、Object Lock 恢复包、最小权限、隔离恢复/物理清除和零残留退场；连续生产启用、整站灾备和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `238 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26 manifest、V27 Parquet、V28 Iceberg/Polaris 和 V29 Object Lock 恢复包已分别闭合目标页面、API、PostgreSQL、对象存储、失败重试/恢复、最小权限和定向清理。V29 marker 还证明隔离 time-travel 恢复、恢复副本物理清除、原 publication 不变和桌面/移动零错误。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准、7-14 天影子验收、整站灾备/容量、MLflow/模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `241` 项，其中 `238 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V29 均有真实落库证据；V29 archive 事实、r1-r7 审计、幂等、两个 exact retained versions 和恢复校验独立保存在 PostgreSQL，并与 MinIO/Polaris 对账，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 V26-V29 数据集链已有真实目标证据。V28 clean release `b7356aa07496` 已闭合真实桌面/移动 Iceberg 发布、失败重试和 post-commit fencing 恢复到同一 snapshot。V29 release `ef8036b8b717` 又由 marker `ADP_E2E_BPI_ARCHIVE_20260722_215300_A1` 闭合页面 `FAILED -> retry -> LOCKED/r7/attempt2`、PostgreSQL r1-r7/幂等、两个 GOVERNANCE exact versions、archiver/recovery operator 最小权限、隔离 recovery snapshot `4888963949559974798` 的 1-row time-travel、恢复 table/namespace/6 个物理版本清除和原 snapshot `2413939455193407789` 不变；浏览器错误、marker、对象和可选 sidecar 均归零，九个相关开关 false。 | 单数据集 Object Lock 恢复包门槛已关闭，但物理 DEVICE/GATEWAY、正式证书、连续 7-14 天、外部 ERP/WMS、整站灾备/生产留存策略/RPO-RTO/容量、MLflow、Phase 4 模型和生产激活仍未完成；G-021 不得改为 READY。 |
