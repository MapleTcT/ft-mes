...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1434 个 CI/治理/runtime 依赖资产，其中 1434 tracked、0 untracked、0 missing references，覆盖全部可维护 source-module、PostgreSQL 迁移、真实验收和 verifier；本轮新增 BPI Flyway V22、来源序列 Kafka/MES/IoT 代码、受控 MQTT 联合验收脚本/文档/机器证据，并继续以 strict tracking 阻止必需文件游离在 Git 外。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=1598`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 1598 条已分类引用、0 条未分类引用；此前 `ShadowRunPostgresRepository` 的 6 条、`FeatureFlagPostgresRepository` 的 2 条以及本轮来源序列 repository/test 的 2 条 Spring JDBC `rowNum` 回调参数均归类为 `java-row-index-noise`，另 1 条新增引用为 PostgreSQL-first 治理说明；PATROL 运行元数据引用归类为 `postgres-compat-reference`，JavaScript `rowNum` / `Number(...)` 均按非 SQL 噪声分类，真实 SQL 方言仍被阻断；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `180 PASS / 0 FAIL / 1 BLOCKED / 2 NOT_APPLICABLE`。独立不良数量 marker `ADP_E2E_20260717141017_WOM_BAD_QTY` 已形成 WOM/QCS/WMS/PostgreSQL 完整证据；历史 WOM 工具栏基线 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z` 继续保留。PATROL 计划/取消、配置 CRUD、执行、异常隐患、统计/监控和 Kafka 消费均有真实证据；material/WMS、ProcessAnalysis、生产列表导出、二维码、手工新建和 RM Web 编辑已闭合；BPI V15 受控退役、延迟 candidate、V21 运行开关及旧 MES 原生菜单门禁已补入同一落库验收总账；本轮又用两次独立 MQTT QoS1 会话完成 JetLinks 双表、Kafka/Flink、MES 来源序列/数据质量 PostgreSQL 和真实页面联合验收。 | PATROL 技术消费链已通过，但测试库 `tmm_tags=0` 且无测点历史，无法验收 `gather_data` 中位数落库和误差图表；GIS 定位/轨迹缺 `SESGISConfig`，完整整改/复查/销项缺真实 SESH；public `produceTaskCreated` 产品范围决定、现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、批准校准和 7-14 天影子验收。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `183` 项，其中 `180 PASS / 0 FAIL / 1 BLOCKED / 2 NOT_APPLICABLE`；迁移库存为 `191 scripts`，191 新增 WOM 报告/事件账本、WMS 分配/事件账本、合格/冻结数量拆分和 WOM/QCS 可见入口；BPI Flyway V22 由独立 Java 17 服务维护，不计入 ADP init SQL 的 191 项库存，并新增来源序列 current evidence、数据质量 incident/raw event、inbox 和审计落库；`metadata/production-module-backlog.json` 当前只保留 1 个 public `produceTaskCreated` 产品范围项，其中 `0 FAIL_BACKLOG / 1 BLOCKED`。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、Flyway V22、操作台、Flink 数据面、transactional outbox、回执和审计已建立；规则生命周期、savepoint、目录与校准治理、数据质量自动链、影子运行、运行开关和应用回退均已闭合。2026-07-20 受控 MQTT 双会话以 `2026072003:5001..5003` 和重连后的 `2026072004:1..3` 完成 6/6 PUBACK、JetLinks 双表、exporter +6、Kafka `37 -> 43`、Flink `RUNNING 36/36`、MES 来源序列 current、六条数据质量 raw event 和真实点位/数据质量页面联合验收；验收窗口内序列为 `QUALIFIED/DEVICE`，点位仅因校准保持 0 READY，candidate/batch=0。证据在 `metadata/bpi-mqtt-ingress-joint-acceptance.json`。Phase 1 仍保持 shadow-only。 | 来源是受控模拟器且证据 30 分钟后过期；当前仍缺与目录版本精确匹配的真实现场证书、物理 DEVICE/GATEWAY 同等序列与断电恢复证据、真实来源遥测 + MES context 同生产指令 START/END candidate/batch、真实业务负载下跨组件整体回切、真实连续 7-14 天影子运行、QCS/WMS 写回和训练数据阶段。 |
...
1. 把已通过的 BPI 同一 marker 联合验收、目标环境 Flyway V18、自动点位目录、校准稳定分页、topology/rule
   产品化、版本比较、职责分离审批、受控退役、savepoint、单 broker 故障和应用组件双向回退固化为
   发布回归基线，继续补真实业务负载下的跨组件整体回切。
2. JetLinks 产品/设备、`instantFlow` metadata 和单位已经闭合；下一步由现场审核标定，把映射切为强制来源序列，并用真实 DEVICE/GATEWAY epoch + 连续单调 sequence 形成未过期证据。自动目录进入 READY 后，再与 MES production context 用同一 marker 闭合 candidate/batch，随后进入 7-14 天影子运行。
