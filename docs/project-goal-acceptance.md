...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1710 个 CI/治理/runtime 依赖资产，其中 1710 tracked、0 untracked、0 missing references。本轮纳入 PostgreSQL QCS 显示兼容迁移 194-196、应用持有的字段同步迁移 197、追加式 material-wms 完工入库冲销迁移 198、默认关闭的 Java 8 QCS quality-gate sidecar、精确 BPI batch resolver、BPI Flyway V26 数据集清单工作台与 V27 Parquet materializer、私有版本化 MinIO 编排、query-first WMS adapter、目标 QCS/Kafka/material-wms/PostgreSQL/browser 历史验收、QCS 列表/编辑/只读页和双 LIMS provider 冷启动证据、WMS 原命令核对与完工入库冲销（含目标正式身份双会话和内部蓝红 durable-receipt 整链证据）、material-wms 故障恢复、受控强制结束及正式双管理员 ADP 会话、真实目标 QCS 页面到 Kafka/BPI 的首发和幂等重放、BPI Service/Adapter/Flink 真实负载整栈回切，以及实体模型字段生命周期、23 类 PostgreSQL 标量类型矩阵、独立 `OBJECT` 关联、受控字段删除和 WOM public endpoint 退役验收资产。内部 WMS 受控激活已验收并恢复关闭；V26 `MANIFEST_ONLY` 与 V27 版本锁定 Parquet 均已有目标页面/API/PostgreSQL/MinIO/清理证据；连续生产启用和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=1621`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`，PostgreSQL 迁移数为 `198`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 1621 条已分类引用、0 条未分类引用。V26 的 1 条政策文档引用归类为 `documentation-or-workflow`，`DatasetPostgresRepository` 的 8 条 Spring JDBC `rowNum` 回调参数和 V27 `DatasetMaterializationPostgresRepository` 的 2 条同类回调均归类为 `java-row-index-noise`，不是 Oracle 默认路径；`ShadowRunPostgresRepository`、`FeatureFlagPostgresRepository`、来源序列、点位校准、`BatchReleasePostgresRepository` 及 `WmsInboundReversalPostgresRepository` 的 Spring JDBC `rowNum` 回调参数同样归类为 `java-row-index-noise`；PATROL 运行元数据引用归类为 `postgres-compat-reference`，JavaScript `rowNum` / `Number(...)` 均按非 SQL 噪声分类，真实 SQL 方言仍被阻断；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-013 | 生产模块完整功能 | PARTIAL | 生产模块测试矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`、44 条 route smoke 全部 PASS，模块 blocker 与 active backlog 均为 0。独立不良数量 marker `ADP_E2E_20260717141017_WOM_BAD_QTY` 已通过 WOM/QCS 真实入口、API 幂等、QCS 关联、WMS 合格/冻结数量分配、PostgreSQL 事件账本、冲销和清理；material/WMS、ProcessAnalysis、生产列表导出、WOM 二维码、可见手工新建指令单和 RM Web 配方编辑继续保持 PASS。旧 public `produceTaskCreated` 已由 marker `ADP_E2E_20260721104747_PUBLIC_PRODUCE_RETIRED` 按 `NOT_APPLICABLE` 退役。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | 仓库内生产功能与产品边界已闭合，但真实现场 Batch/DCS 投递确认、回滚演练和业务负责人签字仍是生产切换前置，因此不能称为正式生产完成。 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `222 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。独立不良数量、PATROL、material/WMS、ProcessAnalysis、生产导出、二维码、手工新建、RM Web 编辑、旧 public endpoint 退役及 BPI/QCS 链均有真实证据；实体模型字段类型 `36/36`、`OBJECT` 关联 `10/10`、受控字段删除 `18/18`，以及目标正式身份完工入库冲销、内部 material-wms 蓝红 durable-receipt、V26 数据集清单和 V27 Parquet 失败恢复/精确对象版本/重启清理整链均已补齐页面/API/PostgreSQL/MinIO/清理证据。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准、7-14 天影子验收及外部 ERP/WMS 查单/冲销/补偿演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `225` 项，其中 `222 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束 `33/33`、23 类标量 `36/36`、`OBJECT` 关联 `10/10`、受控字段删除 `18/18`、BPI V26 数据集清单与 V27 Parquet 物化均已通过；V27 物化事实保存在独立 PostgreSQL 表并保持 manifest 不可变，不引入 Oracle 默认路径。字段删除使用应用自有 `DROP COLUMN RESTRICT`，并证明依赖/漂移保护及元数据失败时 DDL/数据回滚。旧 public `produceTaskCreated` 已通过明确拒绝和 marker `0 -> 0` 按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台和 Flink 数据面已建立；Phase 1 继续保持 shadow-only。marker `BPI_LIVE_20260720_123058` 已用真实 WOM start/stop、受控 MQTT、Kafka/Flink/PostgreSQL 和真实浏览器闭合同一影子批次。Flyway V24 marker `ADP_E2E_20260721053253_BPI_FORCE_CLOSE` 闭合异常人工兜底和职责分离。marker `ADP_E2E_20260721021607_QCS_BPI` 又从真实 WOM/QCS 页面生成生效合格报告，通过同事务 outbox、Java 8 sidecar 和三 broker Kafka 把影子批次推进到 `RELEASED/r3/ACCEPTED`；同一事件重放后 gate/link/inbox/state/audit/WMS-outbox 仍为 `1/1/1/2/2/0`，DLQ/lag 和双库残留均为 0，六开关恢复 false。受控认证 QCS 到 `material-wms` 的完工入库、原命令核对和内部 WMS 停机/DLQ/恢复也已通过。marker `ADP_BPI_INTEGRATED_ROLLBACK_20260721_2054` 进一步用真实候选确认和影子批次负载，同时回退并恢复 Service、Adapter、Flink；恢复前后 ID/revision/计数不变，旧栈与当前栈浏览器 API 全为 200、错误为 0，清理零残留。marker `ADP_BPI_FORMAL_IDENTITY_20260721134944` 又由两个正式 ADP 身份浏览器会话经 Adapter 闭合强制结束 REQUEST/APPROVE。Flyway V25 本地软件链闭合完工入库冲销 accepted/rejected/retry；目标 marker `ADP_BPI_FORMAL_WMS_REVERSAL_20260721175536` 由两个正式 ADP 会话完成 REQUEST、同人 403、独立 APPROVE。最新 marker `ADP_BPI_FORMAL_WMS_REVERSAL_20260721190630` 进一步闭合内部蓝单/红单、durable receipt 和净库存归零。Flyway V26 Phase 3A 数据集清单先在本地 PostgreSQL 与 20 条浏览器回归闭合，再由 marker `ADP_E2E_BPI_DATASET_TARGET_20260722_055000_A7C4` 在目标闭合真实 ADP 页面、API、PostgreSQL、跨工厂隔离、幂等和十类零残留清理。Flyway V27/Phase 3B-A 又已在源码完成 Java 任务 API、Python 3.12 Parquet worker、私有版本桶、精确 `versionId`、SHA 回读、失败重试及桌面/移动交互，本地模拟证据明确标记 `SIMULATED`；目标 marker `ADP_E2E_BPI_PARQUET_20260722_105844_A1` 又闭合真实页面、失败重试、PostgreSQL、MinIO exact versionId/SHA、服务重启、最小权限和零残留清理。 | 当前仍缺物理 DEVICE/GATEWAY 与真实现场证书、真实连续 7-14 天影子运行，以及外部 ERP/WMS 实例的消费、查单/响应丢失/拒绝/补偿、durable receipt 和受控生产激活；内部 material-wms PASS 不能替代外部验收。Phase 3B-A 目标纵切已闭合；WORM、Iceberg/MLflow 与 Phase 4 建议型模型仍未完成。 |
