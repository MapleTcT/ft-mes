...
| G-013 | 生产模块完整功能 | BLOCKED | 生产模块测试矩阵当前 `40 PASS / 3 BLOCKED / 0 NOT_RUN`、43 条 route smoke 全部 PASS；material/WMS、ProcessAnalysis 和生产列表导出均已通过真实浏览器/API/PostgreSQL 或文件响应验收。2026-07-17 导出报告在 `http://10.11.100.17:18080` 达到 `READY/6-of-6`，RM 231 条有效 PostgreSQL 数据对应工作簿 232 行（含表头）。最新 WOM 工具栏基线仍为 marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | 3 条生产用例仍 BLOCKED：RM 外部 Batch 客户端和 PROD-019/PROD-021 独立不良数量范围确认；二维码、手工创建/导入及 public `produceTaskCreated` 范围继续作为独立 backlog。 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `40 PASS / 3 BLOCKED / 0 NOT_RUN`；落库总账当前 `163 PASS / 0 FAIL / 3 BLOCKED / 2 NOT_APPLICABLE`。最新 WOM 工具栏基线为 marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。PATROL 计划/取消、配置 CRUD、执行、异常隐患、统计/监控和 Kafka 消费均有真实证据；material/WMS、ProcessAnalysis 与生产列表导出已闭合，导出报告为 `READY/6-of-6`。 | PATROL 技术消费链已通过，但测试库 `tmm_tags=0` 且无测点历史，无法验收 `gather_data` 中位数落库和误差图表；GIS 定位/轨迹缺 `SESGISConfig`，完整整改/复查/销项缺真实 SESH；另有 3 条生产用例、二维码运行端点和业务负责人签字未完成。 |
