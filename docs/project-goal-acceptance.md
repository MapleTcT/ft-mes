...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个业务页面 smoke 已有证据；生产模块矩阵当前 `39 PASS / 4 BLOCKED / 0 NOT_RUN`；落库总账当前 `131 PASS / 0 FAIL / 3 BLOCKED / 2 NOT_APPLICABLE`。WOM 最新工具栏 marker 仍为 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、`generatedAt=2026-06-22T13:21:24.930Z`；material/WMS 与 ProcessAnalysis 已闭合，依赖 readiness 为 `2 READY / 0 BLOCKED`。 | 4 条生产用例、二维码运行端点、生产导出其余目标和业务负责人签字未完成。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `136` 项，其中 `131 PASS / 0 FAIL / 3 BLOCKED / 2 NOT_APPLICABLE`；`metadata/production-module-backlog.json` 用 7 个 backlog 项覆盖全部未闭合落库行和工具栏动作 blocker，其中 `0 FAIL_BACKLOG / 7 BLOCKED`；material/WMS、ProcessAnalysis 与 BPI 自动点位目录均有 marker/内容 revision 落库证据 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；拓扑/规则产品化已通过 Flyway V1-V9；Flyway V12 点位准入先以 marker `ADP_E2E_20260715_POINTCAT_02` 完成页面写链，随后由 IoT/MES 以 revision `sha256:2a218d12...151ce5` 闭合 JetLinks/Kafka/PostgreSQL/DLT/重启/浏览器自动目录同步；`MapleTcT/iot@a6b381ac` 已用 21 项 exporter 测试、40 模块打包和目标 JetLinks 单容器部署锁定强制 DEVICE/GATEWAY 来源序列准入；`ADP_E2E_20260715_0532_BPI_SOURCE_SEQUENCE` 又以本地 PostgreSQL 16.13、8 条浏览器 E2E、目标 PostgreSQL 15.18 和真实 ADP 页面证明来源序列是 READY 和拓扑发布的硬门槛，目标点位保持 0 READY；`ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；MES production context 已通过真实 WOM 页面、outbox、Kafka/Flink join 和影子批次验收 | 自动同步控制链已 PASS，但试点设备仍未注册/激活且 metadata/标定/sequence 未就绪；真实遥测 + MES context 同 marker candidate/batch、版本比较/审批/产品级回退、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
