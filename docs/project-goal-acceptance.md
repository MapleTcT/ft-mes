...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1745 个 CI/治理/runtime 依赖资产，其中 1745 tracked、0 untracked、0 missing references。本轮纳入 PostgreSQL QCS 显示兼容迁移 194-196、应用持有的字段同步迁移 197、追加式 material-wms 完工入库冲销迁移 198、默认关闭的 Java 8 QCS quality-gate sidecar、精确 BPI batch resolver、BPI Flyway V26 数据集清单工作台、V27 Parquet materializer 与 V28 Iceberg/Polaris catalog publisher、私有版本化 MinIO 编排、query-first WMS adapter、目标 QCS/Kafka/material-wms/PostgreSQL/browser 历史验收、QCS 列表/编辑/只读页和双 LIMS provider 冷启动证据、WMS 原命令核对与完工入库冲销（含目标正式身份双会话和内部蓝红 durable-receipt 整链证据）、material-wms 故障恢复、受控强制结束及正式双管理员 ADP 会话、真实目标 QCS 页面到 Kafka/BPI 的首发和幂等重放、BPI Service/Adapter/Flink 真实负载整栈回切，以及实体模型字段生命周期、23 类 PostgreSQL 标量类型矩阵、独立 `OBJECT` 关联、受控字段删除和 WOM public endpoint 退役验收资产。内部 WMS 受控激活已验收并恢复关闭；V26 `MANIFEST_ONLY` 与 V27 版本锁定 Parquet 已有目标页面/API/PostgreSQL/MinIO/清理证据；V28 已完成清理后的目标 PostgreSQL/MinIO/Polaris 后端检查点，真实 ADP 页面和提交后 fencing 验收仍未闭合；连续生产启用和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台和 Flink 数据面已建立；Phase 1 保持 shadow-only。真实 WOM/受控 MQTT START-END、规则/点位/数据质量/影子验收、正式双身份强制结束、QCS 与内部 material-wms 蓝红 durable receipt、回滚恢复均有页面/API/Kafka/PostgreSQL 证据。Flyway V26 marker `ADP_E2E_BPI_DATASET_TARGET_20260722_055000_A7C4` 闭合不可变 manifest；Flyway V27 marker `ADP_E2E_BPI_PARQUET_20260722_105844_A1` 闭合真实页面失败重试、精确 MinIO versionId/SHA、重启和清理。Flyway V28 Phase 3B-B 后端检查点又以 marker `ADP_E2E_BPI_DATASET_961b1001a363487bb5e68a0419c7a23d` 在真实 PostgreSQL/MinIO/Polaris/PyIceberg 生成 Iceberg snapshot `9198617437104218826`，time-travel 为 1 row/1 file；Polaris 重启无重复，缺失源对象两次失败与 retry 幂等、bootstrap 和定向清理均通过。 | Phase 3B-B 仍缺 clean V28 service/adapter/UI 的真实 ADP 桌面/移动 POST/GET/retry 验收，以及 catalog commit 成功但 BPI PostgreSQL fencing 回写失败的真实注入/reconcile。物理 DEVICE/GATEWAY、正式证书、连续 7-14 天、外部 ERP/WMS、WORM、MLflow、Phase 4 模型和生产激活也未完成；G-021 不得改为 READY。 |
