...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 和 admin 全目录扫描已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `293 PASS / 1 FAIL / 0 BLOCKED / 18 NOT_APPLICABLE`。admin 的 533/533 权限节点无操作缺口，但 398 个可导航页面仅 `185 PASS / 42 WARN / 171 FAIL`，其中 169 个为旧包 HTTP 200 空白页。MES、组织、RBAC、PATROL 与 WTS 代表业务落库已通过，WTS 正常封票；消息中心列表读取也已恢复，但在线 admin 场景的站内信 7/7、移动通知 7/7 仍为 `send_status=0`，登记为 `NOTIFY-001`，不能用列表 PASS 代替通知送达 PASS。最新 WOM 工具栏复验 marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、task `9000006343993284`、时间 `2026-06-22T13:21:24.930Z` 继续保留；既有制造到追溯、果糖试运行、QCS/WTS 配置、BPI 数据集与遥测证据也继续保留。 | 先关闭 `NOTIFY-001`，再逐包处理 169 个空白页；PATROL 真实测点历史、果糖 QA/QC 正式限值、现场 Batch/DCS、连续运行、业务签字、BPI 长周期影子验收及外部 ERP/WMS 演练仍未完成。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `312` 项，其中 `293 PASS / 1 FAIL / 0 BLOCKED / 18 NOT_APPLICABLE`。既有 WOM/QCS/WMS/ProcessAnalysis、产线、果糖试运行、BPI 与配置兼容证据继续保留；迁移 224-227 又补齐 TeamInfo admin 权限、OEE 工厂树层级、WTS `oid ~~ bigint` 窄兼容和移动设备 token 表。当前迁移账本为 227 个脚本、0 个高风险语句、87 个受控观察项、0 个观察项安全问题，且 mapper 审计错误为 0。移动 token 缺表已关闭，但真实在线场景仍有 `NOTIFY-001`：站内信 7/7、移动通知 7/7 均为 `send_status=0`，已进入 1 条 `FAIL_BACKLOG`，不能用消息列表 HTTP 200 代替通知送达。 | 修复 msgmanagement 通知 provider 与在线会话链，用新 marker 完成浏览器/API/PostgreSQL 复验；后续每发现新 PostgreSQL 缺口仍需追加幂等 SQL 或 backlog |
