...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1467 个 CI/治理/runtime 依赖资产，其中 1467 tracked、0 untracked、0 missing references；本轮纳入 BPI Flyway V23、QCS 质量放行/WMS 完工入库合同、PostgreSQL/API 验收和 verifier，Phase 2 仍默认关闭，真实 QCS/WMS 与浏览器验收未完成前不得激活。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台和 Flink 数据面已建立；Phase 1 继续保持 shadow-only。marker `BPI_LIVE_20260720_123058` 已用真实 WOM start/stop、受控 MQTT、Kafka/Flink/PostgreSQL 和真实浏览器闭合同一影子批次。源码提交 `22ddadebd20ed9ed5d7efd19c3c0ed49967b9c90` 又新增 Flyway V23、QCS required-inspection snapshot、`WAIT_QA/RELEASED/REJECTED`、WMS transactional outbox、durable receipt 与未知状态 fail-closed；全新 PostgreSQL V1-V23、定向验收 4/4 和完整后端回归 76 项 0 失败，证据在 `metadata/bpi-quality-release-wms-inbound-acceptance.json`。Phase 2 仍默认关闭。 | 当前仍缺物理 DEVICE/GATEWAY 与真实现场证书、真实连续 7-14 天影子运行、真实 QCS/WMS 事件源、Java 8 入口适配、目标部署、浏览器/补偿验收和受控开关激活；Phase 3/4 训练数据与建议型模型未开始。 |
