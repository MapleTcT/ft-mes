...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1508 个 CI/治理/runtime 依赖资产，其中 1508 tracked、0 untracked、0 missing references。本轮纳入 BPI Flyway V23、material 迁移 192、query-first WMS adapter、受保护升级、Java 8 release/原单核对路由、目标 QCS/Kafka/material-wms/PostgreSQL/browser 全链验收、WMS 原命令核对验收、截图和新版 verifier。Phase 2 仍默认关闭，外部 QCS 主动事件与外部 ERP/WMS 补偿未完成前不得激活。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=1606`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 1606 条已分类引用、0 条未分类引用；`ShadowRunPostgresRepository`、`FeatureFlagPostgresRepository`、来源序列、点位校准及 `BatchReleasePostgresRepository` 的 Spring JDBC `rowNum` 回调参数均归类为 `java-row-index-noise`；PATROL 运行元数据引用归类为 `postgres-compat-reference`，JavaScript `rowNum` / `Number(...)` 均按非 SQL 噪声分类，真实 SQL 方言仍被阻断；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `187 PASS / 0 FAIL / 1 BLOCKED / 2 NOT_APPLICABLE`。独立不良数量 marker `ADP_E2E_20260717141017_WOM_BAD_QTY` 已形成 WOM/QCS/WMS/PostgreSQL 完整证据；历史 WOM 工具栏基线 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z` 继续保留。PATROL 计划/取消、配置 CRUD、执行、异常隐患、统计/监控和 Kafka 消费均有真实证据；material/WMS、ProcessAnalysis、生产列表导出、二维码、手工新建和 RM Web 编辑已闭合；BPI marker `BPI_LIVE_20260720_123058` 已闭合同一指令 START/END，V23 合同 4/4 之外又以两个 marker 闭合受控 QCS、目标 Kafka、query-first WMS adapter、material 单据/库存、durable receipt、真实页面、精确重放和双库清理。 | PATROL 技术消费链已通过，但测试库 `tmm_tags=0` 且无测点历史，无法验收 `gather_data` 中位数落库和误差图表；GIS 定位/轨迹缺 `SESGISConfig`，完整整改/复查/销项缺真实 SESH；public `produceTaskCreated` 产品范围决定、现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准、7-14 天影子验收、外部 QCS 主动事件及外部 ERP/WMS 补偿演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `190` 项，其中 `187 PASS / 0 FAIL / 1 BLOCKED / 2 NOT_APPLICABLE`；迁移库存为 `192 scripts`，192 增加 material-wms 的 BPI source/idempotency/unit 契约和精确唯一索引；BPI Flyway V23 由独立 Java 17 服务维护，不计入 ADP init SQL 库存。目标受控 QCS -> Kafka -> material-wms -> receipt 链已直接核对 BPI 与 material 两库，QCS/Kafka 重放不重复，取证后双 marker 零残留；`metadata/production-module-backlog.json` 当前只保留 1 个 public `produceTaskCreated` 产品范围项，其中 `0 FAIL_BACKLOG / 1 BLOCKED`。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台和 Flink 数据面已建立；Phase 1 继续保持 shadow-only。marker `BPI_LIVE_20260720_123058` 已用真实 WOM start/stop、受控 MQTT、Kafka/Flink/PostgreSQL 和真实浏览器闭合同一影子批次。Flyway V23 的 QCS required-inspection、`WAIT_QA/RELEASED/REJECTED`、WMS transactional outbox、durable receipt 和 fail-closed 合同已完成本地与目标 4/4 回归。实现提交 `1ce3cb996ff81556763283a5401f7c19554099c2` 又部署 material 迁移 192、query-first WMS adapter、API key 门禁和完整编排；受控认证 QCS marker 已通过目标 Kafka，在 `material-wms` 创建真实单据/明细/事务/库存，并由 durable receipt 将批次推进到 `INBOUNDED/r4`。相同 QCS 重放、强制 Kafka command 重放、真实页面五 API、空证据布局修复和双 marker 清理均通过。marker `ADP_E2E_20260721_015610_WMS_RECON` 又从真实批次页执行“重新核对原单”，证明原 command event/原 WMS 幂等键复用、管理员重放、revision 并发控制、审计/幂等/outbox 落库和零残留清理。证据在 `metadata/bpi-quality-release-wms-inbound-acceptance.json`、`metadata/bpi-quality-inventory-ui-acceptance.json`、`metadata/bpi-quality-release-wms-target-acceptance.json` 和 `metadata/bpi-wms-reconciliation-target-acceptance.json`。验收后 Phase 2/WMS 开关全关且 allowlist 为 deny-all。 | 当前仍缺物理 DEVICE/GATEWAY 与真实现场证书、真实连续 7-14 天影子运行、外部 QCS 实例主动事件与检验记录所有权联接、外部 ERP/WMS 冲销/宕机补偿、真实负载跨组件整体回切和受控生产激活；Phase 3/4 训练数据与建议型模型未开始。 |
