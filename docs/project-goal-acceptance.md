...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `287 PASS / 0 FAIL / 0 BLOCKED / 7 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V35 已闭合数据集/遥测链。工具栏最新复验 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`（task `9000006343993284`，`2026-06-22T13:21:24.930Z`）继续保留。2026-07-27 最小输送单元以五个持续信号闭合 BPI 影子批次；MES 以 `...CLOSED_10` 保留完全不续跑基线，并以最新 `...CLOSED_11` 在透明记录预启动分页恢复后完成制造指令、工序、投料及消耗、产出及产出台账、请检、合格、完工、WMS 入库和追溯八步真实页面/API/PostgreSQL 闭环。2026-07-28 marker `ADP_E2E_20260728042954_FACTORY_LINE` 又以纯可见页面交互和 `13/13 PASS` 完成工厂架构新增产线、提交接口、PostgreSQL 和 WOM 生产线参照联合验收；marker `ADP_E2E_FRUCTOSE_LINE_01` 进一步以 `TEST_ONLY_DRAFT` 完成淀粉浆、液化液、糖化液、喷射液化、糖化、九项检验定义、工序交接、QCS 放行、35.1 t 完工入库、批次谱系和 BPI 信号的五页面真实复验。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；果糖质量限值仍需工艺和 QA/QC 正式批准；现场 Batch/DCS 切换确认、连续运行和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `294` 项，其中 `287 PASS / 0 FAIL / 0 BLOCKED / 7 NOT_APPLICABLE`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V35、最小输送单元 START/END 影子批次、`...CLOSED_11` 的 WOM/QCS/WMS/ProcessAnalysis 主链、`...FACTORY_LINE` 的产线新增/参照链和 `ADP_E2E_FRUCTOSE_LINE_01` 的两工序物料谱系、质量放行、WMS/BPI 闭环均有真实落库证据；投料路径生成活动执行和物料消耗事实，产出路径精确生成两条产出事实。迁移 100/206 通过行锁保护运行视图更新并回收替换的大对象，迁移 211-216 闭合工厂树、编辑、节点类型参照、助记码别名与运行元数据，且 mapper 审计错误为 0。Phase 3C-G 复用 V32/V28 表契约，不新增 Oracle 路径或重复业务表。 | 当前已登记 PostgreSQL 缺口已进入幂等 SQL 或明确产品边界；不能证明未来发现项会自动闭环，需要持续记录 |
