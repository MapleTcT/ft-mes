...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；拓扑/规则产品化已通过 Flyway V1-V9、真实 PostgreSQL marker 和 7 条 Chromium E2E，支持页面新建/复制、结构化校验、独立管理员发布和拓扑绑定规则草稿；2026-07-14 目标 runtime/streaming Compose、ADP 会话桥接、三 broker Kafka、Flink/MinIO、TaskManager 恢复通过；marker `ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；`MapleTcT/iot@357a285b` 目标 EventBus marker 已到 Kafka/Flink source；MES production context 已通过真实 WOM 页面、outbox、Kafka/Flink join 和影子批次验收 | 本批 Flyway V9 和产品化界面尚需目标环境复验；真实网关/协议设备点位、IoT + MES context 同 marker candidate/batch、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
...
1. 把已通过的 BPI 同一 marker 联合验收固化为发布回归基线，并将已完成的 topology/rule 产品化与 Flyway V9 部署到目标测试环境复验。
