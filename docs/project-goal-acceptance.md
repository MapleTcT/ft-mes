...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 事件时间/上下文/Broadcast State/checkpoint、transactional outbox、回执和审计已建立；2026-07-14 目标 runtime/streaming Compose、ADP 会话桥接、三 broker Kafka、Flink/MinIO、TaskManager 恢复通过；同一 marker `ADP_E2E_20260714_091536_BPI_JOINT` 已闭合受控浏览器/outbox/Kafka/Flink/候选/影子批次联合链；`MapleTcT/iot@be89aecf` exporter 已通过 9/9 测试和 38 模块打包；MES production context outbox 已通过合同/模块 23 项测试及独立 PostgreSQL 活动、结束、revision、回滚验收 | 受控 Phase 1、exporter 与 context outbox 工程链已通过，但拓扑/规则产品化、IoT/MES 试点配置和目标机部署、真实 WOM marker/Flink join、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成 |
...
2. 基于 `MapleTcT/iot@be89aecf` 和已实现的 MES production context outbox 配置并部署一条试点产线，完成点位语义、单位、质量码、source epoch/sequence、WOM scope/state 映射及真实 marker/Flink join 核对后进入 7-14 天影子运行。
