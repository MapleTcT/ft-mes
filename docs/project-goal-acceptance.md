...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1848 个 CI/治理/runtime 依赖资产，其中 1848 tracked、0 untracked、0 missing references。本轮纳入 PostgreSQL QCS 显示兼容迁移 194-196、应用持有的字段同步迁移 197、追加式 material-wms 完工入库冲销迁移 198、默认关闭的 Java 8 QCS quality-gate sidecar、精确 BPI batch resolver、BPI Flyway V26 数据集清单、V27 Parquet materializer、V28 Iceberg/Polaris catalog publisher、V29 Object Lock retention archiver、V30 MLflow Dataset Input registrar、V31 离线训练准备度评估与隔离 recovery rehearsal、私有版本化 MinIO 编排、query-first WMS adapter，以及真实页面/API/PostgreSQL/MinIO/Polaris/MLflow 验收资产。内部 WMS 受控激活已验收并恢复关闭；V26-V31 已闭合 manifest、Parquet、Iceberg 页面/fencing、Object Lock 恢复包、MLflow Dataset Input、19 项训练前门槛、最小权限、隔离恢复/物理清除和零残留退场；连续生产启用、工艺信号窗口与现场数据量、整站灾备、MLflow 生产治理和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `250 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V31 已分别闭合 manifest、Parquet、Iceberg/Polaris、Object Lock 恢复包、MLflow Dataset Input 和训练准备度评估的目标页面、API、PostgreSQL、外部存储、失败重试/恢复、最小权限、重启与定向清理。V31 marker 证明 19 项门槛按真实数据失败关闭为 8 个明确 blocker，MLflow/model 无副作用且桌面/移动零错误。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需工艺信号窗口、物理设备同等序列、正式批准校准、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `253` 项，其中 `250 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V31 均有真实落库证据；V31 assessment/audit/idempotency、不可变 trigger、MLflow 零模型副作用和模型边界分别保存在 PostgreSQL/MLflow，并与 MinIO/Object Lock 对账，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 V26-V31 数据集链已有真实目标证据。V31 release `89044926c133` 由 marker `ADP_E2E_BPI_READINESS_20260723_091500_A1` 从真实数据集页面追加两次不可变 `BLOCKED` 评估：sequence `1/2`、同一确定性 checksum、19 项门槛、8 个明确阻断；PostgreSQL assessment/audit/idempotency、同键重放、不可变触发器、服务重启、桌面/移动页面和精确退场均通过。MLflow 保持既有 1 run/1 dataset/1 input，模型表仍为 0，训练、模型、推断和激活均未启动。 | Phase 3C-B 评估门槛已关闭，但工艺信号窗口特征、物理 DEVICE/GATEWAY、正式证书、样本/批次/生产日/切分组/类别覆盖、连续 7-14 天、外部 ERP/WMS、整站灾备与 MLflow 生产 RBAC/SSO/TLS/HA 仍未满足；下一步先补信号窗口与现场数据，不得绕过 V31 门槛或把 G-021 改为 READY。 |
