...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1410 个 CI/治理/runtime 依赖资产，其中 1410 tracked、0 untracked、0 missing references，覆盖全部可维护 source-module、PostgreSQL 迁移、真实验收和 verifier，本轮新增 BPI Flyway V21 运行开关治理、目标环境桌面/移动页面、API、PostgreSQL marker 清理、截图和 verifier 资产均已跟踪；`make ci` 包含 strict tracking，必需文件游离在 Git 外会直接失败。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、Flyway V20、操作台、Flink 数据面、transactional outbox、回执和审计已建立；规则生命周期、savepoint、目录与校准治理、数据质量自动链和应用回退均已闭合。目标 marker `ADP_E2E_SHADOW_20260720_0152_V20` 又从真实页面完成 `DRAFT/r1 -> RUNNING/r2 -> EVALUATING/r13 -> APPROVED/r14`，10 批复核得到 19/20 边界认同和累计量偏差 0%，关键事件阻断、独立批准、审计、幂等、外部写隔离和清理均通过，证据在 `metadata/bpi-shadow-run-acceptance.json`。8 天采用受控时间压缩，Phase 1 仍保持 shadow-only。 | 目标点位仍为 1 点/0 READY；当前仍缺与目录版本精确匹配的真实现场证书、强制且连续单调 DEVICE/GATEWAY source sequence、真实来源遥测 + MES context 同 scope candidate/batch、真实业务负载下跨组件整体回切、真实连续 7-14 天影子运行、QCS/WMS 写回和训练数据阶段。 |
