...
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `0 FAIL / 3 BLOCKED` 落库未闭合行；`metadata/production-module-backlog.json` 用 9 个 backlog 项覆盖全部未闭合落库行和工具栏动作 blocker，其中 `0 FAIL_BACKLOG / 9 BLOCKED`，`make production-module-backlog-check` 和 `make project-goal-acceptance-check` 会交叉校验该覆盖关系；`metadata/postgres-migration-inventory.json` 当前有 173 个脚本、`highRiskStatementCount=0`、`watchStatementCount=62`、`watchSafetyIssueCount=0`，且无缺号/重号/非法命名；RBAC 验收发现并闭合 `rbac_roleuser.valid` 默认值缺口、RBAC 数据资源权限缺表、角色资源权限继承用户行未落库缺口；WOM 生产动作追踪发现并闭合 `WOM_WAIT_PUT_RECORDS` 缺表、`RM_PROCESS_ACTIVES` 缺表、legacy `convert(proc_sort,int)` 和 process wait 结束时间同步缺口；QCS 请检/报告明细缺表已落到 `107-qcs-inspect-detail-tables.sql`；`166`/`167` 已闭合 WOM checkoutBill 触发的 PostgreSQL legacy LIKE 类型解析缺口；`docs/postgres-migration-watch-rationale.md` 当前登记 62 条 watch，安全问题 0；SQL 169 已闭合 customProperty 模型映射暴露的 legacy `project_property` 表名兼容缺口；SQL 171 已闭合 WTS 作业许可列表导出按钮 runtime 动作缺口；模块准入门禁现在校验 `scanPolicy/scanCoverage`，真实最新基础模块 report-only 账本记录 `textScanned=9784/9806`、`textSkippedLarge=22`、`nestedFileCount=13990`、`unsupportedArchiveCount=1`，防止新业务包因不透明压缩包或超大文本跳过而绕过 PostgreSQL/Oracle 方言治理 | 不能证明所有未来发现项已闭环，需要持续记录 |
