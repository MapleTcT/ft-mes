...
| G-013 | 生产模块完整功能 | BLOCKED | 生产模块测试矩阵当前 `41 PASS / 3 BLOCKED / 0 NOT_RUN`、44 条 route smoke 全部 PASS；material/WMS、ProcessAnalysis、生产列表导出、WOM 二维码和可见手工新建指令单均已通过真实浏览器/API/PostgreSQL 或文件响应验收。二维码 marker `ADP-WOM-UI-E2E-20260717084656-55303` 与 `ADP-WOM-E2E-20260717084642-55397` 分别完成 10/10 浏览器和 10/10 API/PostgreSQL 验收；手工新建 marker `ADP_E2E_20260717101030_WOM_MANUAL_ENTRY` 完成 9/9 验收并验证同请求幂等、活动批次唯一保护、提交和软删除。历史 WOM 工具栏基线 marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z` 仍保留，当前 8 个动作均 PASS。 | 3 条生产用例仍 BLOCKED：RM 外部 Batch 客户端和 PROD-019/PROD-021 独立不良数量范围确认；public `produceTaskCreated` 仍作为独立产品范围 backlog，不影响已验收的受控可见入口。 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `41 PASS / 3 BLOCKED / 0 NOT_RUN`；落库总账当前 `166 PASS / 0 FAIL / 2 BLOCKED / 2 NOT_APPLICABLE`。WOM 可见手工新建指令单以 marker `ADP_E2E_20260717101030_WOM_MANUAL_ENTRY` 完成 9/9 浏览器/API/PostgreSQL 验收；二维码真实点击、两张 PNG、PostgreSQL 两行、幂等、回填和清理均通过，工具栏 8/8 PASS。历史 WOM 工具栏基线 marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z` 继续保留。PATROL 计划/取消、配置 CRUD、执行、异常隐患、统计/监控和 Kafka 消费均有真实证据；material/WMS、ProcessAnalysis、生产列表导出、二维码与可见新建入口已闭合。 | PATROL 技术消费链已通过，但测试库 `tmm_tags=0` 且无测点历史，无法验收 `gather_data` 中位数落库和误差图表；GIS 定位/轨迹缺 `SESGISConfig`，完整整改/复查/销项缺真实 SESH；另有 3 条生产用例、public `produceTaskCreated` 产品范围决定和业务负责人签字未完成。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `170` 项，其中 `166 PASS / 0 FAIL / 2 BLOCKED / 2 NOT_APPLICABLE`；迁移库存为 `189 scripts`，PATROL 178-186 已完成目标复跑和真实功能验收，188 新增 WOM 二维码事务日序列和包装码表，189 新增 WOM 手工新建请求账本及活动批次 partial unique guard；`metadata/production-module-backlog.json` 用 4 个 active backlog 项覆盖未闭合落库行和产品范围 blocker，其中 `0 FAIL_BACKLOG / 4 BLOCKED` | 不能证明所有未来发现项已闭环，需要持续记录 |
| G-020 | 生产迁移前置项 | BLOCKED | 生产迁移 readiness、cutover 总闸门、9 轨 rehearsal、数据库行数/checksum 对账、MinIO/Keycloak/Nacos/runtime patch、回滚、license、network/TLS、安全加固和 business smoke 签字门禁均已建立；当前 cutover 为 `NOT_READY_FOR_PRODUCTION_CUTOVER`，9 个 gate 中 8 个 PLANNED、1 个 BLOCKED，生产账本为 3 个 blocker、4 个 backlog；rehearsal 为 `REHEARSAL_BLOCKED`，9 个 track 中 8 个 PLANNED、1 个 BLOCKED。`metadata/business-dependency-readiness-smoke.json` 已确认 material-service 与 process-analysis 2/2 READY；WOM 二维码与可见手工新建指令单已通过真实浏览器、API 和 PostgreSQL 验收并退出阻断账本。business smoke signoff 校验器会交叉校验全部 3 个 blocker case、4 个 backlog 和未解决导出 target，要求逐项解决或提供包含 `decision=ACCEPTED`、`acceptedBy`、`signedAt`、真实 `evidence` 的结构化风险接受；CI 仍拒绝模板证据和测试环境证据冒充生产 READY。 | 数据迁移/回滚演练、真实 license 决策、真实 MinIO/Keycloak 迁移、真实生产域名/TLS、安全加固、剩余产品范围/外部 Batch 客户端问题和真实业务 smoke 签字均未 READY；当前门禁只能证明“不允许切生产”及账本一致性。 |
