...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 数据面、transactional outbox、回执和审计已建立；目标环境已完成 Flyway V17。规则退役、savepoint、延迟 candidate、重复目录观测、准入撤销、单 broker 故障和应用组件双向回退均已闭合。JetLinks 试点产品/设备/metadata/单位已完成，marker `ADP_E2E_CAL_20260719_160131` 又从真实点位页闭合校准证据 `PENDING/r1 -> APPROVED/r2 -> REVOKED/r3`、同人审批 `422`、PostgreSQL 审计/幂等和非匹配版本 0 READY。Phase 1 仍保持 shadow-only。 | 目标点位仍为 1 点/0 READY，但平台注册、metadata、单位和软件侧校准治理缺口已关闭；当前仍缺与目录版本精确匹配的真实现场证书、强制且连续单调 DEVICE/GATEWAY source sequence、真实来源遥测 + MES context 同 scope candidate/batch、真实业务负载下跨组件整体回切、7-14 天影子运行、QCS/WMS 写回和训练数据阶段。 |
