...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 总设计、交互设计、API 目录和工程测试计划已落库；Java 8 适配器、Java 17 PostgreSQL 服务、BPI 操作台、Protobuf/OpenAPI、遥测/规则/拓扑/候选/影子批次、Flink 事件时间/生产上下文/Broadcast State/checkpoint、规则 transactional outbox、失败重试和审计已有本地验证；规则应用 `WAITING -> REJECTED -> APPLIED` 已通过本地真实 PostgreSQL 16.13、Flyway V1-V8、inbox 幂等和审计验收，流处理 reactor 78 项测试与 BPI service compile 已通过 | 仍缺回执 Kafka/Flink checkpoint/restart/DLQ/browser 联合验收、目标测试环境部署、IoT exporter 与 MES production context 现场联调、7-14 天影子运行、QCS/WMS 写回和训练数据阶段 |
