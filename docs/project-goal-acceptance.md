...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`make sustainable-check` 会校验生成物忽略规则并阻断误跟踪缓存/构建产物；`metadata/ci-required-file-inventory.json` 当前记录 828 个 CI/治理/runtime 依赖资产，其中 828 tracked、0 untracked、0 missing references，覆盖部署、smoke、PostgreSQL、全部可维护 source-module、BPI 事件/API 契约、Java 17/PostgreSQL 影子服务、Java 8 认证适配层、BPI 操作台浏览器验收与模拟验收、遥测 replay staging 与落库验收、事件时间边界规则运行时验收、迁移、运行补丁、业务模块、ProcessAnalysis 验收、核心链回归、blocker/backlog 和 verifier；`make ci` 包含 strict tracking，必需文件游离在 Git 外会直接失败 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
