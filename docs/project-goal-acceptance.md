...
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `178` 项，其中 `175 PASS / 0 FAIL / 1 BLOCKED / 2 NOT_APPLICABLE`；迁移库存为 `191 scripts`，191 新增 WOM 报告/事件账本、WMS 分配/事件账本、合格/冻结数量拆分和 WOM/QCS 可见入口；BPI Flyway V15 由独立 Java 17 服务维护，不计入 ADP init SQL 的 191 项库存；`metadata/production-module-backlog.json` 当前只保留 1 个 public `produceTaskCreated` 产品范围项，其中 `0 FAIL_BACKLOG / 1 BLOCKED`。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 设计、契约、Java 8 adapter、Java 17 PostgreSQL 服务、操作台、Flink 数据面、transactional outbox、回执和审计已建立；目标环境已完成 Flyway V15，当前 Flink job `0a2dd090eb290f82d052fc7c0465311f` 为 `RUNNING/33-of-33`。marker `ADP_E2E_20260718_065300_BPI_RETIRE_V15B` 已闭合规则 `PUBLISHED/r4 -> RETIRED/r5`、Kafka `ACTIVATE -> RETIRE`、Flink `APPLIED + READY -> INACTIVE`、savepoint、退役后延迟 candidate 唯一落库、真实 candidate 页面和 11 类 marker 清理；浏览器错误为 0，Phase 1 仍保持 shadow-only。 | 目标点位仍为 1 点/0 READY，试点设备未注册/激活且 metadata/单位/标定/连续单调 sequence 未就绪；真实来源遥测 + MES context 同一试点 scope 的 candidate/batch、broker 故障与 service/adapter/Flink 镜像及产品级回退、7-14 天影子运行、QCS/WMS 写回和训练数据阶段仍未完成。 |
