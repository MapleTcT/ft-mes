...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`make sustainable-check` 会校验生成物忽略规则并阻断误跟踪缓存/构建产物；`metadata/ci-required-file-inventory.json` 当前记录 983 个 CI/治理/runtime 依赖资产，其中 983 tracked、0 untracked、0 missing references，覆盖部署、smoke、PostgreSQL、全部可维护 source-module、BPI 事件/API 契约、Java 17/PostgreSQL 影子服务、Java 8 认证适配层、BPI 操作台浏览器验收与模拟验收、遥测 replay staging 与落库验收、事件时间边界规则运行时验收、Java 17/Flink 流式回放和 checkpoint 恢复验收、候选 Protobuf 完整证据落库确认验收、Kafka candidate consumer/DLQ/PostgreSQL 验收、规则发布事务 outbox/PostgreSQL/Kafka 联合验收、失败发布管理员重试审计和发布状态可见性、受控 Kafka/Flink/PostgreSQL marker 联合回放脚本、IoT 信号到 Protobuf/确认/PostgreSQL 的跨模块运行时验收、规则版本化 watermark/迟到/timeout 语义验收、规则发布 Protobuf/AsyncAPI、point-in-time 生产上下文、keyed 双输入等待、Kafka 元数据解码、规则生命周期、作用域索引路由、稳定 UID、双事务 sink、可部署 shaded job、独立 Kafka/Flink Compose、容量预检、集群 smoke 和 marker-scoped read-committed replay 验收、迁移、运行补丁、业务模块、ProcessAnalysis 验收、核心链回归、blocker/backlog 和 verifier；`make ci` 包含 strict tracking，必需文件游离在 Git 外会直接失败 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=984`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 984 条已分类引用、0 条未分类引用；最近新增分类引用来自 BPI 适配器和流式模块校验器中的 Oracle 禁止规则，不是运行依赖；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
1. 处理生产矩阵剩余 6 条 BLOCKED：保持 material/WMS 回归，补外部 Batch 客户端、ProcessAnalysis 等业务包和入口；业务包部署后先跑 `make business-package-scan`、`make smoke-business-dependencies` 和 `make smoke-production-export-readiness`，再从真实前端复验手工创建、库存入库、坏品数量、追溯、RM 手工编辑和导出。
