...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 总设计、交互设计、API 目录和工程测试计划已落库；Java 8 适配器、Java 17 PostgreSQL 服务、BPI 操作台、Protobuf/OpenAPI、遥测/规则/拓扑/候选/影子批次、Flink 事件时间/生产上下文/Broadcast State/checkpoint、规则 transactional outbox、失败重试和审计已有本地验证；规则应用 `WAITING -> REJECTED -> APPLIED` 已通过真实 PostgreSQL 16.13、Flyway V1-V8 和确定性浏览器验收；Embedded Kafka 3.8.1 + PostgreSQL 联合测试覆盖消费重启/幂等/DLQ；Kafka 4.2 + Flink 2.2.1 MiniCluster 真实作业测试覆盖未提交事务不可见、成功 checkpoint 提交、TaskManager 重启恢复和规则终态保护 | 本地 PostgreSQL、浏览器、Kafka 消费和 Flink job 仍是分离证据；尚缺目标三 broker Kafka/Flink/MinIO 与浏览器到 Java/PostgreSQL 联合验收、目标测试环境部署、IoT exporter 与 MES production context 现场联调、7-14 天影子运行、QCS/WMS 写回和训练数据阶段 |
...
1. 闭合 BPI 规则发布 `Outbox -> Kafka -> Flink checkpoint -> application receipt -> PostgreSQL/audit -> UI`，补真实数据库、重复/冲突/DLQ、checkpoint/restart 和浏览器证据。
2. 将 BPI 部署到目标测试环境，并接入一条产线的 IoT exporter 与 MES production context，完成点位语义、单位、质量码和 sequence 核对后进入 7-14 天影子运行。
