...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `39 PASS / 4 BLOCKED / 0 NOT_RUN`；落库总账当前 `142 PASS / 0 FAIL / 3 BLOCKED / 2 NOT_APPLICABLE`。WOM 最新工具栏 marker 为 `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、`generatedAt=2026-06-22T13:21:24.930Z`；PATROL marker `ADP_E2E_20260716155413_PATROL` 已在重新应用 178-185 后通过计划保存、任务生成、真实查询、批量取消、PostgreSQL 回读与状态复显 22/22 断言；录入标准 marker `ADP_E2E_20260716165523_PATROL_INPUT` 已通过新增、修改、停用、启用、删除和 PostgreSQL 回读 16/16 断言；material/WMS 与 ProcessAnalysis 已闭合。 | PATROL 完整路线 CRUD、下发/执行/完成、异常和统计仍待验收；另有 4 条生产用例、二维码运行端点、生产导出其余目标和业务负责人签字未完成。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `147` 项，其中 `142 PASS / 0 FAIL / 3 BLOCKED / 2 NOT_APPLICABLE`；迁移库存为 `185 scripts / 0 high-risk / 71 watch / 0 watch safety issue`，PATROL 178-185 已完成目标幂等复跑和 verify PASS；`metadata/production-module-backlog.json` 用 7 个 backlog 项覆盖全部未闭合落库行和工具栏动作 blocker，其中 `0 FAIL_BACKLOG / 7 BLOCKED` | 不能证明所有未来发现项已闭环，需要持续记录 |
