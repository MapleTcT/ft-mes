...
| ID | 目标项 | 状态 | 当前证据 | 缺口 |
| --- | --- | --- | --- | --- |
| G-001 | 可持续开发仓库基础 | READY | 父 POM、Makefile、`.gitignore`、CI、库存和门禁已建立；`metadata/ci-required-file-inventory.json` 当前记录 1893 个 CI/治理/runtime 依赖资产，其中 1893 tracked、0 untracked、0 missing references。本轮纳入 BPI Flyway V34 遥测窗口索引、default-off 精确 scope Kafka consumer、目标 MQTT/PostgreSQL/浏览器验收脚本和证据，并保留 V26-V33 数据集、MLflow、工艺窗口与权限基线。内部 WMS 受控激活已验收并恢复关闭；V26-V34 已闭合 manifest、Parquet、Iceberg、Object Lock、MLflow Dataset Input、19 项训练前门槛、受控 point-in-time 窗口、最小权限、MQTT 窗口落表和零残留退场；连续生产启用、物理现场窗口与数据量、整站灾备、MLflow 生产治理和外部 ERP/WMS 补偿仍未完成。 | 继续随模块提升补测试；新增或移动治理资产后继续运行 `make ci` |
| G-002 | 当前内容迁移 | PARTIAL | `docs/current-content-inventory.md` 和源码恢复目录 | 业务模块动作级源码/表关系仍需继续排查 |
| G-003 | Oracle 替换为 PostgreSQL 默认路径 | PARTIAL | Docker/POM 默认 PostgreSQL；`metadata/oracle-replacement-status.json` 当前汇总 `blockingIssueCount=0`、`runtimeConfigActiveOracleLineCount=0`、`directOracleDependencyCount=2`、`oracleBacklogReferenceCount=1627`、`postgresMigrationHighRiskCount=0`、`postgresMapperAuditErrorCount=0`，PostgreSQL 迁移数为 `198`；Oracle 只允许 legacy/profile/backlog；`metadata/oracle-migration-audit.json` 当前记录 1627 条已分类引用、0 条未分类引用。新增 V34 文档政策引用均归类为 `documentation-or-workflow`，验收 SQL 的 PostgreSQL `decode(..., 'hex')` 归类为 `postgres-conversion-tooling`；既有 Spring JDBC `rowNum` 回调仍归类为 `java-row-index-noise`，不是 Oracle 默认路径。PATROL 运行元数据引用归类为 `postgres-compat-reference`，真实 SQL 方言仍被阻断；模块准入门禁继续阻断不可检查包和 Oracle 默认路径 | Oracle backlog 仍有引用，需要逐模块迁移 |
...
| G-018 | 业务模块完整测试用例 | PARTIAL | 53 个既有业务页面 smoke 已有证据；生产模块矩阵当前 `44 PASS / 0 BLOCKED / 0 NOT_RUN`；落库总账当前 `264 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`。既有生产、质量、PATROL、material/WMS 与 BPI 证据保持有效；V26-V34 已分别闭合数据集链、训练准备度、受控工艺窗口、权限加固和受控 MQTT 窗口落表。marker `BPI_TLANDING_20260723_094606` 证明预热排除、窗口内 2 events/2 points/0 rejects、五项 `1/1` 与页面/清理通过。历史 WOM 工具栏锚点继续保留：marker `ADP_E2E_20260622131959_WOMSTART_HOLD_RESTART`、taskId `9000006343993284`、generatedAt `2026-06-22T13:21:24.930Z`。 | PATROL 仍缺真实测点历史、`SESGISConfig` 和真实 SESH；现场 Batch/DCS 切换确认和业务负责人签字未完成；BPI 还需物理设备同等序列、正式批准校准下的窗口、200 批/7 天、7-14 天影子验收、整站灾备/容量、MLflow 生产安全、Phase 4 模型及外部 ERP/WMS 演练。 |
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `267` 项，其中 `264 PASS / 0 FAIL / 0 BLOCKED / 3 NOT_APPLICABLE`；迁移库存为 `198 scripts`、`78 watch statements`、`0 watch safety issues`。字段约束、标量/OBJECT 字段生命周期、BPI V26-V34 均有真实落库证据；V34 复用既有 telemetry 表并增加窗口索引，受控 MQTT 已由 PostgreSQL 复验预热排除、窗口内 event/point/reject、来源序列、质量、校准、取消与清理，不引入 Oracle 默认路径。旧 public `produceTaskCreated` 已按 `NOT_APPLICABLE` 关闭，active production backlog 为 0。 | 不能证明所有未来发现项已闭环，需要持续记录 |
...
| G-021 | 智能批次与工艺数据中心（BPI） | PARTIAL | BPI 控制面、数据面、质量/WMS 内部链与 V26-V34 数据/遥测链已有真实目标证据。V31 保留不可变 `BLOCKED` 历史评估；V32/V33 固化 6 条工艺窗口事实；Phase 3C-D 将来源与训练数据量拆开。Phase 3C-E 产品提交 `8c9c4192b179` 和 marker `BPI_TLANDING_20260723_094606` 又闭合受控 MQTT -> JetLinks -> Kafka -> scoped consumer -> PostgreSQL -> 真实页面：预热 3 条被窗口排除，窗口内 2 events/2 GOOD points/0 rejects，五项 `1/1`、event/observation `2/2`、零 GAP/乱序；29 个响应全 2xx，取消、清理和默认恢复通过。MLflow/model 路径仍为 0/false。 | Phase 3C-C/D/E 的受控实现已关闭，但模拟器和临时校准不是物理 DEVICE/GATEWAY、正式证书、容量或真实数据资格。仍需物理来源断电重连、正式校准、多点/多产线容量、至少 200 个复核批次、7 个生产日、正负标签、连续 7-14 天、外部 ERP/WMS、整站灾备与 MLflow 生产 RBAC/SSO/TLS/HA；不得把 G-021 改为 READY。 |
