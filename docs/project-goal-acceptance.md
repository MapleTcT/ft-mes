...
| G-019 | PostgreSQL 缺口进入幂等 SQL/backlog | PARTIAL | Oracle/PostgreSQL 审计、init SQL、backlog 和 watch rationale 已纳入 CI；`metadata/persistence-acceptance.json` 当前有 `314` 项，其中 `294 PASS / 1 FAIL / 0 BLOCKED / 19 NOT_APPLICABLE`。既有 WOM/QCS/WMS/ProcessAnalysis、产线、果糖试运行、BPI 与配置兼容证据继续保留；迁移 224-234 补齐 TeamInfo admin 权限、OEE 工厂树层级、WTS `oid ~~ bigint` 窄兼容、移动设备 token 表、WOM 产耗详情视图、QCS 检验申请配置大对象兼容，以及 WTS 基础设置动作布局/大对象/权限/标签/脚本入口。当前迁移账本为 234 个脚本、0 个高风险语句、89 个受控观察项、0 个观察项安全问题，且 mapper 审计错误为 0。本轮 WTS 作业时间约束 marker 已完成新增、修改和软删除查库；移动 token 缺表已关闭，但真实在线场景仍有 `NOTIFY-001`：站内信 7/7、移动通知 7/7 均为 `send_status=0`，已进入 1 条 `FAIL_BACKLOG`，不能用消息列表 HTTP 200 代替通知送达。 | 修复 msgmanagement 通知 provider 与在线会话链，用新 marker 完成浏览器/API/PostgreSQL 复验；WTS 其余 10 个基础设置表单继续补写操作验收；后续每发现新 PostgreSQL 缺口仍需追加幂等 SQL 或 backlog |
