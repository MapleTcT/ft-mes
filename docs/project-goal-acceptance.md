...
| G-007 | 用户/人员/岗位/公司/组管理 | PARTIAL | 组织部门 CRUD 已落库；组管理 CRUD 已落库；岗位 CRUD 已落库；组织只读 smoke 通过 | 用户、人员、公司 CRUD 落库未补齐 |
| G-008 | 组织管理 | PARTIAL | 部门、组、岗位新增/编辑/删除已用 marker 查 PostgreSQL | 组织域其它对象和权限页仍需动作级验收 |
...
2. 补用户、人员、公司、权限配置页面的真实前端 CRUD 与 PostgreSQL 落库验收。
3. 补基础配置页面的写动作落库验收。
