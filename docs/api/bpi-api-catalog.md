...
| 训练数据 | POST | `/bpi/v1/dataset-materializations/{materializationId}/catalog-publications` | `requestDatasetCatalogPublication` | SERVICE_IMPLEMENTED；只接受已校验、非空 `READY` Parquet，按服务端固定 catalog/namespace/table 合同进入 `QUEUED`；目标后端检查点 PASS，真实 ADP 页面待验收 |
