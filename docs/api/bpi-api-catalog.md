...
- 当前 OpenAPI 有 93 个公开操作，其中 67 个由 Java 17/PostgreSQL 服务实现、76 个纳入确定性模拟；另有候选 JSON、候选 Protobuf、遥测 3 个内部接入端点。其余模拟操作仍不能视为后端已实现。
...
| 工序证据 | GET | `/bpi/v1/process-evidence` | `getProcessEvidence` | TARGET_ACCEPTED；按受信 tenant/plant/line 和不超过 24 小时的 MES 工序窗口读取 PostgreSQL 历史序列，最多 16 个属性筛选、每序列 240 个下采样点；`orderId` 只作上下文标签 |
...
| 训练数据 | POST | `/bpi/v1/dataset-catalog-publications/{publicationId}/retry` | `retryDatasetCatalogPublication` | TARGET_ACCEPTED；仅 `FAILED` revision 可重试，publisher 必须先 reconcile 带同一 BPI source properties 的既有 snapshot；目标 post-commit exit 86 与 stale-claim 已恢复到同一单一 snapshot |
