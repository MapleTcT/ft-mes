...
- Generated At：`2026-07-30T11:25:19+00:00`。
- Repo Commit：`5c5da24cba229c32ad56f086780869a844d4d0d1`。
- CI 阻断问题：`0`。
- 迁移缺口：`1`。
- 关注项：`2`。
- 计划项：`1`。
- 已提升源码模块：`10`。
- 源码模块 Oracle 禁入：`pass`。
- Oracle backlog 引用：`1636`。
- 直接 Oracle 依赖：`2`。
- PostgreSQL migration 脚本：`229`。
...
| oracle-legacy-only | watch | no | Oracle migration backlog has 1636 tracked references. | 逐模块清理 backlog；删除引用前必须保留 PostgreSQL 替代证据。 |
| oracle-audit-current-and-classified | pass | no | generatedAt=2026-07-30T11:25:01+00:00, repoCommit=5c5da24cba229c32ad56f086780869a844d4d0d1, unclassified=0, findingCount=1636, categoryTotal=1636 | 先运行 `make oracle-audit`；新增 Oracle 引用必须分类到 backlog、legacy、tooling 或文档路径。 |
...
| postgres-migration-governance | pass | no | 229 scripts, range=001-229, highRisk=0, watch=89 | 新增 SQL 只能追加编号并保持幂等；watch 语句在 PR 中解释。 |
...
| documentation-or-workflow | 194 |
| frontend-row-index-noise | 4 |
| java-row-index-noise | 93 |
| legacy-ojdbc-dependency | 6 |
| legacy-oracle-sql-resource | 160 |
| postgres-compat-reference | 577 |
| postgres-conversion-tooling | 49 |
