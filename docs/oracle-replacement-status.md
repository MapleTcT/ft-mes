...
- Generated At：`2026-07-17T06:08:10+00:00`。
- Repo Commit：`dff1e812843721d5ff5c84291a145bd4b9ee6c96`。
- CI 阻断问题：`0`。
- 迁移缺口：`1`。
- 关注项：`2`。
- 计划项：`1`。
- 已提升源码模块：`5`。
- 源码模块 Oracle 禁入：`pass`。
- Oracle backlog 引用：`1564`。
- 直接 Oracle 依赖：`2`。
- PostgreSQL migration 脚本：`187`。
...
| oracle-legacy-only | watch | no | Oracle migration backlog has 1564 tracked references. | 逐模块清理 backlog；删除引用前必须保留 PostgreSQL 替代证据。 |
| oracle-audit-current-and-classified | pass | no | generatedAt=2026-07-17T06:02:52+00:00, repoCommit=83cc542992ecb75b23f211d8341e3bfd5c1b3ee3, unclassified=0, findingCount=1564, categoryTotal=1564 | 先运行 `make oracle-audit`；新增 Oracle 引用必须分类到 backlog、legacy、tooling 或文档路径。 |
...
| postgres-migration-governance | pass | no | 187 scripts, range=001-187, highRisk=0, watch=71 | 新增 SQL 只能追加编号并保持幂等；watch 语句在 PR 中解释。 |
| recovered-source-inventory | pass | no | 250 source jars, 991 frontend files, 59 compose services. | 新增包、服务或 source map 后运行 `make inventory`。 |
| source-module-promotion | watch | no | `backend/source-modules` currently declares 5 buildable modules. | 按 auth/rbac/organization/configuration/workflow 顺序提升高频维护模块。 |
| source-module-oracle-policy | pass | no | Source module verification passed. Modules: 5. | 修复 `backend/source-modules` 中的 Oracle JDBC、Oracle 默认配置、Oracle dialect 或 mapper/oracle 资源后重新运行 `make source-module-check`。 |
...
| documentation-or-workflow | 185 |
| frontend-row-index-noise | 4 |
| java-row-index-noise | 25 |
