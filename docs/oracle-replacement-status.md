...
| oracle-legacy-only | watch | no | Oracle migration backlog has 687 tracked references. | 逐模块清理 backlog；删除引用前必须保留 PostgreSQL 替代证据。 |
...
| source-module-promotion | gap | no | `backend/source-modules` currently declares 0 buildable modules. | 按 auth/rbac/organization/configuration/workflow 顺序提升高频维护模块。 |
