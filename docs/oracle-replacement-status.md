...
- 计划项：`1`。
- 已提升源码模块：`1`。
- Oracle backlog 引用：`810`。
- 直接 Oracle 依赖：`2`。
- PostgreSQL migration 脚本：`70`。
...
| oracle-legacy-only | watch | no | Oracle migration backlog has 810 tracked references. | 逐模块清理 backlog；删除引用前必须保留 PostgreSQL 替代证据。 |
...
| postgres-migration-governance | pass | no | 70 scripts, range=001-070, highRisk=0, watch=44 | 新增 SQL 只能追加编号并保持幂等；watch 语句在 PR 中解释。 |
