...
- 计划项：`1`。
- 已提升源码模块：`1`。
- Oracle backlog 引用：`827`。
- 直接 Oracle 依赖：`2`。
- PostgreSQL migration 脚本：`73`。
...
| oracle-legacy-only | watch | no | Oracle migration backlog has 827 tracked references. | 逐模块清理 backlog；删除引用前必须保留 PostgreSQL 替代证据。 |
...
| postgres-migration-governance | pass | no | 73 scripts, range=001-073, highRisk=0, watch=44 | 新增 SQL 只能追加编号并保持幂等；watch 语句在 PR 中解释。 |
