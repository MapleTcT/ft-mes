...
5. MES 上下文真实链和 IoT MQTT source 分段链均已通过；点位准入变为 READY 后，用物理设备替换受控 marker，并与 WOM context 使用同一生产指令完成 Kafka、Flink、BPI PostgreSQL START/END candidate/batch 和浏览器证据链，再连续运行 7-14 天影子批次。
