...
当前仓库不再以“把所有恢复模块全部修到可见”为目标，而是采用共享底座、领域产品和组合方案的
模块化产品线。ADP/BAP 恢复层是兼容基础，不是对外的大而全产品承诺：

1. **共享底座**：Platform Foundation 和 Manufacturing Foundation 统一身份、权限、审计、文件、工作流和制造主数据。
2. **领域产品**：MES、QMS、生产仓储、追溯、BPI、Lab、可靠性、EHS 和 Energy 独立声明成熟度、硬依赖和发布门禁。
3. **商业组合**：`MES Start`、`MES Standard`、`Process Intelligence` 等只组合已登记产品，不制造新的代码依赖。
4. **行业方案**：食品流程、精细化工、离散、实验室、设备和安环通过 manifest、模板和适配器装配，不长期分叉核心代码。

产品依赖、成熟度和组合关系以
[FT 制造软件模块化产品矩阵](product/ft-mes-modular-product-portfolio.md) 和
[`metadata/product-portfolio-matrix.json`](../metadata/product-portfolio-matrix.json) 为准；核心业务体验以
[FT MES 精简标准版产品说明书](product/ft-mes-lean-standard-product-manual.md) 为准；逐项保留、合并、行业化、隐藏或退役的决定以
[功能裁剪与行业分支策略](product/ft-mes-capability-rationalization.md) 和
`metadata/product-capability-rationalization.json` 为准。

既有仓库主体仍来自 ADP/BAP 平台运行包，不等于原厂完整 MES 业务产品；BPI 则是本仓库中按可编译源码、版本化契约和真实验收新建的产品模块，不能与恢复代码的完整度混为一谈。恢复资产仍然保留用于兼容、取证和行业包建设，但默认菜单是否出现由版本能力清单决定。
...
标准版固定八个一级入口：工作台、生产执行、质量管理、物料库存、批次追溯、智能批次、主数据和
系统管理。原 WOM 记录页、QCS 多套检验页、RM 多种配方页和 LIMS 样品页优先合并为任务型工作区，
不再逐个修复所有旧菜单。低代码实体/视图/脚本/流程设计器、Redis/接口/微服务配置和许可管理转入
实施控制台。EAM、PATROL、OEE、WTS、完整 LIMS 和 EMS 进入行业包或保持隐藏。

裁剪执行遵循“隐藏 -> 观察 -> 解耦 -> 退役”：第一阶段只改变 edition manifest、菜单、权限和
Compose profile，不删除历史源码、表和运行资产。物理删除必须有依赖图、一个发布周期观测、数据导出、
恢复演练和标准核心回归。
...
相同冻结事实 checksum 一致；浏览器全套 20/20。随后 exact revision `e116580a` expand-only 部署到
`10.11.100.17`，marker `ADP_E2E_BPI_DATASET_TARGET_20260722_055000_A7C4` 又闭合真实 ADP 页面、
...
2. marker `ADP_E2E_20260727_MES_FULL_CLOSED_10` 保留从空 marker 开始且完全不续跑的基线；最新
   `ADP_E2E_20260727_MES_FULL_CLOSED_11` 在透明记录业务动作前分页恢复的前提下，以精确计数再次闭合
   制造指令、投料/报工、投入/产出台账、请检、合格处置、完工入库和批次追溯。保持双基线为发布
   回归，不因受控测试通过就宣称生产 READY。
3. 当前投料结束会补建活动执行记录并生成 `wom_mat_consum_recods`，真实页面也不再误报空白警告；下一步用真实主数据和现场 Batch/DCS 重跑同一流程，并完成连续运行、迁移演练和业务签字，不扩散到低优先级模块。
4. 同步更新前端报告、后端落库报告和机器可读账本；接口 `200` 但未落库必须记为 `FAIL`。
