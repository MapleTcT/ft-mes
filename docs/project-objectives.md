...
2. 将已完成的拓扑/规则产品化和 Flyway V9 部署到目标测试环境，执行真实 ADP 会话、API、PostgreSQL marker 与回滚复验；日常配置不得回退到 SQL fixture。
