...
| --- | --- | --- | --- | --- |
| `businessDependencyReadiness` | `metadata/business-dependency-readiness-smoke.json` | `2026-07-10T09:18:52.471Z` | `READY` | baseUrl=http://100.99.133.43:18080, blocked=0, dependencies=2, ready=2, sshHost=100.99.133.43 |
...
| `cutoverGate` | `metadata/production-cutover-gate.json` | `2026-07-17T10:22:51+00:00` | `NOT_READY_FOR_PRODUCTION_CUTOVER` | blocked=1, notStarted=0, planned=8, productionBacklogItems=4, productionBlockers=3, ready=0, totalGates=9 |
...
| `migrationReadiness` | `metadata/production-migration-readiness.json` | `2026-07-17T10:22:51+00:00` | `NOT_READY_FOR_PRODUCTION_MIGRATION` | blocked=1, notStarted=0, planned=8, ready=0, totalTracks=9 |
...
| `productionBacklog` | `metadata/production-module-backlog.json` | `2026-07-17T09:51:20Z` | `BLOCKED` | blocked=4, failBacklog=0, totalItems=4 |
...
| `business-smoke-signoff` | `BLOCKED` | `BLOCKED` | `BLOCKED` | `make business-package-scan`<br>`make smoke-business-dependencies ADP_BASE_URL=/production-or-rehearsal-url`<br>`make smoke-production-export-readiness ADP_BASE_URL=/production-or-rehearsal-url`<br>`make production-business-smoke-signoff-ready-check BUSINESS_SMOKE_SIGNOFF=/secure/path/adp-business-smoke-signoff.json` | 剩余 3 条生产用例仍 BLOCKED：RM 外部 Batch 客户端显式编辑，以及 PROD-019/PROD-021 独立不良数量产品范围确认。<br>business smoke signoff READY 校验必须覆盖 metadata/production-module-blockers.json 的每个 PROD-* 阻断 case；未解决 case 需要逐项签署结构化风险接受，riskAcceptance 必须包含 decision=ACCEPTED、acceptedBy、signedAt 和真实 evidence。<br>business smoke signoff READY 校验必须覆盖 metadata/production-module-backlog.json 的每个未闭合 backlog item；public produceTaskCreated 这类 HTTP 200 但不落库的假成功不能被生产签字遗漏。<br>business smoke signoff is blocked by unresolved product-scope/external-client items and the absence of signed production evidence; material/ProcessAnalysis/QR/visible WOM manual entry are no longer dependency blockers.<br>业务负责人尚未基于真实主流程签字。<br>Production module blocker ledger still contains 3 BLOCKED cases and production backlog still contains 4 unresolved items; signed production business-smoke evidence is not available. |
